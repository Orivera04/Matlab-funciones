function [myield, beyield] = mbsoas2yield(varargin)
%MBSOAS2YIELD Price of mortgage pool from OAS and a spot curve.
%   Yield of mortgage pool given OAS, a spot curve,
%   and prepayment vector.
%
% [myield, beyield] = mbsoas2yield(ZeroRates, OAS, Settle, Maturity, ...
%   IssueDate, GrossRate)
%
% [myield, beyield] = mbsoas2yield(ZeroRates, OAS, Settle, Maturity, ...
...
% 
% OAS       = [17.1810; 18.6678; 20.0811; 22.6267];
% Settle    = datenum('20-Aug-2002');
...
% Delay = 14;
% Interpolation = 1;
% PrepaySpeed = [0 50 100 200];
% 
% [myield beyield] = mbsoas2yield(ZeroMatrix, OAS, Settle, ...
%    Maturity, IssueDate, GrossRate, CouponRate, Delay, Interpolation, PrepaySpeed)

%   Copyright 2003 The MathWorks, Inc.
%   $Revision: 1.8.6.3 $  $Date: 2003/10/23 12:15:18 $

if nargin <6
    error('finfixed:mbsoas2yield:invalidInputs',['Need at least ZeroMatrix, ZVOAS,',sprintf('\n'),... 
    ...
    ZVOAS     = varargin{2}/10000; %unit-ized the price
    Settle    = datenum(varargin{3});
    
    if any(Settle ~= Settle(1))
        error('finfixed:mbsoas2yield:invalidSettle',['Please make sure that all Settle date are the same, ',...
...
if nargin <9 | isempty(varargin{9})
    ...
    if any(Interpolation ~=Interpolation(1))
      error('finfixed:mbsoas2yield:invalidInterpolation',['Please make sure that all Interpolation is of ONE type, ',...
...
if customized
    if ~isempty(varargin{10})
      error('finfixed:mbsoas2yield:invalidPrePaySpeed',['Cannot use benchmark when supplying customized prepayment CPR.',...
             sprintf('\n'),...
    ...
    if isempty(varargin{11})
      error('finfixed:mbsoas2yield:invalidPrePayMatrix',['Please supply a prepayment (SMM) matrix when',... 
...
if any(Settle > min(CurveDates))
  error('finfixed:mbsoas2yield:invalidSpotCurve',['Settle is less than the earliest point in the Spot curve.',...
         sprintf('\n') ... 
...
if any(crc_Maturity > max(CurveDates))
  error('finfixed:mbsoas2yield:invalidMaturity',['Maturity must be both within CurveDates to', sprintf('\n'), ...
...
price = CFlowAmounts .*  ...
    (1./(1 + spotrates/12 + ZVOAS(:, ones(NumCF,1)))).^TFactors;
price(isnan(price)) = 0;
price = sum(price,2);

% Guess that the monthly yield is close 
% is close to monthly coupon. 
x0 = zeros(size(CouponRate)); 

options1 = optimset('Display','off');
X = fsolve(@mbsintfun, x0, options1, CFlowAmounts, TFactors, ...
        spotrates, ZVOAS, NumCF, price);

myield = ( 1./X - 1 ).*12; % Mortgage Yield
beyield = 2*((1+myield/12).^6 - 1); % BE Yield

function y = mbsintfun(x, CF, TF, spotrates, ZVOAS, b, price)
% This internal function computes the NPV
% of cash flows. The correct discount will
% sum dcf to zero after substracting that
% with price given spotrates and OAS.
