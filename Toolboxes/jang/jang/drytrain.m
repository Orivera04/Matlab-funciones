...
%input_index = [1 8];		% obtain from drypick2.m
input_index = [1 7 8];		% obtain from drypick.m
%input_index = [1 2 7];		% obtain from drypick3.m
%input_index = [1 3 7 8];	% obtain from drypick4.m
%input_index = [1 2 7 8];	% obtain from afss2.m
...
title(['(a) Training Data (Solid Line) and ANFIS Prediction (Dots) with RMSE = ' num2str(rmse)]);
xlabel('Time Index'); ylabel('');
%legend('Desired curve', 'ANFIS prediction');

subplot(2,1,2);
index = trn_data_n+1:size(data,1);
plot(index, data(index, size(data,2)), '-', ...
     index, y_hat(index), '.');
rmse = norm(y_hat(index)-data(index,size(data,2)))/sqrt(length(index));
title(['(b) Test Data (Solid Line) and ANFIS Prediction (Dots) with RMSE = ' num2str(rmse)]);
xlabel('Time Index'); ylabel('');
%legend('Desired curve', 'ANFIS prediction');
