...
n=menu('Seleccionar un caso','Centro origen,radio:r','Centro:(h,k),radio:r',...
    ...
    'Pasa por A(x0,y0),Centro:(h,k)','Extremos de un diámetro: A(x0,y0),B(x1,y1)',...
...
switch n
    case 1
        ...
        rcuad=r^2;
        fprintf('\nLa ecuación es: x^2 + y^2 = %2.0f \n',rcuad)
    case 2
         r=input('Dar el radio: r= ');
         rcuad=r^2;
         h=input('Dar la cordenada h del centro h= ');
         k=input('Dar la cordenada k del centro k= ');
         fprintf('\nLa ecuación es: (x-%2.0f)^2+(y-%2.0f)^2 = %2.0f \n',h,k,rcuad);
    case 3
        r=input('Dar el radio: r= ');
         fprintf('\nLa ecuación es: x^2+y^2-%2.0fy = 0\n', 2*r);
end
