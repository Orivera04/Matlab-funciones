function [out1,out2,savepts] = eckert6(mstruct,in1,in2,object,direction,savepts)

%ECKERT6  Eckert VI Pseudocylindrical Projection
%
%  This is an equal area projection.  Scale is true along the 49 deg,
%  16 min parallels, and is constant along any parallel and between
%  any pair of parallels equidistant from the Equator.  It is not free
%  of distortion at any point except at 49 deg, 16 min N and S along
...
if nargin == 1                  %  Set the default structure entries
	  ...
	  mstruct.mapparallels = angledim(4916,'dms',mstruct.angleunits);
...
%  and inverse points (the hard point set) using TMAPCALC
     ...
     x = radius * long .* (1 + cos(thetanew)) / sqrt(2+pi);
...
% Inverse projection
	 ...
	 long = sqrt(2+pi)*x ./ (radius*(1+cos(theta)));
