...
%   $Revision: 1.8.4.6 $  $Date: 2004/01/16 20:04:24 $
...
% Error checking.
if ~isa(obj, 'instrument')    
    error('matlab:serial:instrfind:invalidOBJ', 'OBJ must be an instrument object.');
end

% Error if invalid.
if ~all(isvalid(obj))
   error('matlab:serial:instrfind:invalidOBJ', 'Instrument object OBJ is an invalid object.');
end
...
try
	while index <= nargin
		...
		case 'cell'
			...
			if (iscell([prop{:}]))
				errflag = 1;
				lasterr('Invalid param-value pairs specified.', 'matlab:serial:instrfind:invalidPVPair');
				return;
			end
			allProps = {allProps{:} prop{:}};
