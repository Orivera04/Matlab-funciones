/*
 * ifft_dbllen_trig_d_zbr_oop_rt.c- Signal Processing Blockset FFT run-time library components.
 *
 *  Copyright 1995-2003 The MathWorks, Inc.
 *  $Revision: 1.5.2.3 $  $Date: 2004/04/12 23:44:12 $
 */
 ...
 *
 */
EXPORT_FCN void MWDSP_Ifft_DblLen_TRIG_D_Zbr_Oop(
    creal_T       *y,
    const real_T  *x,
    const int_T    fftLen
    )
{
...

    {
        ...
        if (N2 > 1) {
             ...
             */
            theta = (real_T)DSP_TWO_PI / (real_T)fftLen;
            W_re = cos(theta);
            W_im = sin(theta);
...
/* [EOF] ifft_dbllen_trig_d_zbr_oop_rt.c */
