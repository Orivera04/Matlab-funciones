...
%   will be converted to grayscale if neccessay.
%
...
%   ---------------------------------------------------------------
%   The PPM/PGM file format specification does not allow that a binary
%   PPM/PGM image has a maximum pixel value larger than 255, but since some
%   software (notably NetPBM) supports binary PPM/PGM images with a maximum
%   pixel value of up to 65535, this is supported.  Note, however, that this
%   is non-standard, so some software might not be able to read the
...
%  $Revision: 1.1.6.1 $  $Date: 2003/12/13 03:01:41 $
   ...
   nd = ndims(data);
   if nd > 3
      error('MATLAB:writepnm:tooManyDims', ...
            '%d-D data not supported for PPM/PGM/PBM files', nd);
   end
   ...
   switch lower(suffix)
      ...
      case '.pnm', format = 'pnm';
      otherwise
         error('MATLAB:writepnm:badFileExtension', ...
               [filename ': unrecognized suffix "' suffix '"']);
   end
   ...
   if rem(length(varargin), 2)
      error('MATLAB:writepnm:mismatchedParamValuePairs', ...
   ...
   for k = 1:2:length(varargin)
      ...
      if ~ischar(param)
         error('MATLAB:writepnm:badParameterName', ...
               'Parameter name must be a string');
      end
      idx = strmatch(param, paramStrings);
      if isempty(idx)
         error('MATLAB:writepnm:unrecognizedParameter', ...
               'Unrecognized parameter name "%s"', param);
      elseif (length(idx) > 1)
         error('MATLAB:writepnm:ambiguousParameter', ...
      ...
      switch param
         ...
         case 'maxvalue'
            ...
            if any(size(maxval) ~= 1) || maxval ~= round(maxval) || maxval < 1
               error('MATLAB:writepnm:badMaxValue', ...
                     '"MaxValue" must be a positive integer.');
            end

         case 'encoding'
            encoding = varargin{k+1};
            a = strfind('ascii',lower(encoding));
            r = strfind('rawbits',encoding);
            if ~isempty(a) && (a == 1)
                encoding = 'plain';
            elseif ~isempty(r) && (r == 1)
                encoding = 'raw';
            else
                error('MATLAB:writepnm:badEncoding', ...
                      'Invalid "Encoding" property value.');
            end
   ...
   if strcmp(encoding, 'raw') && maxval > 65535
      warning('MATLAB:writepnm:usingPlainEncoding', ...
              ['MaxValue > 65535 not allowed with raw encoding. Using' ...
               ' plain encoding.']);
      encoding = 'plain';
   end
   ...
   switch format
      ...
      otherwise
         ...
         error('MATLAB:writepnm:badImageFormatString', ...
               'Internal error: Unrecognized image format string.');
   end
...
function writepbm(data, map, filename, encoding)
   ...
   if isempty(map)
      ...
      if channels > 1
         ...
         if islogical(data)
            ...
            switch class(data)
               ...
               case {'double', 'single'}, data = uint8(data >= 0.5);
            end
      ...
      otherwise
          error('MATLAB:writepnm:badColormapClass', ...
                'Index array is of invalid class.');
      end
   ...
   switch encoding
      ...
      otherwise
         % we should never get here, but just in case...
         error('MATLAB:writepnm:invalidEncoding', ...
               'Internal error: Invalid encoding string');
   end
         ...
         if islogical(data)
            ...
            switch cls
               ...
               case {'double', 'single'}
                  ...
                  data = round(data);
               otherwise
                  error('MATLAB:writepnm:invalidImageClass', ...
...
function write_raw_data(filename, format, data, height, width, maxval)
   ...
   if numel(data)/size(data, 1) ~= 1
      error('MATLAB:writepnm:dataMustBeColumnVector', ...
            'Internal error: data should be a column vector.');
   end
   ...
   if (fid < 0)
       error('MATLAB:writepnm:fileOpen', ...
   ...
   switch format
      ...
      otherwise
         % we should never get here, but just in case...
         error('MATLAB:writepnm:invalidFormat', ...
   ...
   if ~strcmp(format, 'pbm')
      ...
      elseif maxval > 65535
         % we should never get here, but just in case...
         error('MATLAB:writepnm:asciiEncodingRequired', ...
               'Internal error: maxval > 65535 requires ascii encoding.');
      end
   end
   ...
   if count < length(data)
      error('MATLAB:writepnm:writeFailed', ...
            'Failed to write all image data to file.');
   end
...
%AUTO_DETERMINE_FORMAT Automatically determine image format (PPM/PGM/PBM)
   ...
   if isempty(map)
      ...
      switch class(data)
         ...
         otherwise
            error('MATLAB:writepnm:invalidImageClass', ...
                  'Internal error: Unrecognized class.');
      end
