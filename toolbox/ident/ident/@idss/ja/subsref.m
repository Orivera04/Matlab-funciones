...
% 
% これらの表現は、MOD(1,[2 3]).inputname や MOD.cov(1:3,1:3) のようなサ
% ブスクリプト参照として正しい記法を利用できます。
%
...
% す。
% MOD('noise') は、MOD への付加的ノイズ特性(測定入力チャンネルではない)
% の時系列記述を与えます。
%     
% 測定チャンネルとノイズチャンネルに共に、アクセスするためには、最初に、
% NOISECNVを使用して、ノイズチャンネルを変換します。
