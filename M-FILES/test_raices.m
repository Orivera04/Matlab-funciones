...
tol1=input('Introducir el grado de error de las raíces: ');
absim=abs(imag(r_aprox));
f_c=find(absim<=tol1);
v_c=find(absim>tol1);
raices1=real(r_aprox(f_c));
if numel(raices1)==k
    disp('Todas las raíces son reales:');
    raices=raices1
    break
end
raices1
if (polyval(p,real_aprox)<tol1) 
    disp('Las raíces reales son exactas o aproximadas');
    raices_reales1=real_aprox
else
    disp('Roots no encontró raíces reales')
end
