...
%   $Revision: 1.6.4.3 $  $Date: 2004/01/16 20:04:19 $

% Error checking.
if ~isa(obj, 'icinterface')
    error('MATLAB:serial:fwrite:invalidOBJ', 'OBJ must be an interface object.');
end

if length(obj)>1
    error('MATLAB:serial:fwrite:invalidOBJ', 'OBJ must be a 1-by-1 interface object.');
end

% Parse the input.
switch nargin
case 1
   error('MATLAB:serial:fwrite:invalidSyntax', 'The input argument A must be specified.');
case 2
   cmd = varargin{1};
   precision = 'uchar';
   mode = 0;	
case 3
   ...
   if ~(isa(precision, 'char') || isa(precision, 'double'))
	   error('MATLAB:serial:fwrite:invalidArg','The third input argument must be a string.');
   end
...
case 4
   ...
   if ~ischar(mode)
	   error('MATLAB:serial:fwrite:invalidMODE','MODE must be either ''sync'' or ''async''.');
   end
...
otherwise
   error('MATLAB:serial:fwrite:invalidSyntax','Too many input arguments.');
end   

% Error checking.
if ~isa(precision, 'char')
	error('MATLAB:serial:fwrite:invalidPRECISION','PRECISION must be a string.');
end
if ~(isnumeric(cmd) || ischar(cmd))
	error('MATLAB:serial:fwrite:invalidA','The input argument A must be numeric or a string.');
end
...
case {'double' ,'float64'}
    ...
    signed = 1;
otherwise
    error('MATLAB:serial:fwrite:invalidPRECISION','Invalid PRECISION specified. Type ''instrhelp fwrite'' for more information.');
end
...
try
   ...
   error('MATLAB:serial:fwrite:opfailed', lasterr);
end   
