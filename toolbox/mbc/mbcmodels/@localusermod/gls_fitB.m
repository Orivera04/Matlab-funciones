function [B,yhat,res,J,OKall]= gls_fitB(L,B,DATA,Wc)
%LOCALUSERMOD/GLS_FITB least-squares estimation of localpspline
%
% [B,res,J,yhat]= gls_fitB(L,B,DATA,Wc)
%   L    localusermod object
%   B     initial parameter matrix (cols= sweeps)
%   DATA  sweepset of data to fit [X,Y]
%   Wc    optional weights Wc'*Wc= inv(covmatrix)

%  Copyright 2000-2004 The MathWorks, Inc. and Ford Global Technologies, Inc.
% $Revision: 1.4.6.2 $  $Date: 2004/02/09 07:43:49 $

Ns= size(DATA,3);

Y= DATA(:,end);
if isa(DATA,'sweepset')
    CDATA= cell(DATA);
elseif ~isa(DATA,'cell') 
   CDATA= {DATA};
else
   CDATA= DATA;
end
if nargin < 4 | isempty(Wc)
   Wc=cell(Ns,1);
end


OKall= zeros(1,Ns);

res= cell(Ns,1);
J= res;
yhat= J;
U= L.userdefined;
if isTBS(L)
    yt= get(L,'ytrans');
    U= set(U,'tbs',0);
    U= set(U,'ytrans',yt);
    Y(:,1)= yinv(U,double(Y));
    U= set(U,'tbs',1);
end
for i=1:Ns
   d= CDATA{i};
	% code X data
   Xs= d(:,1:end-1);
   Ys= Y{i};
   U= update(U,B(:,i));
	% fit the linear model
   [U,OK]= fitmodel(U,Xs,Ys,Wc{i});
   
   p=  double(U);
   B(:,i)= p(:);
   if nargout>1
      % this makes sure that the polynomial is the right size
      % and sets the datum
		
      [res{i},J{i},yhat{i}]= lsqcost(U,Xs,Ys,Wc{i});
   end
   OKall(i)= OK;
end


L.userdefined= U;


% not sure if I want to do this in here or whether this is best post GLS 

if isa(DATA,'sweepset')
   res= cell2sweeps(DATA(:,1),res);
else
   res= cat(1,res{:});
end

