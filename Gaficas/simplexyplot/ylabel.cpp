...
static mxChar _array5_[6] = { 'y', 'l', 'a', 'b', 'e', 'l' };
...
void InitializeModule_ylabel() {
}

void TerminateModule_ylabel() {
}

static mwArray Mylabel(int nargout_, mwArray string, mwArray varargin);

_mexLocalFunctionTable _local_function_table_ylabel
  = { 0, (mexFunctionTableEntry *)NULL };

//
// The function "Nylabel" contains the nargout interface for the "ylabel"
...
// appearing above.
//
mwArray Nylabel(int nargout, mwArray string, mwVarargin varargin) {
    mwArray hh = mwArray::UNDEFINED;
    hh = Mylabel(nargout, string, varargin.ToArray());
    return hh;
}

//
// The function "ylabel" contains the normal interface for the "ylabel"
// M-function from file "c:\matlab6p5\toolbox\matlab\graph2d\ylabel.m" (lines
// 1-41). This function processes any input arguments and passes them to the
// implementation version of the function, appearing above.
//
mwArray ylabel(mwArray string, mwVarargin varargin) {
    int nargout = 1;
    mwArray hh = mwArray::UNDEFINED;
    hh = Mylabel(nargout, string, varargin.ToArray());
    return hh;
...
//
// The function "Vylabel" contains the void interface for the "ylabel"
// M-function from file "c:\matlab6p5\toolbox\matlab\graph2d\ylabel.m" (lines
...
//
void Vylabel(mwArray string, mwVarargin varargin) {
    mwArray hh = mwArray::UNDEFINED;
    hh = Mylabel(0, string, varargin.ToArray());
}

//
// The function "mlxYlabel" contains the feval interface for the "ylabel"
// M-function from file "c:\matlab6p5\toolbox\matlab\graph2d\ylabel.m" (lines
// 1-41). The feval function calls the implementation version of ylabel through
    ...
    MW_BEGIN_MLX();
    {
        ...
        if (nlhs > 1) {
            error(
              mwVarargin(
                mwArray(
                  "Run-time Error: File: ylabel Line: 1 Column: 1"
                  " The function \"ylabel\" was called with more "
...
static mwArray Mylabel(int nargout_, mwArray string, mwArray varargin) {
    ...
    mwValidateOutput(hh, 1, nargout_, "hh", "ylabel");
    return hh;
}
