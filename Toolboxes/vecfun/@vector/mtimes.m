...
%*  Multiply.
%   V*W is the elementwise product of V and W,
%   where V and W are a scalar and a vector function.
%   If both V and W are vector functions, then V*W
%   will be the same as cross(V,W).

% Copyright (c) 2001-04-17, B. Rasmus Anthin.

As=isscalar(A);Bs=isscalar(B);
Av=isvector(A);Bv=isvector(B);
...
[x y z]=vars(A);
if (Av | Al==3) & (Bv | Bl==3)
   C=cross(A,B,inputname(1),inputname(2));
else
   ...
   findAx=any(Anamex=='+') | any(Anamex=='-');
   findAy=any(Anamey=='+') | any(Anamey=='-');
   findAz=any(Anamez=='+') | any(Anamez=='-');
   findBx=any(Bnamex=='+') | any(Bnamex=='-');
   findBy=any(Bnamey=='+') | any(Bnamey=='-');
   findBz=any(Bnamez=='+') | any(Bnamez=='-');
   if findAx & findBx
      C.fx=['(' Anamex ')*(' Bnamex ')'];
