function text = display(sys,conf)
%DISPLAY   Pretty-print for IDSS models.
%
%   DISPLAY(SYS) is invoked by typing SYS followed
%   by a carriage return.    

%   Copyright 1986-2003 The MathWorks, Inc.
%   $Revision: 1.10.4.1 $ $Date: 2004/04/10 23:17:55 $

% Extract state-space data and sampling/delay times

if isempty(sys)
    text = 'Empty IDSS model.';
    if nargout == 0
        disp(text)
    end
...
if nargout
   ...
   if isempty(nv) | norm(nv) == 0
    ...
    if isempty(sys.As)
        disp(['Static State-space model:  y(t) = D u(t)',noisetxt2])
elseif nu>0
...
function txt = printsys(a,b,c,d,k,x0,da,db,dc,dd,dk,dx0,...
