...
%    $Revision: 1.11.2.4 $  $Date: 2003/08/29 04:40:01 $
...
% Extract the CreationTime and channel handles from the store variable, info.
time = info{1}{3};
chan_handle = info{2};

% Need to recreate the parent object so that the Parent field of the Channel
% object being loaded is correct.  All the channel objects need to be
% created initially and then the handles of the object that is being loaded
% will be compared to the parent object's channel objects and the correct
% channels will be extracted.

% Obtain a cell array of AnalogInput, AnalogOutput and DigitalIO objects.
basevar = daqfind;
for i = 1:length(basevar)
   tempobj = get(basevar, i);
   % If the object is an AnalogOutput compare the creation time.
   if isa(tempobj, 'analogoutput')
...
% Return those channels that were saved only.
obj = [];
for i = 1:length(B.handle)
   index = find(B.handle(i) == chan_handle{:});
   obj = [obj; parent.Channel(index)];
end

% If the sizes are not equal then transpose the channel array.
