...
%   $Revision: 1.1.6.2 $  $Date: 2004/02/01 22:04:43 $
...
switch info.CompressionType
 ...
 case 'bitfields'
  error('MATLAB:readbmpdata:bitfieldsCompression', ...
        'Bitfield compression not supported');
  
 case 'Huffman 1D'
  error('MATLAB:readbmpdata:huffman1dCompression', ...
        'Huffman 1D compression not supported');
  
 case '24-bit RLE'
  error('MATLAB:readbmpdata:rle24BitCompression', ...
...
if status==-1
  fclose(fid);
  error('MATLAB:readbmpdata:dataOffset', ...
...
if (count ~= paddedWidth*abs(height))
    warning('MATLAB:readbmpdata:truncatedImageData', ...
            'Invalid BMP file: truncated image data');
