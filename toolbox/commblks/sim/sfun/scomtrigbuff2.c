/*
   ...
   * insufficent data in the internal buffer, the block will generate a
   * function call event on port 0 to trigger generation of the data from an
   * 'upstream' block.
   *
   * Be aware:
   *
   ...
   * INPUT_WIDTH_MODE = 3 -> Input width is based on OUTPUT_WIDTH. A minimum
   *                         and step size are specified. If the input width is
   *                         not a multiple of the step size (when started from
   *                         the minimum), the next highest increment is
   *                         chosen.
   *
   ...
   * a deterministic startup transient.  The block pre-triggers by specifying
   * the input sample offset before output samples become available.
   *
   *  Copyright 1996-2003 The MathWorks, Inc.
   *  $Revision: 1.5.4.3 $  $Date: 2004/04/12 23:03:40 $
   *
 */


#define S_FUNCTION_NAME scomtrigbuff2
...
/* --- The following defines how the block works in an enabled subsystem
   ...
   *
   * NOTE: When included in a trigered sub-system, this pre-triggering only
   *       occurs on the initial trigger.
 */
#define RESET_POINTERS_ON_ENABLE true

/* --- Define i/o signals */
enum {
    INPORT_SIGNAL = 0, NUM_INPORTS
};
enum {
    OUTPORT_FCNCALL = 0, OUTPORT_SIGNAL, NUM_OUTPORTS
};

/* --- Define mask parameters */
enum {
    SAMPLE_START_A = 0, CHANNELS_A, INPUT_WIDTH_MODE_A, INPUT_WIDTH_A, INPUT_WIDTH_MIN_A, INPUT_WIDTH_STEP_A, NUM_ARGS
};
...
enum {
    DATA_AVAIL = 0, IDX_BOTTOM, IDX_TOP, INT_BUFF_LEN, PRE_TRIGGER_COMPLETE, NUM_IWORK
};

/* --- Define pointer work vectors */
enum {
    INTBUF_PTR = 0, NUM_PWORK
};


/* --- Define the modes of input width selection */
enum {
    INPUT_WIDTH_INHERITED = 1, INPUT_WIDTH_SPECIFIED, INPUT_WIDTH_BASED_ON_OUTPUT, NUM_MODES
};
...
static void mdlInitializeSizes(SimStruct * S)
{
	...
	ssSetInputPortFrameData(S, INPORT_SIGNAL, FRAME_YES);
	ssSetInputPortMatrixDimensions(S, INPORT_SIGNAL, DYNAMICALLY_SIZED, DYNAMICALLY_SIZED);
    }
    ...
    ssSetOutputPortFrameData(S, OUTPORT_SIGNAL, FRAME_YES);
    ssSetOutputPortMatrixDimensions(S, OUTPORT_SIGNAL, DYNAMICALLY_SIZED, DYNAMICALLY_SIZED);
...
/* adding an mdlStart to initialize the pWork */
static void mdlStart(SimStruct *S)
{
    ...
    if (!cplx)
	ssSetPWorkValue(S, INTBUF_PTR, (real_T *) calloc(intBuffLen * numChan, sizeof(real_T)));
    else
	ssSetPWorkValue(S, INTBUF_PTR, (creal_T *) calloc(intBuffLen * numChan, sizeof(creal_T)));
...
{
    free(ssGetPWorkValue(S, INTBUF_PTR));
}
					  ...
					  const DimsInfo_T * dimsInfo)
{
    ...
    if (dimsInfo->width % numChan != 0) {
	THROW_ERROR(S, "Output port width must be a multiple of the number of channels.");
    }
    ssSetOutputPortMatrixDimensions(S, port, dimsInfo->dims[0], dimsInfo->dims[1]);
	...
	if (ssGetInputPortWidth(S, INPORT_SIGNAL) == DYNAMICALLY_SIZED) {
	    ...
	    ssSetInputPortMatrixDimensions(S, INPORT_SIGNAL, inWidth, numChan);
	} else {
					 ...
					 const DimsInfo_T * dimsInfo)
{
	...
	if (dimsInfo->dims[0] % numChan != 0) {
	    THROW_ERROR(S, "Input port width must be a multiple of the number of channels.");
	}
	ssSetInputPortMatrixDimensions(S, port, dimsInfo->dims[0], dimsInfo->dims[1]);
    } else {
