/* $Revision: 1.4 $ $Date: 2002/03/25 04:01:36 $ */
/* canac2104send_mb1.c - xPC Target, non-inlined S-function driver for CAN-AC2-104 from Softing (multi board)  */
...
#define 	S_FUNCTION_NAME 	canac2104send_mb1
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
	...
	if (((int_T)mxGetPr(CAN_PORT_ARG)[0])==1) {		// CAN 1
    	for (i=0;i<mxGetN(CAN_INDEX_ARG);i++) {
    		...
    		frc = CANPC_write_object_mb1(index, 8, p_data);
			if (frc) {
    	...
    	for (i=0;i<mxGetN(CAN_INDEX_ARG);i++) {
    		...
    		frc = CANPC_write_object2_mb1(index, 8, p_data);
			if (frc) {
