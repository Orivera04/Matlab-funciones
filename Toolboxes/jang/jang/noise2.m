...
%load noise.mat			% load n(k) from file

% ====== load data from sound files
load handel;
handel = y;
freq = Fs;
t = (1:sample_n)'/Fs;
...
%save noise2 out_fismat error stepsize
%load noise2.mat
...
title('(a) Information Signal');
ylabel('x(k)'); xlabel('k');
axis(axis_2D);
tmp = get(gca, 'pos');
uicontrol('style', 'push', 'string', 'play', 'unit', 'norm', ...
	'pos', [tmp(1:2) 0.1071    0.0476], ...
	'callback', 'sound(x)');

subplot(2,2,2); plot(t*freq, n);
title('(b) Noise Source Signal');
ylabel('n(k)'); xlabel('k');
axis(axis_2D);
tmp = get(gca, 'pos');
uicontrol('style', 'push', 'string', 'play', 'unit', 'norm', ...
...
title('(c) Distorted Noise Signal');
ylabel('d(k)'); xlabel('k');
axis(axis_2D);
tmp = get(gca, 'pos');
uicontrol('style', 'push', 'string', 'play', 'unit', 'norm', ...
...
% ====== plot spectra 
point_n = 1000;	% for Fourier transform
%x = x(2000:2500);
%y = y(2000:2500);
%n = n(2000:2500);
%d = d(2000:2500);
...
blackbg;
subplot(2,2,1);
plot(f, Pxx(1:point_n/2));
axis(spectrum_axis);
title('(a) Power Spectral Density of x(k)');
xlabel('Frequency (Hz)');
subplot(2,2,2);
plot(f, Pyy(1:point_n/2));
axis(spectrum_axis);
title('(b) Power Spectral Density of y(k)');
xlabel('Frequency (Hz)');
subplot(2,2,3);
plot(f, Pnn(1:point_n/2));
axis(spectrum_axis);
title('(c) Power Spectral Density of n(k)');
xlabel('Frequency (Hz)');
subplot(2,2,4);
plot(f, Pdd(1:point_n/2));
axis(spectrum_axis);
title('(d) Power Spectral Density of d(k)');
xlabel('Frequency (Hz)');
%print -deps spectra2
...
title('(a) Estimated Distorted Noise');
ylabel('d_hat(k)'); xlabel('k');
axis(axis_2D);
tmp = get(gca, 'pos');
uicontrol('style', 'push', 'string', 'play', 'unit', 'norm', ...
	'pos', [tmp(1:2) 0.1071    0.0476], ...
	'callback', 'sound(d_hat)');

subplot(2,2,2); plot(t*freq, x_hat)
title('(b) Estimated Info Signal')
ylabel('x_hat(k)'); xlabel('k');
axis(axis_2D);
tmp = get(gca, 'pos');
uicontrol('style', 'push', 'string', 'play', 'unit', 'norm', ...
	'pos', [tmp(1:2) 0.1071    0.0476], ...
	'callback', 'sound(x_hat)');

subplot(2,2,3); plot(t*freq, diff);
title('(c) Estimation Error');
ylabel('x(k)-x_hat(k)'); xlabel('k');
axis(axis_2D);
tmp = get(gca, 'pos');
uicontrol('style', 'push', 'string', 'play', 'unit', 'norm', ...
	'pos', [tmp(1:2) 0.1071    0.0476], ...
...
title('(d) Original Information Signal');
ylabel('x(k)'); xlabel('k');
axis(axis_2D);
tmp = get(gca, 'pos');
uicontrol('style', 'push', 'string', 'play', 'unit', 'norm', ...
