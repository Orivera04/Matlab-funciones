...
% 照できます(たとえば、SYS.Ts)。
%
% 単一のSISOモデル SYS に対して、書式 [Z,P,K] = ZPKDATA(SYS,'v') は、零点
% Z と極 P がセル配列ではなく列ベクトルとして出力します。
%
% LTIモデルの配列 SYS に対して、Z, P, K は SYS と同じサイズの配列になり、m番
% 目のモデル SYS(:,:,m) の ZPK 表現は、Z(:,:,m), P(:,:,m), K(:,:,m)です。
%
