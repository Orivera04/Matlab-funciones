function output = imaqfind(varargin)
...
%    $Revision: 1.1.6.4 $  $Date: 2004/03/30 13:05:56 $

% Initialize.
output = [];

% Locate our package.
imaqpck = findpackage('imaq');
if isempty(imaqpck)
    return;
end

% Find all the objects. 
imaqdb = imaqpck.DefaultDatabase;
out = find(imaqdb);
if isempty(out)
    % No objects exist
	return;
...
try
    if (nargin>0),
        output = imaqfind(output, varargin{:});
    end
catch
    privateFixError;
    rethrow(lasterror);
