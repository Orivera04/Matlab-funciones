/*
 * ifft_dbllen_trig_rbr_cbr_oop_rt.c- Signal Processing Blockset FFT run-time library components.
 *
 *  Copyright 1995-2003 The MathWorks, Inc.
 *  $Revision: 1.4.2.3 $  $Date: 2004/04/12 23:44:15 $
 */
    ...
    )
{
    ...
    {
        real32_T  W_re, W_im, theta, xsum, xdiff;
        if (N2 > 1) {
            /*
             * N2 >= 2
             ...
             */
            theta = (real32_T)DSP_TWO_PI * (1.0F/fftLen);
            W_re  = cosf(theta);
            W_im  = sinf(theta);
...

        {
            ...
            for (i=2; i<N4; i++) {
                ...
                theta = (real32_T)DSP_TWO_PI * ((real32_T)i/fftLen);
                W_re  = cosf(theta);
                W_im  = sinf(theta);
...
/* [EOF] ifft_dbllen_trig_rbr_cbr_oop_rt.c */
