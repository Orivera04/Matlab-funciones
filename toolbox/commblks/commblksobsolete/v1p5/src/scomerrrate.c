/*
 * SCOMERRRATE Communications Toolbox S-Function for error rate calculation.
 *
 *  Copyright 1996-2002 The MathWorks, Inc.
 *  $Revision: 1.1.6.1 $  $Date: 2003/01/26 18:45:13 $
 */
#define S_FUNCTION_NAME scomerrrate
#define S_FUNCTION_LEVEL 2
#include "simstruc.h"
#include "comm_defs.h"

enum {RX_DELAY_C=0, ST_DELAY_C, CP_MODE_C, SUB_FRAME_C, OUTPUT_C, VAR_NAME_C, RESET_BOX_C, NUM_PARAMS};

enum {IN_PORT1=0, IN_PORT2, OPT_PORT1, OPT_PORT2};
enum {OUT_PORT=0};

enum {RATIO=0, ERRORS, SYMBOLS, ST_CNT, TX_BUFF, FR_BUFF, USE_FRAME, WIDTH, TX_SCALAR, RX_SCALAR, NUM_DWORK};
...

{
    ...
    if (mxGetPr(CP_MODE)[0]==3) {
        if (mxGetPr(RESET_BOX)[0]==1) {
            ...
            ssSetInputPortWidth(            S, OPT_PORT2, 1);
        ...
        ssSetInputPortWidth(            S, OPT_PORT1, DYNAMICALLY_SIZED);
        ...
        ssSetInputPortWidth(            S, OPT_PORT1, 1);
        ssSetInputPortDirectFeedThrough(S, OPT_PORT1, 1);
        ssSetInputPortComplexSignal(    S, OPT_PORT1, COMPLEX_NO);
        ssSetInputPortSampleTime(       S,OPT_PORT1, INHERITED_SAMPLE_TIME);
    }else{
     ...
     * complex-inherited.
     */
    ssSetInputPortWidth(            S, IN_PORT1, DYNAMICALLY_SIZED);
    ...
    ssSetInputPortWidth(            S, IN_PORT2, DYNAMICALLY_SIZED);
    ...
    ssSetSFcnParamNotTunable(S, RX_DELAY_C);
    ssSetSFcnParamNotTunable(S, ST_DELAY_C);
    ssSetSFcnParamNotTunable(S, CP_MODE_C);
    ssSetSFcnParamNotTunable(S, SUB_FRAME_C);
    ssSetSFcnParamNotTunable(S, OUTPUT_C);
    ssSetSFcnParamNotTunable(S, VAR_NAME_C);
    ssSetSFcnParamNotTunable(S, RESET_BOX_C);
}
...
static void mdlInitializeConditions(SimStruct *S)
{
    ...
    int32_T   *errors    = (int32_T *)   ssGetDWork(S, ERRORS);
    int32_T   *symbols   = (int32_T *)   ssGetDWork(S, SYMBOLS);
...
{
    /* Inherit input port width. */
    if (port == IN_PORT1) {
        int_T Port2Width = ssGetInputPortWidth(S, IN_PORT2);

        if ( (Port2Width > 1) && (inputPortWidth > 1) &&
             (Port2Width != inputPortWidth) ) {
            THROW_ERROR(S, "Input Port Widths unequal.");
        }

        ssSetInputPortWidth(S,  port, inputPortWidth);
    } else if (port == IN_PORT2) {
        int_T Port1Width = ssGetInputPortWidth(S, IN_PORT1);

        if ( (Port1Width > 1) && (inputPortWidth > 1) &&
             (Port1Width != inputPortWidth) ) {
            THROW_ERROR(S, "Input Port Widths unequal.");
        }

        ssSetInputPortWidth(S, port, inputPortWidth);
    } else if (port == OPT_PORT1) {
        ssSetInputPortWidth(S, port, inputPortWidth);
    } else {
        THROW_ERROR(S, "Input port width propagation failure in error "
                  "rate block.");
    }
}
                                   ...
                                   int_T outputPortWidth)
{
     ...
     * Only needed because MATLAB-MEX requires SetInput/OutputPortWidth pair.
     */
}
...
static void mdlSetWorkWidths(SimStruct *S)
{
    ...
    ssSetDWorkDataType(     S, ERRORS, SS_INT32);
    ssSetDWorkComplexSignal(S, ERRORS, COMPLEX_NO);

    ssSetDWorkWidth(        S, SYMBOLS, 1);
    ssSetDWorkDataType(     S, SYMBOLS, SS_INT32);
