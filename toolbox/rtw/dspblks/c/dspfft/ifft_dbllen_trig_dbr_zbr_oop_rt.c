/*
 * ifft_dbllen_trig_dbr_zbr_oop_rt.c- Signal Processing Blockset FFT run-time library components.
 *
 *  Copyright 1995-2003 The MathWorks, Inc.
 *  $Revision: 1.4.2.3 $  $Date: 2004/04/12 23:44:13 $
 */
 ...
 *
 */
EXPORT_FCN void MWDSP_Ifft_DblLen_TRIG_Dbr_Zbr_Oop(
    creal_T       *y,
    const real_T  *x,
    const int_T    fftLen
    )
{
    ...
    {
        real_T  W_re, W_im, theta, xsum, xdiff;
        if (N2 > 1) {
            /*
             * N2 >= 2
             ...
             */
            theta = (real_T)DSP_TWO_PI * (1.0/fftLen);
            W_re = cos(theta);
            W_im = sin(theta);

            xsum  = x[N2] + x[fftLen - 2];   /* X(n) + X(N2-n) */
            xdiff = x[N2] - x[fftLen - 2];   /* X(n) - X(N2-n) */
...

        {
            ...
            for (i=2; i<N4; i++) {
                ...
                theta = (real_T)DSP_TWO_PI * ((real_T)i/fftLen);
                W_re = cos(theta);
                W_im = sin(theta);
                ...
                xsum  = x[jbr_f_in] + x[jbr_b_in];   /* X(n) + conj(X(N2-n)) */
                xdiff = x[jbr_f_in] - x[jbr_b_in];   /* X(n) - conj(X(N2-n)) */
...
/* [EOF] ifft_dbllen_trig_dbr_zbr_oop_rt.c */
