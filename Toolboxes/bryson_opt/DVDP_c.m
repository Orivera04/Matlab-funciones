function [c,ceq]=dvdp_c(p,N,h)
% Subroutine for Pb. 9.3.09; DVDP for min time to x=xf w. y<=h+x*tan(th);
...
for i=1:N, si=sin(u(i)); co=cos(u(i)); dl=dt*v(i)+dt^2*si/2;
   ...
   c(i+1)=y(i)-x(i)*tan(th)-h;                  % Inequality constraints 
end
%
ceq=x(N+1)-1;                                      % Equality constraint
