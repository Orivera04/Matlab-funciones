 ...
 * $Revision: 1.1.6.1 $  $Date: 2003/12/13 03:01:32 $
 */
                           ...
                           const int *size)
{
    switch (imageType) {
    case RGB_IMG:
        WriteRGBFromUint8(cinfoPtr, (uint8_T *) mxGetData(inputArray), size);
        break;

    case GRAY_IMG:
        WriteGRAYFromUint8(cinfoPtr, (uint8_T *) mxGetData(inputArray), size);
        break;

    case PACKED_RGB_IMG:
        WriteRGBFromUint32(cinfoPtr, (uint32_T *) mxGetData(inputArray), size);
        break;
