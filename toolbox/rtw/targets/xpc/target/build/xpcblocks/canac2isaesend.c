/* $Revision: 1.4 $ $Date: 2002/03/25 04:02:03 $*/
...
#define 	S_FUNCTION_NAME 	canac2isaesend
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
	...
	if (((int_T)mxGetPr(CAN_PORT_ARG)[0])==1) {		// CAN 1
    	for (i=0;i<mxGetN(CAN_INDEX_ARG);i++) {
			...
			frc = CANPC_write_object(index, (int_T)mxGetPr(CAN_SIZE_ARG)[i], p_data);		
			if ((int_T)mxGetPr(OUTPUT_ARG)[0]) {
    	...
    	for (i=0;i<mxGetN(CAN_INDEX_ARG);i++) {
			...
			frc = CANPC_write_object2(index, (int_T)mxGetPr(CAN_SIZE_ARG)[i], p_data);		
		   	if ((int_T)mxGetPr(OUTPUT_ARG)[0]) {
				y=ssGetOutputPortSignal(S,i);
				y[0]=(double)frc;
			}
