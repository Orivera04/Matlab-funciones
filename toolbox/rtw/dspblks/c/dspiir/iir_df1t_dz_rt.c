/*
 *  IIR_DF1T_DZ_RT.C - DSP filter runtime helper function
 *
 *  Copyright 1995-2003 The MathWorks, Inc.
 *  $Revision: 1.4.2.2 $  $Date: 2004/04/12 23:45:48 $
 */
#include "dsp_rt.h"

EXPORT_FCN void MWDSP_IIR_DF1T_DZ( const real_T          *u,
                         creal_T              *y,
                         ...
                         const boolean_T      one_fpf)
{
    ...
    for (k=0; k < numChans; k++) {
        ...
        while (i--) {
            ...
            sum.re = in + (filt_mem_den->re);  /* temporaray variable to hold the sum (x + v0) */
            sum.im = filt_mem_den->im;         /* temporaray variable to hold the sum (x + v0) */
