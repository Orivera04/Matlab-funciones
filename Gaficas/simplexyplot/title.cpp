...
static mxChar _array3_[14] = { 'M', 'W', 'B', 'Y', 'P', 'A', 'S',
                               'S', '_', 't', 'i', 't', 'l', 'e' };
static mwArray _mxarray2_ = mclInitializeString(14, _array3_);
...
static mwArray _mxarray14_ = mclInitializeString(8, _array15_);
static mwArray _mxarray16_ = mclInitializeDouble(0.0);

static mxChar _array18_[6] = { 's', 't', 'r', 'i', 'n', 'g' };
static mwArray _mxarray17_ = mclInitializeString(6, _array18_);

void InitializeModule_title() {
}

void TerminateModule_title() {
}

static mwArray Mtitle(int nargout_, mwArray string, mwArray varargin);

_mexLocalFunctionTable _local_function_table_title
  = { 0, (mexFunctionTableEntry *)NULL };

//
// The function "Ntitle" contains the nargout interface for the "title"
...
// appearing above.
//
mwArray Ntitle(int nargout, mwArray string, mwVarargin varargin) {
    mwArray hh = mwArray::UNDEFINED;
    hh = Mtitle(nargout, string, varargin.ToArray());
    return hh;
}

//
// The function "title" contains the normal interface for the "title"
// M-function from file "c:\matlab6p5\toolbox\matlab\graph2d\title.m" (lines
// 1-42). This function processes any input arguments and passes them to the
// implementation version of the function, appearing above.
//
mwArray title(mwArray string, mwVarargin varargin) {
    int nargout = 1;
    mwArray hh = mwArray::UNDEFINED;
    hh = Mtitle(nargout, string, varargin.ToArray());
    return hh;
...
//
// The function "Vtitle" contains the void interface for the "title" M-function
// from file "c:\matlab6p5\toolbox\matlab\graph2d\title.m" (lines 1-42). The
...
//
void Vtitle(mwArray string, mwVarargin varargin) {
    mwArray hh = mwArray::UNDEFINED;
    hh = Mtitle(0, string, varargin.ToArray());
}
    ...
    MW_BEGIN_MLX();
    {
        ...
        if (nlhs > 1) {
            error(
              mwVarargin(
                mwArray(
                  "Run-time Error: File: title Line: 1 Column: 1"
                  " The function \"title\" was called with more "
...
static mwArray Mtitle(int nargout_, mwArray string, mwArray varargin) {
    ...
    mwValidateOutput(hh, 1, nargout_, "hh", "title");
    return hh;
}
