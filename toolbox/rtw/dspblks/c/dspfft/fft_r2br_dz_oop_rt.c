/*
 * fft_r2br_dz_oop_rt.c - Signal Processing Blockset FFT run-time library components.
 *
 *  Copyright 1995-2003 The MathWorks, Inc.
 *  $Revision: 1.4.2.3 $  $Date: 2004/04/12 23:43:34 $
 */
 ...
 */
EXPORT_FCN void MWDSP_R2BR_DZ_OOP(
    creal_T       *y,
    const  real_T *x,
    int_T          nChans,
    const int_T    nRows,
    ...
    )
{
    while(nChans-- > 0) {
        ...
        for (i=0; i<fftLen-1; i++) {
            ...
            y[j].im = 0.0;  /* Set imaginary part to zero         */
            {
                int_T bit = fftLen;
