...
%    $Revision: 1.1.6.1 $  $Date: 2004/03/24 20:42:51 $
...
if nargin<2
    ...
    siblings = get(parent, 'Group');
    newObj = [];
    for k=1:length(uddValid)
        % We need to replace this one with the copy
        newFound = opcfind('Type', 'dagroup', ...
            'Parent', parent, ...
            'Name', uddValid(k).Name);
        if isempty(newFound),
            rethrow(mkerrstruct('opc:copyobj:objnotcopied', 'Parent object did not copy me!'));
        end
    ...
    for k=1:length(uddValid)
        ...
        try
            thisObj = addgroup(parent, thisName);
            thisObj = setpropsfromudd(thisObj, uddThis);
            % Finally, copy the children into this object.
            if ~isempty(uddThis.Item),
                copyobj(uddThis.Item, thisObj);
            end
            if isempty(newObj)
                newObj = thisObj;
            else
            ...
            errMsg{end+1} = sprintf('Group ''%s'' returned: ''%s''', thisName, lasterr);
        end
    end
...
for k=1:length(props)
    ...
    if ~strcmpi(pi.ReadOnly, 'always') && ~strcmp(thisProp, 'Name')
