function [info,msg,tags] = tifinfo(filename)
%TIFINFO Information about a TIFF file.
%   [INFO,MSG,TAGS] = TIFINFO(FILENAME) returns a structure containing
...
%   information about one image in the TIFF file. INFO is empty
%   if an error occurs.
%
%   MSG is a string describing any error condition, such as an error 
%   opening the file; otherwise MSG is empty. 
%
%   TAGS is a 1-by-N array containing the Tiff ID's found in the file.
%
%   See also IMREAD, IMWRITE, IMFINFO.

%   Copyright 1996-2003 The MathWorks, Inc. 
%   $Revision: 1.1.8.2 $  $Date: 2003/08/01 18:23:49 $
...
% All argument checks must be be made prior to calling tifinfo
...
if (~isequal(sig, [73 73 42 0]) && ...
    ...
    info = [];
    %msg = 'Not a TIFF file';
    msg = sprintf('The file ''%s'' is not a TIFF file.', filename);
    fclose(fid);
    return;
end
...
if (numImages == 0)
    ...
    msg = sprintf('The file ''%s'' does not contain images.', filename);
    fclose(fid);
    return;
end
