...
%SPECGRAMDEMO スペクトログラム デモ
%   SPECGRAMDEMO(y,Fs) は、Fs Hzのサンプリングレートを仮定して、信号y 
%　 のスペクトログラムを表示します。y が指定され、Fs が指定されていない
%　 場合、 1 Hz のサンプリングレートが仮定されています。
%   入力引数がない場合、 y および Fs には、デフォルトのデータファイル
%   "mtlb.mat" の値が使用されます。
%
%   GUIの使用で、状況に応じたメニューとヘルプの表示が可能になります。
%　 スペクトログラム、カラーバーなどを含むさまざまなGUIのアイテムを
%　 右クリックすることにより、可視化オプションを探してください。
%
%　 たとえば、panner は、強調表示されたズーム領域の左端と右端を
%　 マウスで、ドラッグすることにより拡大されます。
%　 強調表示されたズームエリアを右クリックすると、ズーム領域上に 焦点を
%　 当てるために、メニューを表示し、さらに、 状況に応じたヘルプへのリンク
%   します。
%   
% 参考：SPECGRAM, SPTOOL, FDATOOL.
...
maxlen=0;
for i=1:lenx,
   maxlen=max(maxlen,length(x{i}));
end
y=ones(lenx,maxlen) .* ' ';
if isRight,
   for i=1:lenx,
      j=length(x{i});
      y(i,end-j+1:end)=x{i}; % right-align
   end
else
   for i=1:lenx,
      y(i,1:length(x{i}))=x{i};  % left-align
   end
end
y=char(y);

%---------------------------------------------------------------
function [cs,eu] = convert_vect_to_eng_str(yy,timeStr)
...
hasDecimal=0;
for i=1:length(yy),
   c{i}=sprintf('%g',yy(i).*ee);
   if any(c{i}=='.'), hasDecimal=1; end
end
...
if hasDecimal,
   ...
   for i=1:length(yy),
      j=find(c{i}=='.');
      if ~isempty(j),
         newPlaces = length(c{i})-j;
         maxPlaces = max(maxPlaces,newPlaces);
      end
   end
   maxPlaces = min(3,maxPlaces);
   % Print using maxPlaces decimal places:
   for i=1:length(yy),
      c{i}=sprintf(['%.' num2str(maxPlaces) 'f'],yy(i).*ee);
   end
end
cs = cell2mat(c);
...
[cs,eu] = convert_vect_to_eng_str(yy);
...
[cs,eu] = convert_vect_to_eng_str(yy,'time');
...
%   [Y,E,U]=ENGUNITS(X) converts scalar input value X into
%   a new value Y, with associated scale factor E, such that
%   Y = X * E.  The engineering unit prefix is returned as a
%   character in U.
%
...
if isTime,
   ...
   varargin(end)=[];
   [varargout{1:nargout}] = getTimeUnits(varargin{:});
else
   % General engineering units conversion:
   [varargout{1:nargout}] = getEngPrefix(varargin{:});
end
return
...
if isempty(eng_units),
   eng_units       = {'a','f','p',  'u','m','','k','M','G','T','P','E'};
   eng_units_latex = {'a','f','p','\mu','m','','k','M','G','T','P','E'};
...
if x==0,
   ...
   norm_exp = ceil(log10(abs(x))); % normalized exponent
end
