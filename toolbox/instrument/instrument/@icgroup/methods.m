...
%   $Revision: 1.1.6.3 $  $Date: 2004/01/16 20:00:12 $

% Error checking.
if (nargout > 1)
    error('icgroup:methods:maxlhs', 'Too many output arguments.');
end

if (nargin > 2)
    error('icgroup:methods:invalidSyntax', 'Invalid syntax. Type ''help methods'' for more information.');
end

if ~isvalid(obj)
    error('icgroup:methods:invalidObj', 'OBJECT must be a valid device group object.');
end

if (nargin == 2)
    if ~strcmp(varargin{1}, '-full')
       error('icgroup:methods:invalidArg', ['Invalid second argument. The only valid second argument is ''-full''.',...
