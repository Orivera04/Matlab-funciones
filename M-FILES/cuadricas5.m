%Ecuaciones canónicas de las cuádricas
clc;
syms x y z x1 y1 z1 ;
...
disp(['Vector de traslación:[h,k,m]= [',num2str([h,k,m]),']']);
s=A*h^2+B*k^2+C*m^2+D*h+E*k+F*m+G;
disp(['T. independiente después de traslación: s=',num2str(s)]);
...
if numel(u(u>0))==2 %caso #1
    ...
    if numel(v(v==0))==3
       ...
       elseif s>0
           disp('Conjunto vacío');  
       elseif s<0
             disp('Cilndro elíptico o circular')
       end
   elseif numel(v(v~=0))==1  
           disp('Paraboloide elíptico')  
   end
...
elseif numel(u(u~=0))==2 %caso #2 
       ...
       if numel(v(v==0))==1
         if s==0
           ...
           disp('Cilindro hiperbólico')
         end
       elseif numel(v(v==0))==2
           disp('Paraboloide hiperbólico') 
       end 
elseif numel(u(u~=0))==1 %caso #3
    if A~=0,D=0;end;if B~=0,E=0;end;if C~=0,F=0;end;
        v=[D,E,F];
        if numel(v(v==0))==1
            disp('Cilindro parabólico rotado')
        elseif numel(v(v==0))==2
            disp('Cilindro parabólico')    
