...
%   $Revision: 1.1.6.4 $ $Date: 2004/04/11 00:33:49 $
...
if ~isequal(s1,s2)
    error('timeseries:minus:sizemis','Size of the data arrays are mismatching')
end
...
try
   ...
   try 
       ...
       errstr = sprintf('%s\n%s','Minus failed for this type of ordinate data object.', ...
                 'Try implementing a double cast method or overloaded addition for this object');
       error('timeseries:minus:badcast',errstr)
   end
end
...
set(tsout,'Name',['Minus_' ts1.Name '_' ts2.Name]);
