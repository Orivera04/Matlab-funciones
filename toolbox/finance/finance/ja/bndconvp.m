% BNDCONVP   与えられた価格から債券のコンベクシティ
%
% この関数は、NUMBONDS 確定利付証券のコンベクシティを、それぞれの債券の
% クリーン価格から計算します。この関数は、債券のクーポン構造が最初または
% 最後の期間が、短期または長期である場合でも、債券のコンベクシティを計算
% することができます(すなわち、クーポン構造が満期と同期しているかどうかに
% かかわらず債券のコンベクシティを計算できます)。この関数は、ゼロクーポン
% 債のコンベクシティの計算も可能にします。
%
...
%                    は無視されます)
%  Face            - 債権の額面価値：デフォルトは100
%         
% 出力:    
%  YearConvexity   - 年間コンベクシティ
%  PerConvexity    - 期間(Periodic)コンベクシティ
%         
% 出力は、NUMBONDS行1列のベクトルです。
%
% 参考 : BNDCONVY, BNDDURP, BNDDURY.
