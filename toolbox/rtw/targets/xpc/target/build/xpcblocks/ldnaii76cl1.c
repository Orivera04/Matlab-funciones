// ldnaii76cl1.c - xPC Target, non-inlined S-function driver for the
// L/D section of the NAII Apex 76CL1 Series LVDT/RVDT boards
// Copyright 2003 The MathWorks, Inc.

#define S_FUNCTION_LEVEL         2
#undef  S_FUNCTION_NAME
#define S_FUNCTION_NAME          ldnaii76cl1
...
#define NUM_ARGS                 (9)
#define CHANNEL_ARG              ssGetSFcnParam(S, 0) // vector int 1-8
...
#define LD_VELOCITY              (0x020 / 2) // read
#define SCALE                    (0x040 / 2) // read/write
#define ACTIVE_CHANNELS_LD       (0x060 / 2) // read/write
#define TEST_D2_VERIFY_LD        (0x064 / 2) // read/write
#define TEST_ENABLE_LD           (0x068 / 2) // read/write
#define SIGNAL_STATUS_LD         (0x06C / 2) // read
#define EXCITATION_STATUS_LD     (0x070 / 2) // read
#define TEST_STATUS_LD           (0x074 / 2) // read
#define LATCH                    (0x078 / 2) // write
#define LVDTD_TEST_POSITION      (0x07C / 2) // read/write
#define WIRE_MODE_LD             (0x080 / 2) // read/write
#define A_B_RESOLUTION           (0x084 / 2) // read/write
#define VELOCITY_SCALE           (0x0A4 / 2) // read/write
#define MAGNITUDE_A_B            (0x0C4 / 2) // read/write
...
static void mdlStart(SimStruct *S)
{
    ...
    for (i = active = wiring = 0; i < nChans; i++) {
        ...
        if (chan >= MAX_LD) {
            sprintf(msg, "No such NAII Apex 76CL1 L/D channel: %d", chan + 1);
            ssSetErrorStatus(S, msg);
            return;
        }
