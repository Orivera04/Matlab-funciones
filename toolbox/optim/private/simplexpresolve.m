...
%SIMPLEXPRESOLVE Preprocessing procedure to reduce redundancy in a linear system with bounds.
...
%   $Revision: 1.5.6.6 $  $Date: 2004/02/01 22:09:42 $
...
%       detect the incompatible solution solved from all singleton rows
%       check whether the solution from singleton rows satisfy the boundary conditions
RowSgtonsExist = 0;
...
if (~isempty(Acurr)) && ( nnz(Acurr)/(mcurr*ncurr) < 0.1 )
    nnzAcurr = nnz(Acurr);
    ubImplied = repmat(Inf, nnzAcurr, 1);
    lbImplied = -ubImplied;
    ...
    sumcolnz = sum( spones(Acurr), 1 );
    colcurr = zeros(length(lbcurr), 1); 
    colcurr = (sumcolnz == 1)' & ( ( isinf(ubcurr) & isinf(lbcurr) ) | freeImp );
    ...
    if  ColSgltonsExist && ( length(bcurr) > nnz(colcurr) )
        ...
        [tmp, coldel, valdel] = find(ssubAcurr);                            
        ...
        if any(rowdel) && (~isempty(coldel))    
            ...
            if computeLambda
                ...
                restoreSol(3).lorow = Aindex(rowdel);
