function project(u,w)                          %last updated 4/4/00
...
if nargin==0
   ...
   while validch=='N'
      ...
      ch=input(s1);
      if isempty(ch)==1
         disp('No selection made. Try again.')
         disp(cont)
         ch=999;
         pause
      end   
...
if kind==2   %----- 2D code
   ...
   axis(axis)
   axis('off')
   title('ORTHOGONAL PROJECTION of 2D VECTORS','fontweight','bold')
   hold on
   tic;while toc<2,end
   plot([0,u(1)],[0,u(2)],'-y','erasemode','none','linewidth',2)%PLOT VECTOR U
   text(u(1)/2,u(2)/2,'U','color','y','erasemode','none','fontweight','bold')
   drawnow 
   tic;while toc<4,end
   plot([0,w(1)],[0,w(2)],'-b','erasemode','none','linewidth',2)%PLOT VECTOR W
   text(4*w(1)/8,4*w(2)/8,'W','color','b','erasemode','none','fontweight','bold')
   drawnow
   tic;while toc<4,end
   %working on the projection next
   plot([u(1) proj(1)],[u(2) proj(2)],':r','erasemode','none')
   tic;while toc<4,end
   drawnow
   plot([0,proj(1)],[0,proj(2)],'-m','erasemode','none','linewidth',2)
   text(proj(1)/2,proj(2)/2,'P','color','m','erasemode','none','fontweight','bold')
   drawnow
   tic;while toc<3,end
   finst=[s8 '      ' cont];
   handelq=text(.2,.05,finst,'units','normalized',...
           'color','red','erasemode','none','fontweight','bold');
   drawnow
   hold off
...
if kind==3    %----- 3D code
   ...
   drawnow
   hold on
   plot([0 xax2(1)],[0 xax2(2)],'-k',[0 yax2(1)],[0 yax2(2)],'-k',...
        [0 zax2(1)],[0 zax2(2)],'-k','erasemode','none')
   title('ORTHOGONAL PROJECTION of 3D VECTORS','fontweight','bold')
   plot(0,0,'*k','erasemode','none','markersize',16)
   drawnow
   text(xax2(1),xax2(2),'X','erasemode','none','fontweight','bold')
   drawnow
   text(yax2(1),yax2(2),'Y','erasemode','none','fontweight','bold')
   drawnow
   text(.9*zax2(1),.9*zax2(2),'Z','erasemode','none','fontweight','bold')
   drawnow
   tic;while toc<2,end
   plot([0 u2(1)], [0 u2(2)],'-y','erasemode','none','linewidth',2)
   drawnow
   text(u2(1)/2,u2(2)/2,'U','color','y','erasemode','none','fontweight','bold')
   drawnow
   tic;while toc<2,end
   plot([0 w2(1)], [0 w2(2)],'-b','erasemode','none','linewidth',2)
   drawnow
   text(.5*w2(1),.5*w2(2),'W','color','b','erasemode','none','fontweight','bold')
   drawnow
   tic;while toc<2,end
   %drawing projection things
   plot([u2(1) proj(1)],[u2(2) proj(2)],':r','erasemode','none')
   drawnow
   tic;while toc<3,end
   plot([0,proj(1)],[0,proj(2)],'-m','erasemode','none','linewidth',2)
   drawnow
