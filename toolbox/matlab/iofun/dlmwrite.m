...
%   platform.
%
%   See also DLMREAD, CSVWRITE, CSVREAD, WK1WRITE, WK1READ, NUM2STR, 
...
%   $Revision: 5.20.4.8 $  $Date: 2004/04/10 23:29:20 $
%-------------------------------------------------------------------------------
if nargin < 2
...
if options > 0
    ...
    if newinputform
        ...
        while (i <= length(varargin))
            if strmatch(lower(varargin{i}),append,'exact')
                appendmode = true;
                i = i+1;
            else
                ...
                if strmatch(lower(varargin{i}),delimiter,'exact')
                    dlm = setdlm(varargin{i+1},dlm);
                elseif strmatch(lower(varargin{i}),lineterminator,'exact')
                    newline = setnewline(varargin{i+1},newline);
                elseif strmatch(lower(varargin{i}),rowoffset,'exact')
                    r = setroffset(varargin{i+1},r);
                elseif strmatch(lower(varargin{i}),coloffset,'exact')
                    c = setcoffset(varargin{i+1},c);
                elseif strmatch(lower(varargin{i}),precision,'exact')
                    precn = varargin{i+1};
                else
        ...
        if options > 0  && ~isempty(varargin{1})
            dlm = setdlm(varargin{1},dlm);
        end

        % row and column offsets defaults to zero
        if options > 1 && ~isempty(varargin{2})
            r = setroffset(varargin{2},r);
        end
        if options > 2 && ~isempty(varargin{3})
            c = setcoffset(varargin{3},c);
        end
    end
end
%------------------------------------------------------------------------------
function out = setdlm(in,default)
tmp = sprintf(in);
...
%------------------------------------------------------------------------------
function out = setnewline(in,default)
if ischar(in)
    if strcmpi(in,'pc')
        out = sprintf('\r\n');
...
%------------------------------------------------------------------------------
function out = setroffset(in,default)
if isnumeric(in)
    out = in;
else
...
%------------------------------------------------------------------------------
function out = setcoffset(in,default)
if isnumeric(in)
    out = in;
else
