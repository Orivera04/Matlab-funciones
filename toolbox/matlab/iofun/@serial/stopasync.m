...
%   $Revision: 1.8.4.3 $  $Date: 2004/01/16 20:04:44 $
...
if errorOccurred
    if length(jobject) == 1
        error('MATLAB:serial:stopasync:opfailed', lasterr);
    else
        warnState = warning('backtrace', 'off');
        warning('MATLAB:serial:stopasync:invalid', 'An object in OBJ could not be stopped.');
        warning(warnState);
    end
end
