...
%   $Revision: 1.1.6.2 $  $Date: 2003/08/01 18:14:31 $

pkg = findpackage('MapViewer');
c = schema.class(pkg,'DataTipState');

p = schema.prop(c,'LabelHandles','MATLAB array');
...
p = schema.prop(c,'ActiveLayerName','MATLAB array');
p.AccessFlags.PrivateGet = 'on';
p.AccessFlags.PrivateSet = 'on';
p.AccessFlags.PublicGet  = 'on';
p.AccessFlags.PublicSet  = 'on';
