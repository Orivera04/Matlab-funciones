...
disp('Enter a 2 by 3 matrix A by entries between square brackets [ ... ]')
disp('or use the name of an existing 2 by 3 matrix.')
disp(s0)
...
disppmat=[disppmat 'prevl1=text(.2,vpos(jj),B(jj,:),''color'',''blue'',''fontweight'',''bold'');end,'];
disppmat=[disppmat 'if jj==2,'];
disppmat=[disppmat 'prevl2=text(.2,vpos(jj),B(jj,:),''color'',''red'',''fontweight'',''bold'');end,'];
disppmat=[disppmat 'end'];

% show lines of current matrix
dispcmat='axes(basehndl);for jj=1:m,';
dispcmat=[dispcmat 'if jj==1,'];
dispcmat=[dispcmat 'currl1=text(.2,vposcur(jj),B(jj,:),''color'',''blue'',''fontweight'',''bold'');end,'];
dispcmat=[dispcmat 'if jj==2,'];
dispcmat=[dispcmat 'currl2=text(.2,vposcur(jj),B(jj,:),''color'',''red'',''fontweight'',''bold'');end,'];
...
%Screen Title
axes(basehndl);
text(.3,1.05,header,'color','magenta','fontsize',20,...
...
%
axes(basehndl);
text(-.15,.68,'<>Input Regions<>','color','black','fontsize',16,...
    'fontweight','bold','erasemode','none')
...
intexpos=[.10 .50 .05 .05;.22 .50 .05 .05;.14 .57 .05 .05];
...
%First Message
set(txtHndl,'string','ORIGINAL SYSTEM of EQUATIONS.');
