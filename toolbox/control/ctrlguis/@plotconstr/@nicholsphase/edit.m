...
%EDIT  Builds Nichols phase margin constraint parameter editor.

%   Authors: Bora Eryilmaz
%   Revised:
%   Copyright 1986-2002 The MathWorks, Inc. 
%   $Revision: 1.10 $ $Date: 2002/04/10 05:09:33 $
...
P2 = MWPanel(GL_21);
W = MWLabel(sprintf('%s', 'Phase Margin > '), LEFT);
...
props = [Constr.findprop('MarginPha'); ...
...
Margin = unitconv(Constr.MarginPha, 'deg', XUnits);
...
if ~isempty(v) & (v>0) & (v<=180)
  % Update phase margin (in deg)
  R = Constr.recordon;
  Constr.MarginPha = v;
  Constr.recordoff(R);
