function [Price, PriceTree] = capbyhjm(HJMTree, varargin)
%CAPBYHJM Price caps from an HJM interest rate tree.
%
...
%        the HJM Tree.  The cap argument "Settle" is ignored.
%
% See also HJMTREE, CFBYHJM, FLOORBYHJM, SWAPBYHJM

%   Author(s): M. Reyes-Kattar 09/27/1999
%   Copyright 1998-2003 The MathWorks, Inc. 
%   $Revision: 1.20.2.1 $  $Date: 2003/08/29 04:46:16 $

%---------------------------------------------------------
%Checking the input arguments.
%---------------------------------------------------------
if (nargin < 4)
   error('finderiv:capbyhjm:InvalidInputs','You must enter HJMTree, Strike, Settle and Maturity');
end

if ~isafin(HJMTree,'HJMFwdTree')
  error('finderiv:capbyhjm:InvalidTree','The first argument must be an HJM tree created by HJMTREE');
end
...
[Strike, Settle, Maturity, CapReset, Basis, Principal] = ...
   instargcap(varargin{1}, HJMTree.TimeSpec.ValuationDate, varargin{3:end});
...
if ~isempty(CapSettle) & any(CapSettle ~= Settle(1))
   warning('finderiv:capbyhjm:IgnoredSettle','Caps are valued at HJM Tree ValuationDate rather than Settle');
end
...
if (~all(ismember(AllTimes, TreeTimes)))
    ...
    if(ConstRate)
        if(ShowWarnings)
            warning('finderiv:capbyhjm:ResultsApproximated','Not all cash flows are aligned with the tree. Result will be approximated.');    
        end    	    
    else        
        if(ShowWarnings)
            warning('finderiv:capbyhjm:RebuildingTree','Not all cash flows are aligned with the tree. Rebuilding tree.');    
        end
...
function CF = FindCF(HJMTree, EndTime, StartTimes, Strike, Principal, CapReset)
