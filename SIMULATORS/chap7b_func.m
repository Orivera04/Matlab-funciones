...
%(b)Given T = 1 minute, mean speed = 40 km/hour, standard deviation = 5km/hour, find 
%the mean sojourn time for alpha equal to pi/6, pi/4 and pi/2 respectively.  Comment 
%on the effect of alpha.

function chap7b_func (action)

handle = findobj(gcbf, 'Tag', 'mean');
mean = eval(get(handle,'String'));
...
for j= 1:N
   ...
   while (Done == 0 & count < 1000)
      ...
      %direction change in next period uniformly distributed between -alpha to alpha
      delta_D = 2* alpha(i) * rand(1) - alpha(i);
