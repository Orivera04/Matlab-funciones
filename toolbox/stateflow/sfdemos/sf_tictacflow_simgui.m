function handle = sf_tictacflow_simgui(method,varargin)

%   Copyright 1995-2003 The MathWorks, Inc.
%   $Revision: 1.4.2.2 $  $Date: 2004/04/15 00:53:10 $
...
if isnumeric(method)
	...
	otherwise
		error('unknown numeric method in sf_tictacflow_simgui');
	end
else
	switch method
	case 'fig'
	...
	case 'ioButtonNumber';
		...
		handle = ioButtonNumber;
	otherwise
		error('unknown char method in sf_tictacflow_simgui');
	end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
set_param('sf_tictacflow_sim/io subsystem','StopFcn',sprintf('if ishandle(%f); delete(%f); end;',fig,fig));
...
%callback = sprintf('set_param(''sf_tictacflow/simulink io/ioPressedButton'',''Value'',''%.20g'');sf_tictacflow_simgui(''ioPressToggle'');',buttonNumber);
callback = sprintf('sf_tictacflow_simgui(''ioButtonNumber'',%.20g);sf_tictacflow_simgui(''ioPressToggle'');',buttonNumber);
...
callback = 'sf_tictacflow_simgui(''ioStartToggle'');';
