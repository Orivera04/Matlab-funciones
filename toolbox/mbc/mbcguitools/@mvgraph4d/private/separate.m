...
%   X=SEPARATE(X) checks the matrix X for identical rows
%   and if it finds them, separates them geometrically
%   so that their separation is 4*sqrt(eps).
%   The algorithm is designed for 2-column data, ie it
%   spreads points in the first two dimensions.
%   X=SEPARATE(X,DIST) provides a distance to separate
%   points by.
%
...
%   $Revision: 1.2.2.2 $  $Date: 2004/02/09 07:20:12 $
