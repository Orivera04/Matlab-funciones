function ret = getIRInfo_C6416DSK(modelName,RTWSampleTimes,adcSampleTime,dacSampleTime)

% $RCSfile: getIRInfo_C6416DSK.m,v $
% $Revision: 1.1.6.1 $ 
% $Date: 2004/01/22 18:37:24 $
% Copyright 2001-2003 The MathWorks, Inc.

try
    
    % DSK has fixed CPU clock rate
    CPU_CLOCK_RATE = 600e6; 
    
    ADCblock = find_system(modelName,'FollowLinks','on','LookUnderMasks','on','MaskType','C6416DSK ADC');
    DACblock = find_system(modelName,'FollowLinks','on','LookUnderMasks','on','MaskType','C6416DSK DAC');
    ...
    if (isempty(ADCblock) && isempty(DACblock)),
        ...
        ret.timerIntPeriod = floor (CPU_CLOCK_RATE * BaseSampleTime / 8);
        ...
        ret.DMAIntPeriod = floor (CPU_CLOCK_RATE * CODECsampleTime / 8);
