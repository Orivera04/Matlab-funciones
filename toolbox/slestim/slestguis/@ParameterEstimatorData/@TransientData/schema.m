...
% $Revision: 1.1.6.5 $ $Date: 2004/04/11 00:37:55 $

% Get handles of associated packages and classes
hCreateInPackage = findpackage('ParameterEstimatorData');

% Construct class
c = schema.class(hCreateInPackage, 'TransientData');

% Proxy to ParameterEstimator.TransientData properties
...
p = schema.prop(c, 'Time' , 'string');
p.SetFunction = @LocalSetTime;

schema.prop(c, 'TimeSrc', 'string');
schema.prop(c, 'TimeVal', 'MATLAB array');
...
for k = 1:length(value)
  ...
  if ~isempty( this.TimeVal )
    str2 = sprintf( '%d', length(this.TimeVal) );
  end
