function y = single_as_uint16_to_double(x)
%SINGLE_AS_UINT16_TO_DOUBLE Convert single precision numbers to double.
%
%   SINGLE_AS_UINT16_TO_DOUBLE converts single precision floating point
%   numbers, each represented by two uint16 values (unsigned 16-bit integers),
%   to double precision numbers.
%
%   SINGLE_AS_UINT16_TO_DOUBLE(X), where X is an N-by-2 matrix of uint16's,
%   returns an N-by-1 vector of doubles.  Each row vector in X is converted to
%   one double.
%
%   SINGLE_AS_UINT16_TO_DOUBLE(X), where X is an N-by-2*M-by-P-by-... array
%   returns an N-by-M-by-P-by... array of doubles.
%
%   X doesn't have to be of class uint16.  It is only assumed that X contains
%   uint16 values, i.e., that 0 <= X <= 2^16.
%
...
%
%      u16 = fread(fid, [2 n], '*uint16').';    % read bytes into n-by-2 array
%      dbl = single_as_uint16_to_double(u16);   % convert to double
%
%   To convert a single precision number represented as a hex string, one can
%   use
%
%      str = '40490fdb';                        % pi as hex string
%      u16 = sscanf(str, '%4x').';              % convert to two uint16's
%      dbl = single_as_uint16_to_double(u16);   % convert to double
%
%   See also HEX2NUM.
...
%   URL:         http://home.online.no/~pjacklam
   ...
   sx = size(x);
   if rem(sx(2), 2)
      error('Number of columns must be a multiple of 2.');
   end
   ...
   x = reshape(x, [sx(1) 2 prod(sx)/(sx(1)*2)]);

   % extract the sign
   s = logical(bitget(x(:,1,:), 16));

   % extract the exponent
   e = bitand(bitshift(x(:,1,:), -7), 255);

   % extract the fraction
   f =   pow2(double(bitand(x(:,1,:), 127)),  -7) ...
       + pow2(double(       x(:,2,:)      ), -23);
