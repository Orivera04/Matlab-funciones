/*
 * fft_r2dif_tblm_z_rt.c - Signal Processing Blockset FFT run-time library components.
 *
 *  Copyright 1995-2003 The MathWorks, Inc.
 *  $Revision: 1.6.2.3 $  $Date: 2004/04/12 23:43:39 $
 */
 ...
 */
EXPORT_FCN void MWDSP_R2DIF_TBLM_Z(
    creal_T      *y,
    int_T         nChans,
    const int_T   nRows,
    const int_T   fftLen,
    ...
    while(nChans--) {
        ...
        for(k=twiddleStep; k < nHalf; k<<=1) { /* skip last butterfly */
            ...
            for (j = 0; j < n2; j++) {
                ...
                twid.re = (j>n4)
                        ? -twiddleTable[nHalf-tableIdx]
                        :  twiddleTable[tableIdx];
                twid.im =  twiddleTable[ (j>=n4) ? tableIdx-nQrtr : nQrtr-tableIdx ];
