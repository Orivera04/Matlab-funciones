...
%      editcoeff(f);
%
%   See also SELECTFIT, FIT, UNDOFIT, RMFIT, PLOTSAMPLE, PICKDATA, INTERP1.
    ...
    if ischar(fp.fitcolor) || length(fp.fitcolor)==3
        ...
        if ishandle(f.hdata)
            ...
            if isfield(co,'Color'),
                ...
                % hist data do not have Color property
                fitcolor=[0 0 0];
            end;
        else
            fitcolor=[0 0 0];  % problem if f is a fit structure from a deleted curve (new v2.05)
        end;
    end;
end;
