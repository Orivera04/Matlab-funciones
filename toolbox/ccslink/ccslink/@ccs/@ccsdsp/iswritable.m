...
%   writing on the DSP processor referenced by CC. Conversely, this test
...
%   values to be checked for write ability.  To mimic the syntax of READ, 
%   this parameter can be a vector to define multidimensional data blocks.  
%   The size of the data block that is tested by this routine is always equal 
%   to the product of all elements in the COUNT vector.  Thus the following
%   examples are equivalent:
%     cc.iswritable('1000','int16',[6 3 2])  % check write status of 3d array
%     cc.iswritable('1000','int16',36)       % Same as above
%
%   ISWRITABLE(CC,ADDRESS,DATATYPE)  Same as above, except COUNT defaults 
...
%   Code Composer workspace, this method will not produce meaningful results.
%
%   See also ISREADABLE, WRITE, READ, HEX2DEC.

%   Copyright 2001-2004 The MathWorks, Inc.
%   $Revision: 1.11.4.3 $ $Date: 2004/04/06 01:04:48 $
...
elseif iscell(address) & length(address) == 1,
    address{2} = cc.page;
...
if nargin == 3,
    resp=callSwitchyard(cc.ccsversion,[12 cc.boardnum cc.procnum 0 0],address,datatype);
elseif nargin >= 4,
    resp=callSwitchyard(cc.ccsversion,[12 cc.boardnum cc.procnum 0 0],address,datatype,count);
end
result = (resp==1);
