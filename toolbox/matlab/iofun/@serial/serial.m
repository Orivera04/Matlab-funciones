...
%   $Revision: 1.10.4.3 $  $Date: 2004/01/16 20:04:40 $
...
if ~(strcmp(computer, 'PCWIN') || strcmp(computer, 'SOL2') || strcmp(computer, 'GLNX86'))
    error('MATLAB:serial:serial:invalidPlatform', 'The serial port object is supported on the Windows, Solaris and Linux platforms only.');
end
...
case 1
    if (strcmp(class(varargin{1}), 'char'))
        ...
        try
            ...
            error('MATLAB:serial:serial:cannotCreate', lasterr);
        end
        obj.type = 'serial';
