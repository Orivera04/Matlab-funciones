/* $Revision: 1.5.2.3 $ */
/*
 * chol_c_rt.c - Signal Processing Blockset Cholesky factorization run-time function
...
EXPORT_FCN void MWDSP_Chol_C(creal32_T *A, int_T n)
{
    ...
    for (j=0; j<n; j++) {
        ...
        A_jn[j].im = 0.0F;  /* Remove imaginary part on diagonal since */
                            /* this matrix is supposed to be Hermitian. */

        for (k=0; k<j; k++) {
            creal32_T t = {0.0F, 0.0F};
