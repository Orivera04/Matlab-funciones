function [xdata, ydata, errWorst] = look1_func_approx(funcStr,xMin,xMax,xdt,xscale,ydt,yscale,rndMeth,errMax,nPtsMax,varName,spacing,Recompute);
%LOOK1_FUNC_APPROX find points for lookup table approximate to a function. 
%
%[ XDATA, YDATA, ERRWORST ] = LOOK1_FUNC_APPROX( FUNCSTR, XMIN, XMAX,
%                                                      XDT, XSCALE,
%                                                      YDT, YSCALE, RNDMETH,
...
%     Example:
%       look1_func_approx('sin(2*pi*x)',0,0.25,ufix(16),2^-16,sfix(16),2^-14,'floor',0.01,11,'un')
%
...
%     Example:
%       look1_func_approx('sin(2*pi*x)',0,0.25,ufix(16),2^-16,sfix(16),2^-14,'floor',0.01,[],'pow2');
%     
...
%     Example:
%       look1_func_approx('sin(2*pi*x)',0,0.25,ufix(16),2^-16,sfix(16),2^-14,'floor',[],17,'un');
%
...
% $Revision: 1.1.6.2 $  
% $Date: 2004/04/04 03:39:09 $

if nargin < 11 || isempty(varName)
    varName = 'x';
end
if nargin < 13 || isempty(Recompute)
  % Set Recompute to default value 
  Recompute = 1;
end
if nargin < 12 || isempty(spacing)
   spacing = 'uneven';
end

if ~isfield(xdt, 'Class')
  xdt = float(xdt);
end
if ~isfield(ydt, 'Class')
  ydt = float(ydt);
end
...
if (Recompute ~= 1)
  ...
  [xdata,ydata,errWorst] = fixpt_func_approx(funcStr,xMin,xMax,xdt,xscale, ...
					     ydt,yscale,rndMeth,errMax,nPtsMax,spacing,varName);
  if isempty(DATA)
    DATA(1).ParamSettings = InData;
...
function [ xdata, ydata, errWorst ] = fixpt_func_approx(funcStr,xMin,xMax, ...
                                                    ...
                                                    rndMeth,errMax,nPtsMax,spacing,varName)
...
if errMaxValid & nPtsMaxValid
  ...
  %
  xdata = handle_pow2(funcStr,errMax,xMinFix,xMaxFix,xdt,xscale,ydt,yscale,rndMeth,varName);
  
  if length(xdata) > nPtsMax & evenAllowed
    %
    %  power of 2 was too big, try even space if allowed
    %
    xdata = handle_even(funcStr,errMax,xMinFix,xMaxFix,xdt,xscale,ydt,yscale,rndMeth,varName);
    
    if length(xdata) > nPtsMax & unevenAllowed
      %
      %  even was too big, so use uneven space if allowed
      %
      xdata = handle_uneven(funcStr,errMax,xMinFix,xMaxFix,xdt,xscale,ydt,yscale,rndMeth,varName);
    end
  end
...
elseif nPtsMaxValid
  ...
  %
  xdata = handle_best_npts(funcStr,nPtsMax,xMinFix,xMaxFix,xdt,xscale,ydt,yscale,rndMeth,unevenAllowed,evenAllowed,varName);
  
else
  error('Both upper limits were invalid.');
end  

errWorst = get_sim_error(funcStr,xdata,xMin,xMax,xdt,xscale,ydt,yscale,rndMeth,varName);    
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function xdata = handle_uneven(funcStr,errMax,xMinFix,xMaxFix,xdt,xscale,ydt,yscale,rndMeth,varName);
%
% uneven spacing case
%
...
while 1
  ...
  while ( xSmallestKnownBad - xLargestKnownGood ) > xbit
    ...
    if max( abs( yMid_ideal - yMid_fix ) ) > errMax
      ...
      errCur = get_sim_error(funcStr,xlast2,xlast2(1),xlast2(2),xdt,xscale,ydt,yscale,rndMeth,varName);    
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function xdata = handle_even(funcStr,errMax,xMinFix,xMaxFix,xdt,xscale,ydt,yscale,rndMeth,varName);
%
% even spacing case
%
...
while ( nSmallestKnownGood - nLargestKnownBad ) > 1
  ...
  errCur = get_sim_error(funcStr,xDataTry,xMinFix,xMaxFix,xdt,xscale,ydt,yscale,rndMeth,varName);    
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function xdata = handle_pow2(funcStr,errMax,xMinFix,xMaxFix,xdt,xscale,ydt,yscale,rndMeth,varName);
%
% power of 2 case
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function errCur = get_sim_error(funcStr,xdata,xMin,xMax,xdt,xscale,ydt,yscale,rndMeth,varName);
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function xdata = handle_best_npts(funcStr,nPts,xMinFix,xMaxFix,xdt,xscale,ydt,yscale,rndMeth,unevenAllowed,evenAllowed,varName);
...
errAttained = get_sim_error(funcStr,xdata,xMinFix,xMaxFix,xdt,xscale,ydt,yscale,rndMeth,varName);
...
while ( errAttained - errFailed ) > 0.5*errTol;
  ...
  if unevenAllowed
    ...
    xx = handle_uneven(funcStr,errAttempt,xMinFix,xMaxFix,xdt,xscale,ydt,yscale,rndMeth,varName);
    
  elseif evenAllowed
    
    xx = handle_even(funcStr,errAttempt,xMinFix,xMaxFix,xdt,xscale,ydt,yscale,rndMeth,varName);
  else
    
    xx = handle_pow2(funcStr,errAttempt,xMinFix,xMaxFix,xdt,xscale,ydt,yscale,rndMeth,varName);
  end
  
  errNew = get_sim_error(funcStr,xx,xMinFix,xMaxFix,xdt,xscale,ydt,yscale,rndMeth,varName);
