% Script p10_3_3a.m; min distance tractor-trailer problem; bang-zero-
...
%
flg=3;  
u0=5/3; s0=zeros(4,1); u=u0*[1 0 -1 1]'; optn=optimset('Display','Iter'); 
if flg==1,                                               % Simple offset
 sfd=[0 0 1 3]'; d0=[1 3 2 1]'; ax=[-1 4 .625*[-1.5 3.5]];  
 elseif flg==2,                                       % Fig. 3.12 Nguyen
  sfd=[-3*pi/4 0 -.3 1.3]'; d0=[.1 2 2 1]'; ax=[-1 4 .625*[-2.5 2.5]];  
 elseif flg==3,                                       % Fig. 3.11 Nguyen
  sfd=[0 -pi/2 1.2 1.8]'; u=-u; d0=[.1 2 2 2]'; ax=[-1.2 3.8 .625*[-1 4]]; 
 elseif flg==4,                                       % Fig. 3.10 Nguyen
  sfd=[pi/2 -pi/4 .25 .75]'; u=-u; d0=[.1 2 2 1]'; 
  ax=[-1 4 .625*[-2.5 2.5]];
end; d1=fsolve('tto',d0,optn,u,s0,sfd);
%
...
N=length(d); N1=N+1; [f,s,u]=tto(d,u,s0,sfd); 
ps=s(1,:); al=s(2,:); y=s(3,:); x=s(4,:); yf=y+.5*sin(ps); xf=x+.5*...
   cos(ps); be=ps-al; yb=y-sin(be); xb=x-cos(be); 
