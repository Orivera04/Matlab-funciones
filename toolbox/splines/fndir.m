...
%   $Revision: 1.15 $  $Date: 2003/04/25 21:12:11 $

if nargin<2
   error('SPLINES:FNDIR:needdir','The DIRECTION(S) must be specified.'), end
...
case 'pp' % the function is in ppform:
   ...
   if fnbrk(f,'var')>1 % the function is multivariate
      ...
      if dirsize(1)~=m
         error('SPLINES:FNDIR:mustbevec',...
              ['DIRECTION(s) should be  ',num2str(m),'-vector(s).'])
      end
...
case {'rB','rp'}
   error('SPLINES:FNDIR:notforst',...
        ['FNDIR does not work for rational splines. Use FNTLR instead.'])
case 'st'
   error('SPLINES:FNDIR:notforst',['FNDIR does not work for st functions.'])
otherwise
   error('SPLINES:FNDIR:unknownfn','F does not appear to describe a function.')
end
