/* 
 * polyval_zd_rt.c - Signal Processing Blockset Polynomial Evaluation run-time function 
 * 
 * Specifications: 
 * 
 * - Complex (double precision) Inputs, 
 * - Real (double precision) Coefficients,
 * - Complex (double precision) Outputs.    
 *  
 * 
 *  Copyright 1995-2004 The MathWorks, Inc.
 *  $Revision: 1.3.2.4 $  $Date: 2004/04/12 23:47:40 $ 
 */  
#include "dsp_rt.h" 

EXPORT_FCN void MWDSP_polyval_ZD(
    ...
    ) 
{
    ...
    for (i = 0; i < n; i++) { /* loop over inputs */
        ...
        for (j = 1; j < N; j++) {
            volatile real_T reInputProduct; /* volatile to fix MSV5 compiler bug */
            volatile real_T imInputProduct; /* volatile to fix MSV5 compiler bug */
