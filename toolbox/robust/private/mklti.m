function sys=mklti(varargin)
% MKLTI creates an LTI object SYS from system data matrices.
% SYS=MKLTI(A,B,C,D) or
% SYS=MKLTI(V1,V2,...,VN, TY) packs matrices describing a system into 
...
if nvars>nin,
   error(['Too few input arguments for MKLTI type ' ty]),
elseif nvars<nin,
   error(['Too many input arguments for MKLTI type ' ty]),
end
...
%  -------- End MKLTI.M -----------RYC/MGS 10/25/90 (Rev. 9/97, 12/98) %
