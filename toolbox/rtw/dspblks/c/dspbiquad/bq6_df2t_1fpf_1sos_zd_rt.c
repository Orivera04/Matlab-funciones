 ...
 *  $Revision: 1.3.2.2 $  $Date: 2004/04/12 23:42:02 $
 */

#include "dsp_rt.h"

EXPORT_FCN void MWDSP_BQ6_DF2T_1fpf_1sos_ZD (const creal_T *u,
                                  creal_T *y,
                                  creal_T *state,
                                  const real_T *coeffs,
                                  const real_T  a0inv,
  ...
  for (i=0; i++ < numChans; ) {
    for (j=0; j++ < sampsPerChan; ) {
      ...
      creal_T  in = *u++;             
      out.re      = (mem->re + in.re * *c) * a0inv; 
      out.im      = (mem->im + in.im * *(c++)) * a0inv; 
