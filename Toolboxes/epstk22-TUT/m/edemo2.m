...
eYAxisWestLabelText='Range';            % set West Label of YAxis
eYAxisWestLabelDistance=10;             % set West Label disctance from YAxis
...
eXGridDash=[1 0.5 2];                        % switch y-Grid on
eYGridVisible=1;                        % switch y-Grid on
eYGridDash=[1 0.5 ];                        % switch y-Grid on
...
% frame 
eframe(0,0,eWinWidth,eWinHeight,0.5,[1 1 3 6 3],[0 0 0],0,10); % draw frame around window 
...
eYAxisWestValueFormat='%d km';
eXAxisSouthValueFormat='sector %d';
% 1. plot lines
x=0:0.1:2*pi;
eplot(x,sin(x)*15+30,'street',0,[0.8 0.8 0],3) %solid line 
eplot(x,sin(x)*15+33,'power line',[0.5 0.8],[1 0 1]) % dash plot
eplot(x,sin(x)*15+36,'railway',[1 3 1],[0.3 0.3 0.4],1) % dot-line plot

% 2. plot symbols 
x=0:0.5:2*pi;
edsymbol('spire','spire.psd',0.3,0.3,0,0,90) % define symbol 'spire' with a symbol file
eplot(x,cos(x)*10+20,'tree','spire',[0 0.7 0]) % plot trees
edsymbol('textplot','10 0 moveto (<-tree) show',0.5,0.5,0,0,45) % define symbol 'textplot' with postscript commands
text='newpath 0 0 moveto -10 -10 lineto 10 -10 lineto closepath fill';
etxtwrit(text,'myNewSymbol.psd'); % write new symbol file
edsymbol('mysymbol','myNewSymbol.psd',0.5,0.5) % define symbol 'mysymbol'

eplot(x,cos(x)*10+20,'','textplot',[1 0 0]) % plot trees
eplot(x,cos(x)*10+20,'','mysymbol',[1 0 0]) % plot mysymbol
...
x1=x1';
x2=flipud(x1);
girl=[x1+2.8 sin(x1)*10+sin(x1*5)+45;x2+2.8 -sin(x2)*10+45];
im=eimgread;
...
posNo=3;
etext('3. Value=',valuePos(posNo,1),valuePos(posNo,2),4,4);
etext('(a+b)',0,0,4,1,13);
posNo=7;
...
delete('myNewSymbol.psd'); % delete new symbol file
if ~exist('noDemoShow')
