function E=symelem5(size,type,i,j,k)           % last updated 9/10/97
%SYMELEM5  Symbolic elementary matrix constructor in MATLAB 5.
...
%                          (k is a string)
%
%          Use in the form ==>  E = symelem5(size,type,i,j,k)  <==
%
...
if  nargin==5
   if isstr(k)~=1
      disp('Error in symelem5: last argument must be a string.')
      E=[];
      return
   end
end
...
if fix(abs(type))==0 | fix(abs(type))~=type | fix(abs(type))>3
   disp('Error in symelem5: type of operation is incorrectly specified.')
...
if type==2,E(i,i)=k;end %CODE for MATLAB 5
if type==3,E(j,i)=k;end
