...
% 2006/10/03: v1.03, now checks the version number in the file
%                    'ezyfitversion.txt'

verb=0;  % verbose mode

if ~exist('opt','var'), opt='command'; end;

v1=ver('ezyfit');  curv=str2double(v1.Version);
...
if newv>curv,
    if strcmp(opt,'command')
        disp(['New: EzyFit ' s ' is now available online.']);
