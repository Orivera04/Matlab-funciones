% Script p3_5_05f.m; DTDP for max xf with gravity, and spec. yf, u0,
% v0 using DOP0F; (u,v) in a*tf, (x,y) in a*tf^2, t in tf, g in a;
%                                                      3/97, 5/27/02
%
clear; clear global; global yf g; yf=0; g=0; N=20; u0=.5/sqrt(2);
v0=u0; c=pi/180; tf=1; la0=[1 -1 1 -1]; s0=[u0 v0 0 0]';
t=[0:N]/N; name='dtdpgic'; nu=-.403; p0=[la0 nu]; th0=-.38; 
optn=optimset('Display','Iter','MaxIter',5); 
p=fsolve('dopcf',p0,optn,name,th0,s0,tf,N);
[f,s,th]=dopcf(p,name,th0,s0,tf,N); thh=[th th(N)]/c;
...
%
figure(2); clf; subplot(211), zohplot(t,thh); grid; axis([0 1 -50 0])
