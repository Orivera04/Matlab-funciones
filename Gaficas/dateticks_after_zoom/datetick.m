...
%   $Revision: 1.23 $  $Date: 2001/04/15 12:03:25 $

[nin,ax,dateform,keep_ticks,keep_limits] = parseinputs(varargin);

labels = [];

% Compute data limits.
if keep_limits | isempty(get(gca,'children'))
   lim = get(gca,[ax 'lim']);
   vmin = lim(1);
   vmax = lim(2);
else
...
%-------------------------------------------------
function [nin,ax,dateform,keep_ticks,keep_limits] = parseinputs(v)
%Parse Inputs
