...
%   Mouse pointer/WindowButtonMotionFcn Revision by Richard Los 5-4-05
...
if(isempty(rotaObj))
    ...
    set(gcbf,'WindowButtonMotionFcn','rotate3d2(''motion'')');
end
...
if isempty(rotaObj) | ...
        isa(get(hFig,'WindowButtonMotionFcn'),'function_handle') || ~strcmp(get(hFig,'WindowButtonMotionFcn'),'rotate3d2(''motion'')')
    return;
else
    set(hFig,'WindowButtonMotionFcn',@localFigMotion);
    rdata = get(rotaObj,'UserData');
...
set(fig,'WindowButtonDownFcn','rotate3d2(''down'')');
set(fig,'WindowButtonUpFcn'  ,'rotate3d2(''up'')');
set(fig,'WindowButtonMotionFcn',@localFigMotion);
...
for i=1:length(allAxes),
   ...
   b = hggetbehavior(candidate_ax,'Rotate3d2','-peek');
...
function localFigMotion(hFigure,event)
rotaObj = findobj(allchild(hFigure),'Tag','MATLAB_Rotate3D_Axes');
rdata = get(rotaObj,'UserData');
hTarget = hittest(hFigure);
if ismember(ancestor(hTarget,'axes'),rdata.destAxis) | isempty(rdata.destAxis)
