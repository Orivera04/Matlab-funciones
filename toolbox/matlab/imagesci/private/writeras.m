...
%  $Revision: 1.1.6.1 $  $Date: 2003/12/13 03:01:42 $
   ...
   nd = ndims(data);
   if nd > 3
      error('MATLAB:writeras:tooManyDims', ...
            '%d-D data not supported for RAS files', nd);
   end
   ...
   if rem(length(varargin), 2)
      error('MATLAB:writeras:mismatchedParamValuePairs', ...
            'Odd number of Parameter-Value arguments.');
   end

   for k = 1:2:length(varargin)
      param = lower(varargin{k});
      if ~ischar(param)
         error('MATLAB:writeras:badParameterName', ...
               'Parameter name must be a string');
      end
      idx = strmatch(param, paramStrings);
      if isempty(idx)
         error('MATLAB:writeras:unrecognizedParameter', ...
               'Unrecognized parameter name "%s"', param);
      elseif (length(idx) > 1)
         error('MATLAB:writeras:ambiguousParameter', ...
      ...
      switch param
         ...
         case 'type'
            type = varargin{k+1};
            if ~ischar(type)
               error('MATLAB:writeras:badType', ...
                     'TYPE must be a string');
            end
            if ~ismember(type, {'standard', 'rgb', 'rle'})
               error('MATLAB:writeras:invaldRasType', ...
                     'Invalid RAS file type: "%s".', type);
            end
   ...
   if ~ismember(class(data), {'double', 'single', 'uint8', 'uint16'})
      error('MATLAB:writeras:invalidImageClass', ...
            'Unsupported image class "%s"', class(data));
   end
   if ~strcmp(class(map), {'double', 'single'})
      error('MATLAB:writeras:invalidMapClass', ...
            'Unsupported colormap class "%s"', class(map));
   end
   if ~ismember(class(alpha), {'double', 'single', 'uint8', 'uint16'})
      error('MATLAB:writeras:invalidAlphaClass', ...
            'Unsupported image class "%s"', class(alpha));
   end
   ...
   if islogical(data)
      ...
      if ~isempty(map)
         error('MATLAB:writeras:bitmapWithColormap', ...
               'Colormap not supported with RAS bitmap files.');
      elseif ~isempty(alpha)
         error('MATLAB:writeras:bitmapWithAlpha', ...
               'Alpha channel not supported with RAS bitmap files.');
      elseif ncomp ~= 1
         error('MATLAB:writeras:bitmapWith3dData', ...
               'Bitmap image matrix can not be 3-D with RAS files.');
      end
   ...
   elseif ~isempty(map)
      ...
      % First do some checking.
      if ~isempty(alpha)
         error('MATLAB:writeras:indexedWithAlpha', ...
               'Alpha channel not supported with indexed RAS files.');
      elseif ncomp ~= 1
         error('MATLAB:writeras:indexedWith3dData', ...
               'Index matrix can not be 3-D with RAS files.');
      end
      if size(map, 1) > 256
         error('MATLAB:writeras:tooManyColormapEntries', ...
               ['Colormaps with more than 256 colors are not supported' ...
                ' with RAS files.']);
      end
      ...
      k = map > 1;
      if any(k)
         warning('MATLAB:writeras:colormapClipped', ...
                 'Some colormap values are > 1, so clipping colormap.');
         map(k) = 1;
      end
      k = map < 0;
      if any(k)
         warning('MATLAB:writeras:colormapClipped', ...
                 'Some colormap values are < 0, so clipping colormap.');
         map(k) = 0;
      end
      ...
      switch class(data)
         case {'double', 'single'}
            if any(data(:) < 1)
               error('MATLAB:writeras:badIndexValues', ...
                     ['Index matrix of class double or single must have' ...
                      ' positive values.']);
            end
            if any(data(:) > maplength)
               error('MATLAB:writeras:badIndexValues', ...
                     'Index values too large for colormap.');
            end
...
%                           ' not "%s".'], class(data)));
   ...
   %
      ...
      % First do some checking.
      if (ncomp ~= 1) && (ncomp ~= 3)
         error('MATLAB:writeras:wrongDims', ...
               ['Data with %d components not supported for' ...
                        ' RAS files'], ncomp);
      end
      ...
      switch class(data)
         ...
         case 'uint8'
            % Nothing to do.
         case 'uint16'
            warning('MATLAB:writeras:reducedTo8Bits', '%s', ...
                    ['RAS images does not allow 16 bits pr' ...
      ...
      if ~isempty(alpha)
         if ncomp == 1
            error('MATLAB:writeras:grayscaleWithAlpha', ...
                  ['Alpha channel not supported with grayscale RAS' ...
                  ' files.']);
         end
         if size(alpha,1) ~= size(data,1) || size(alpha,2) ~= size(data,2)
            error('MATLAB:writeras:sizeOfAlpha', ...
                  ['Height and width of alpha data must match that' ...
                   ' of image data.']);
         end
   ...
   if strcmp(type, 'rle')
      if bitdepth <= 8
         ...
         warning('MATLAB:writeras:writingStandardNotRle', ...
                 ['RLE not supported with %d bit RAS files.' ...
   ...
   if fid < 0
      error('MATLAB:writeras:fileOpen', ...
            [filename ': can''t open file for writing.']);
   end
