...
if dimension==1;
    if strcmp(analysisarealabel,'matrix');
        [loadingname, loadingpath]=uigetfile('*.mat','Select Primary State',5,5); load([loadingpath,loadingname]);
...
btotal(find(isnan(btotal)))=aoriginal(find(isnan(btotal)))+arange;
...
if compare_any_successful==1;
    ...
    high=(lowestavalue+(arange*2)-1);
    set(findobj('Tag','highinput'), 'string',num2str(high));
% blanked since created autoscale    rangescale=(arange*2); set(findobj('Tag','rangescaleinput'), 'string',num2str(rangescale));
