/* $Revision: 1.11.6.1 $ $Date: 2003/11/20 11:57:28 $ */
/* daupdategsadadio.c -- D/A update module for the General Standards PMC-ADADIO */
/* multifunction board. */
/* Copyright 1994-2003 The MathWorks, Inc. */

#define 	S_FUNCTION_LEVEL 	2
#undef 		S_FUNCTION_NAME
#define 	S_FUNCTION_NAME 	daupdategsadadio
...
static void mdlStart(SimStruct *S)
{
    ...
    //printf("daupdate: mdlStart: addrmode = %d\n", addrmode );
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    if( enabled )
    {
        readLongIO( BCR, temp );  // Don't clear INTERRUPT_REQUEST.
        // We must clear the manual AD trigger bit or we might start another
        // acquisition if the last one finished right after we read BCR.
        temp = ( temp & ~MANUAL_AD_TRIGGER ) | OUTPUT_STROBE;
...
{   
    ...
    //printf("daupdate: terminate starting\n");
    if ((int_T)mxGetPr(INIT_ARG)[0])
    {
        // Write the values from the DtoA initial value vector
        {
    ...
    //printf("daupdate: terminate ending\n");
#endif
