...
local($RAM_DEFINITION);
local($ROM_DEFINITION);
...
sub readRuleFile {
	...
	while(<$FILE_HANDLE>) {
		...
		/^ROM_DEFINITION/ and $ROM_DEFINITION = $current_line[2];
		/^RAM_DEFINITION/ and $RAM_DEFINITION = $current_line[2];
		if (/^MM_/) {
			if (/^MM_LABEL/) {
...
local(@BSS_SECTIONS);
local(@GROUP1_SECTIONS);
local(@GROUP2_SECTIONS);
local(@GROUP3_SECTIONS);
...
sub readIRFile {
	...
	while(<$FILE_HANDLE>) {
		...
		/^BSS end/ and $in_bss_section = 0;
		/^GROUP1 end/ and $in_group1_section = 0;
		/^GROUP2 end/ and $in_group2_section = 0;
		/^GROUP3 end/ and $in_group3_section = 0;
		...
		if ($in_bss_section) {
			push @BSS_LINES, $_;
			if (!isInList($current_line2[0], BSS_SECTIONS)) {
				push @BSS_SECTIONS, $current_line2[0];				
			}
		}
		if ($in_group1_section) {
			push @GROUP1_LINES, $_;
			if (!isInList($current_line2[0], GROUP1_SECTIONS)) {
				push @GROUP1_SECTIONS, $current_line2[0];				
			}
		}
		if ($in_group2_section) {
			push @GROUP2_LINES, $_;
			if (!isInList($current_line2[0], GROUP2_SECTIONS)) {
				push @GROUP2_SECTIONS, $current_line2[0];				
			}
		}
		if ($in_group3_section) {
			push @GROUP1_LINES, $_;
			if (!isInList($current_line2[0], GROUP3_SECTIONS)) {
				push @GROUP3_SECTIONS, $current_line2[0];				
			}
		}
		if ($in_stack_section) {
			push @STACK_LINES, $_;
		...
		/^BSS begin/ and $in_bss_section = 1;
		/^GROUP1 begin/ and $in_group1_section = 1;
		/^GROUP2 begin/ and $in_group2_section = 1;
		/^GROUP3 begin/ and $in_group3_section = 1;
...
sub RAMorROM {
	...
	if (index($ROM_DEFINITION, $section_name) >=0 ) {
		return "ROM";
	#} elsif (($section_name eq "DATA")) {
	} elsif (index($RAM_DEFINITION, $section_name) >=0 ) {
		return "RAM";
	} else { print "haha ", $section_name};
}

sub isRAM {
	my($section_name) = @_;
	if (index($RAM_DEFINITION, $section_name) >=0 ) {
		return 1;
	} else {
		return 0;
	}
...
sub createEntireSummary {
	...
	local(@ROM_LINES) = ();
 	...
 	foreach $eachline(@SL_LINES) {
 		...
 		if (isRAM($comma_deli_words[1])) {
 			push @RAM_LINES, $temp;
 			$ram_sum += $comma_deli_words[5];
 			if (!isInList($comma_deli_words[6], RAM_SECTIONS)) {
 				push @RAM_SECTIONS , $comma_deli_words[6];
 			} 			
 		}
 		#} elsif (RAMorROM($comma_deli_words[1]) eq "ROM") {
 		if (isROM($comma_deli_words[1])) {
 			push @ROM_LINES, $temp;
 			$rom_sum += $comma_deli_words[5];
 			if (!isInList($comma_deli_words[6], ROM_SECTIONS)) {
 				push @ROM_SECTIONS , $comma_deli_words[6];
 			} 			
 		} 
 	}
 	push @returnlines, '<P CLASS="margin">';
	...
	if ($ListSectionName) {
		$RAM_string = 'Read/Write Memory Usage (' . join(" ", @RAM_SECTIONS) . ')';
		$ROM_string = 'Read Only Memory Usage (' . join(" ", @ROM_SECTIONS) . ')';
	}
...
sub createMAPfile {
	@mapfile = ();
	push @mapfile, '<p><A NAME="MAPfile"><FONT SIZE=+2 COLOR=#000066> <B CLASS="midprod">Original MAP File</B></FONT></A></p>';
...
sub createMemoryMAPSummary {
	...
	$BSS_size = 0;
	foreach $eachline(@BSS_LINES) {
		@comma_deli_words = split(/,+/, $eachline);
		$BSS_size = $BSS_size + $comma_deli_words[2];
	}
	
	$GROUP1_size = 0;
	foreach $eachline(@GROUP1_LINES) {
		@comma_deli_words = split(/,+/, $eachline);
		$GROUP1_size = $GROUP1_size + $comma_deli_words[2];
	}
	
	$GROUP2_size = 0;
	foreach $eachline(@GROUP2_LINES) {
		@comma_deli_words = split(/,+/, $eachline);
		$GROUP2_size = $GROUP2_size + $comma_deli_words[2];
	}
	
	$GROUP3_size = 0;
	foreach $eachline(@GROUP3_LINES) {
		@comma_deli_words = split(/,+/, $eachline);
		$GROUP3_size = $GROUP3_size + $comma_deli_words[2];
	}
	...
	foreach $eachcol (@MM_TYPES) {
		...
		if (index($MM_CONTENTS[$idx], 'CODE') >=0) {
			...
			foreach $each_section(@CODE_SECTIONS) {
				if (!(index($MM_header, $each_section . " ")>=0)) {
					$MM_header = $MM_header . $each_section . " ";
				}
			}
		}
		if (index($MM_CONTENTS[$idx], 'CONST') >=0) {
			...
			foreach $each_section(@CONST_SECTIONS) {
				if (!(index($MM_header, $each_section . " ")>=0)) {
					$MM_header = $MM_header . $each_section . " ";
				}
			}
		...
		if (index($MM_CONTENTS[$idx], 'DATA') >=0) {
			...
			foreach $each_section(@DATA_SECTIONS) {
				if (!(index($MM_header, $each_section . " ")>=0)) {
					$MM_header = $MM_header . $each_section . " ";
				}
			}
		...
		if (index($MM_CONTENTS[$idx], 'UNKNOWN') >=0) {
			...
			foreach $each_section(@UNKNOWN_SECTIONS) {
				if (!(index($MM_header, $each_section . " ")>=0)) {
					$MM_header = $MM_header . $each_section . " ";
				}
			}
		...
		if (index($MM_CONTENTS[$idx], 'DWARF') >=0) {
			...
			foreach $each_section(@DWARF_SECTIONS) {
				if (!(index($MM_header, $each_section . " ")>=0)) {
					$MM_header = $MM_header . $each_section . " ";
				}
			}
		}
		if (index($MM_CONTENTS[$idx], 'BSS') >=0) {
			$MM_size = $MM_size + $BSS_size;
			#$MM_header = $MM_header . join(" ", @BSS_SECTIONS);
			foreach $each_section(@BSS_SECTIONS) {
				if (!(index($MM_header, $each_section . " ")>=0)) {
					$MM_header = $MM_header . $each_section . " ";
				}
			}
		}
		if (index($MM_CONTENTS[$idx], 'GROUP1') >=0) {
			$MM_size = $MM_size + $GROUP1_size;
			#$MM_header = $MM_header . join(" ", @GROUP1_SECTIONS);
			foreach $each_section(@GROUP1_SECTIONS) {
				if (!(index($MM_header, $each_section . " ")>=0)) {
					$MM_header = $MM_header . $each_section . " ";
				}
			}
		}
		if (index($MM_CONTENTS[$idx], 'GROUP2') >=0) {
			$MM_size = $MM_size + $GROUP2_size;
			#$MM_header = $MM_header . join(" ", @GROUP2_SECTIONS);
			foreach $each_section(@GROUP2_SECTIONS) {
				if (!(index($MM_header, $each_section . " ")>=0)) {
					$MM_header = $MM_header . $each_section . " ";
				}
			}
		}
		if (index($MM_CONTENTS[$idx], 'GROUP3') >=0) {
			$MM_size = $MM_size + $GROUP3_size;
			#$MM_header = $MM_header . join(" ", @GROUP3_SECTIONS);
			foreach $each_section(@GROUP3_SECTIONS) {
				if (!(index($MM_header, $each_section . " ")>=0)) {
					$MM_header = $MM_header . $each_section . " ";
				}
			}
...
sub createMemoryMAP {
	...
	#push @returnlines, '<H4><FONT SIZE=+2 COLOR=#000000><A NAME=MemoryMAP>Memory MAP</A></FONT></H4>';
 	...
 	push @ENTIRE_LINES, @BSS_LINES;
 	push @ENTIRE_LINES, @GROUP1_LINES;
 	push @ENTIRE_LINES, @GROUP2_LINES;
 	push @ENTIRE_LINES, @GROUP3_LINES;
 	...
 	foreach $eachline(@ENTIRE_LINES) {
		...
		# without this line below, $last_end_addr will be treated as negative number
...
sub createJumpTable {
	...
	if ($MAPfile) {
		push @returnlines, '<LI><A HREF=' . "$ARGV[2]#MAPfile" . ' TARGET="rtwreport_document_frame">Original MAP file</A><BR></LI>';
	}
