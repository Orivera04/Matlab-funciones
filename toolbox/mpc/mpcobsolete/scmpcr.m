function [y,u,ym]=scmpcr(pmod,imod,ywt,uwt,blocks,...
                        p,tend,setpts,ulim,ylim, ...
                        Kest,ydist,mdist,umdist,udist,rant,vant)
%SCMPCR Simulate CL systems for constrained problems.
% 	[y,u,ym]=scmpcr(pmod,imod,ywt,uwt,M,P,tend, ...
%                 r,ulim,ylim,Kest,z,v,w,wu,rant,vant)
% SCMPCR designs an MPC-type controller for constrained problems
% and simulates the closed-loop system with hard constraints
% (inputs and outputs) using quadratic programming.
% NOTE: Assumes that at each sampling period, all future variations
%       of the setpoints are known, allowing MPC to respond in advance.
%       to setpoint changes.  See SCMPC for an alternative approach.
% Inputs:
...
%  rant: Vector, same length as r.  Set rant(i)=1 (true) if future
%        values of r(i) are to be assumed known.  Otherwise set
%        rant(i)=0.  This allows selective anticipation of setpoints.
%        Default is rant=ones(size(r)).
%  vant: Same purpose as rant, but for anticipation of the measured
%        disturbances, v.  If vant(i)=1, future of v(i) will be
%        assumed known.
...
if nargin == 0
   disp('USAGE:  [y,u,ym]=scmpcr(pmod,imod,ywt,uwt,M,P,tend, ...')
   disp('                       r,ulim,ylim,Kest,z,v,w,wu,rant,vant)')
   return
elseif nargin < 8
...
if nargin > 15
   if isempty(rant)
      ...
      [rows,cols]=size(rant);
      if length(rant(:))~=nyi
         error(['RANT dimensions are [',int2str(rows),',',int2str(cols),...
               '].  Should be [',int2str(nyi),', 1].']);
      end
   end
else
   rant=ones(nyi,1);
...
if nargin > 16
   if isempty(vant)
      ...
      [rows,cols]=size(vant);
      if length(vant(:))~=nvi
         error(['VANT dimensions are [',int2str(rows),',',int2str(cols),...
               '].  Should be [',int2str(nvi),', 1].']);
      end
   end
else
   vant=ones(nvi,1);
...
Hwait=waitbar(0,'SCMPCR Simulation in Progress ...');
...
% Save SS (LTI) model for use in simulation section
if nvi == 0
	Sys=ss(PHI,[],C,[],tsamp);
else
	Sys=ss(PHI,GAM(:,iv),C,D(:,iv),tsamp);
end
...
for i=1:nstep
   ...
   if nvi > 0
		% Define measured disturbances -- depending on whether or not
		% they are being anticipated.
		for iv=1:nvi
			if vant(iv)
				if i > nmd
					deltav(:,iv)=[mdist(nmd,iv);zeros(p,1)];
				elseif i+p > nmd
					deltav(:,iv)=[mdist(i:nmd,iv);zeros(i+p-nmd,1)];
				else
					deltav(:,iv)=mdist(i:i+p,iv);
				end
			else
				deltav(:,iv)=[mdist(min([i,nmd]),iv);zeros(p,1)];
			end
		end
   end
   if nwp > 0
...
% Define setpoints for prediction horizon, depending on setting of RANT

	for iy=1:nyi
		ii=[iy:nyi:pny];  % index vector
      if rant(iy)
			% Anticipate future setpoints for output iy
			if i+1 > nset
				setpt(ii,1)=setpts(nset,iy);
			elseif i+p > nset
				setpt(ii,1)=[setpts(i+1:nset,iy);...
								setpts(nset*ones(i+p-nset,1),iy)];
			else
				setpt(ii,1)=setpts(i+1:i+p,iy);
			end
		else
			% Use constant setpt for output iy
         setpt(ii,1)=setpts(min([i, nset]),iy);  
      end
   end
...
% Use LSIM to project the effect of current state and measured
% disturbances over the prediction horizon.
   y0=lsim(Sys,deltav,[],xi);

   y0=reshape(y0(2:end,:)',nyi*p,1);
   rhsa=a+SuTQ*(setpt-y0); % Starting basis for QP
