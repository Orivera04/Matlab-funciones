...
%   $Revision: 1.17.4.2 $  $Date: 2004/04/10 23:13:33 $
...
if Ts<0,
    ...
    if Ts,
        ...
        idx = find(r==1); % Case s=0;
        wn(idx) = 0;
        z(idx) = -1;
        idx = find(isfinite(r) & r~=0 & r~=1); % Case s isfinite and s~=0
		s = log(r(idx))/Ts;
	else
        idx = find(r == 0); % Case s=0
        wn(idx) = 0;
        z(idx) = -1;
		idx = find(isfinite(r) & r~=0 ); % Case s isfinite and s~=0
		s = r(idx);
	end
    f = abs(s);
	wn(idx) = f;
