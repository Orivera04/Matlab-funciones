function examplerwave
% This function creates an example wave plot
% for a rectangular membrane
clf
%[u,x,y,t]=membwave(1,[3,2,3*.25,1],1,12,2.5);
[u,x,y,t]=membwave(1,[2,1,.5,.5],1,8*pi,1.5);
colormap([1 1 0])
k=4; n=1+k*(0:8); ninesurfs(x,y,1.5*u(:,:,n)); 
m=size(u,3); clf
d=[min(x(:)), max(x(:)), min(y(:)), max(y(:)),...
   min(u(:)), max(u(:))]; 

for j=1:m
  surf(x,y,u(:,:,j)), xlabel(num2str(j))
  axis(d)
  axis equal
  colormap([1 1 0])
  colormap('default')
