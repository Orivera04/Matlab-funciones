...
%   $Revision: 1.9.2.7 $  $Date: 2004/01/16 20:01:15 $
...
for i=1:length(index1)
   if ~isnumeric(index1{i}) && (~(ischar(index1{i}) && (strcmp(index1{i}, ':'))))
       if ischar(index1{i})
           ...
           error('instrument:subsref:UndefinedFunction', ['Function ''subsindex'' is not defined for values of class ''' class(index1{i}) '''.']);
       end
   end
end
...
if any(cellfun('isempty', index1))
    for i = 1:length(index1)
        if ~isempty(index1{i}) && index1{i} > size(obj, i)
            error('instrument:subsref:exceedsdims', 'Index exceeds matrix dimensions.');
        end
    end
    result = [];
    return;
