function [out1,out2,savepts] = gstereo(mstruct,in1,in2,object,direction,savepts)

%GSTEREO  Gall Stereographic Cylindrical Projection
%
%  This is a perspective projection from a point on the Equator
%  opposite a given meridian onto a cylinder secant at the 45 degree
...
%
%  This projection was presented by James Gall in 1855.  It is also
%  known simply as the Gall projection.  It is special form of the
%  Braun Perspective Cylindrical projection secant at 45 deg N and S.
%
...
%  Projection transformation
	 ...
	 x = geoid(1) * long / sqrt(2);
     y = geoid(1) * ( 1 + sqrt(2)/2) * tan(lat/2);
...
% Inverse projection
     ...
     lat  = 2* atan(y/(geoid(1)*(1+sqrt(2)/2)) );
	 long = sqrt(2) * x / geoid(1);
