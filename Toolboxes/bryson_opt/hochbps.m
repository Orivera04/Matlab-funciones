% Script hochbps.m; hom. chauf. bang-bang soln.; finds (tb,tf)
% given (xe0,ye0,ps;w,el) for min tf;                   3/8/02
%
tic; w=1/3; el=1/4; xe0=2.5; ye0=0; ps=4.9482-.05; X0=[.72 6];
optn=optimset('display','iter','maxiter',50,'TolFun',1e-9);
X=fsolve('hochbps_f',X0,optn,w,el,xe0,ye0,ps);
tb=X(1); tf=X(2); cp=cos(ps); sp=sin(ps); 
[f,xf,yf,xef,yef,tf,thf]=hochbps_f(X,w,el,xe0,ye0,ps);
...
figure(1); clf; plot(xe0,ye0,'go',xf,yf,'bo',-1+cb,sb,'bo',...
   ...
   'b--',[xf xef],[yf yef],'g--'); grid; axis([-1 3 -1.5 2.5]);
...
plot(x,y,'b.'); axis([-.5 3.5 -2.5 1.5]); axis('square');
hold off
toc