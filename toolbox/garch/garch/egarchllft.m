function [LLF, G, H, e, h, LogLikelihoods] = egarchllft(Parameters, y, R, M, P, Q, X, isConstantInMean, isVarianceInMean, e0, s0, y0)
%EGARCHLLFT Univariate EGARCH process objective function (Student T Distribution).
...
%   conditional variances are then fit to an EGARCH model. Student T-distributed
%   innovations are assumed.
%
%   [LLF, G, H, Innovations, Sigmas, LogLikelihoods] = egarchllft(Parameters, 
...
%     logarithm of conditional variance included in the EGARCH process. This
%     is the number of 'GARCH' terms in the EGARCH variance equation.
%
%   Q - Non-negative, scalar integer representing the number of lags associated 
%     with the standardized innovations of the EGARCH process. This is the number
%     of 'ARCH' and 'Leverage' terms in the EGARCH variance equation.
%
...
%     variance-in-mean functionality is added to EGARCH models. This input 
%     simply allows all objective functions to share a common calling syntax.
%
% Optional Inputs:
...
%   $Revision: 1.1.4.1 $   $Date: 2003/05/08 21:45:18 $
...
%
%     z(t) = standardized i.i.d. innovations noise process = e(t)/sqrt(h(t))     
%
...
% solving for the current values of the y(t) and log[h(t)] time series. 
%
% Consider the following general ARMAX(R,M,nX)/EGARCH(P,Q) form:
%
...
% expected value. For T-distributed innovations, 
%
% E(|z(t-i)|) =  sqrt((DoF - 2) / pi) * gamma(0.5 * (DoF - 1)) / gamma(0.5 * DoF)
%
...
% and variance of an ARMAX(R=2,M=2,nX=1)/EGARCH(P=2,Q=2) composite model:
%
%   y(t) =  1.3 + 0.5y(t-1) - 0.8y(t-2) + e(t) - 0.6e(t-1) + 0.08e(t-2) + 1.2X(t)
%
%   log[h(t)] =  0.5 + 0.2log[h(t-1)]               + 0.1log[h(t-2)] 
%                    + 0.1[|z(t-1)| - E(|z(t-1)|)]  + 0.2[|z(t-2)| - E(|z(t-2)|)] 
%                    + 0.1z(t-1)                    + 0.1z(t-2)
%
...
% is not estimated.
%
% Now consider another ARMAX(R=2,M=2,nX=1)/EGARCH(P=2,Q=2) composite model:
%
...
%
% Compute the mean absolute deviation (MAD). This is the unconditional (i.e., 
% time-independent) expected value of the absolute magnitude of the standardized
% innovations. Note that this is a scalar parameter dependent upon the distribution.
%
...
%       bypass the calculation and simply replace the T-distributed mean
%       absolute deviation by the asymptotic Gaussian limit. The allowable
...
%
   [e , h]  =  egarchllf(R , M , P , Q , [repmat(mean(y),maxRMPQ,1) ; y], armaCoefficients    , ...
...
%
   [e , h]  =  egarchllf(R , M , P , Q , [y0 ; y]           , armaCoefficients    , ...
                                         regressCoefficients, varianceCoefficients, ...
                                         X                  , MAD                 , ...
                                         e0                 , s0.^2);
end
