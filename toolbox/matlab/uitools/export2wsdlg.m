function export2wsdlg(checkboxLabels, defaultVariableNames, itemsToExport, varargin)
...
%   $Revision: 1.1.6.4 $  $Date: 2004/04/10 23:33:35 $

if (~iscell(checkboxLabels) || ~iscell(defaultVariableNames) || ...
    ~iscell(itemsToExport))
    error(['CHECKBOXLABELS, DEFAULTVARIABLENAMES and '...
...
if length(unique(defaultVariableNames)) ~= checkBoxCount
    error('Default variable names must be unique');
end
...
if (nargin > 4)
    ...
    if ~islogical(selected)
        error('SELECTED must be a logical array');
    elseif length(selected) ~= checkBoxCount
        error('SELECTED must be the same length as ITEMSTOEXPORT');
    end
else
...
if (nargin > 6)     % export functions
    ...
    if length(functionlist) ~= checkBoxCount
        error('FUNCTIONLIST must be the same length as ITEMSTOEXPORT');
    end
else
