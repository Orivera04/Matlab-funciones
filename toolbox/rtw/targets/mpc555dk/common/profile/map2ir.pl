...
local(@BSS);		# name lists
local(@GROUP1);		# name lists
local(@GROUP2);		# name lists
local(@GROUP3);		# name lists
...
sub readRuleFile {
	...
	while(<$FILE_HANDLE>) {
		...
		/^BSS\s+/ and @BSS = @comma_deli_words;
		/^GROUP1\s+/ and @GROUP1 = @comma_deli_words;
		/^GROUP2\s+/ and @GROUP2 = @comma_deli_words;
		/^GROUP3\s+/ and @GROUP3 = @comma_deli_words;
...
sub groupSections {
	next1: foreach $eachline (@format_KEY_LINES) {
		...
		foreach $eachname(@BSS) {
			if ($linearray[0] =~ /^$eachname$/) {
				push @BSS_LINES, $eachline;
				$thislineisknown = 1;
				#next next1;
			}
		}
		foreach $eachname(@GROUP1) {
			if ($linearray[0] =~ /^$eachname$/) {
				push @GROUP1_LINES, $eachline;
				$thislineisknown = 1;
				#next next1;
			}
		}
		foreach $eachname(@GROUP2) {
			if ($linearray[0] =~ /^$eachname$/) {
				push @GROUP2_LINES, $eachline;
				$thislineisknown = 1;
				#next next1;
			}
		}
		foreach $eachname(@GROUP3) {
			if ($linearray[0] =~ /^$eachname$/) {
				push @GROUP3_LINES, $eachline;
				$thislineisknown = 1;
				#next next1;
			}
		}								
...
sub writeIRFile {
	...
	print($FILE_HANDLE "BSS end\n");
	
	#GROUP1 sections
	$entire_size = 0;
	print($FILE_HANDLE "GROUP1 begin\n");
	#print($FILE_HANDLE "section name, start address(dec), section size(dec), end address(dec)\n");
	...
	#print($FILE_HANDLE " entire section size:,, " , "$entire_size\n");
	print($FILE_HANDLE "GROUP1 end\n");

	#GROUP2 sections
	$entire_size = 0;
	print($FILE_HANDLE "GROUP2 begin\n");
	#print($FILE_HANDLE "section name, start address(dec), section size(dec), end address(dec)\n");
	...
	#print($FILE_HANDLE " entire section size:,, " , "$entire_size\n");
	print($FILE_HANDLE "GROUP2 end\n");

	#GROUP2 sections
	$entire_size = 0;
	print($FILE_HANDLE "GROUP3 begin\n");
	#print($FILE_HANDLE "section name, start address(dec), section size(dec), end address(dec)\n");
	...
	while(<$FILE_HANDLE>) {
		...
		foreach $each_file(@current_line) {
			foreach $each_extension(@ModelFileExtension) {
				push @Gen_File_list, stripFileExtension($each_file) . "." . $each_extension;
			}
		}
	}
	if ($EchoDebugInfo) {
...
# translate c:\mydir\abc.efg into abc.efg
sub stripFilePath {
	my($fully_qualified_fileName) = @_;
	local($shortFileName) = $fully_qualified_fileName;
	$indexParenth = rindex($shortFileName, "\\");
	if ($indexParenth > 0) {
		$shortFileName = substr($shortFileName, $indexParenth+1, length($shortFileName));
	}
...
# translate abc.efg into abc
sub stripFileExtension {
	my($fully_qualified_fileName) = @_;
	local($shortFileName) = $fully_qualified_fileName;
	$indexParenth = rindex($shortFileName, "\.");
	if ($indexParenth > 0) {
		$shortFileName = substr($shortFileName, 0, $indexParenth);
	}
	return $shortFileName;
}
...
sub searchRtwlibSymbols {
	for ($i=0; $i<=$#SL_lib; $i++) {
		if (isInList(stripFilePath($SL_lib[$i]), RTW_Lib_list)) {
