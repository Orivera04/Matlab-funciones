...
% $Revision: 1.1.6.12 $ $Author: batserve $ $Date: 2004/04/14 23:57:15 $
...
% initial drawing
fig = figure('Renderer','ZBuffer','Name','Phylogenetic Tree Tool',...
           'NumberTitle','off','IntegerHandle','off');
tr.ha = axes; hold on;
set(tr.ha,'Position',[.05 .05 .7 .9],'YTick',leafIndex,'FontSize',9,...
...
switch tr.editMode
    ...
    %case 'Pa&n';      toolsmenufcn(gcbf,'Pan');
    case 'Pa&n';      pan(gcbf,'off');
...
switch get(h,'Type')
    ...
    case 'uitoggletool'
        ...
        switch newEditMode
            ...
            case 'Exploration.Pan';     newEditMode = 'Pa&n';
        end
...
elseif numel(hp) % && isempty(tr.highligth)
   ...
   childrenNames = char(flipud(tr.names(children(1:tr.numLeaves))));
   ...
   fp = get(gcbf,'Position')*[0;0;0;1]; % fig size in points
   ap = get(tr.ha,'Position');          % axis position normalized
   yl = ylim(tr.ha);
   yl = yl - [ap(2) ap(2)+ap(4)-1]*diff(yl)/ap(4); % fig size in axis units
   yPosPt = (yPos - yl(1))*fp/diff(yl); % datatip position in pts
   ...
   %adjust position of datatip
   topBox = yl(1)+min(fp,max(yPosPt,reqPt))*diff(yl)/fp;
   switch tr.editMode
       case {'Collapse/Expand','Rotate Branch','Prune'} 
           set(tr.datatip,'Position',[xPos-3*xThres,topBox,1])
           set(tr.datatip,'horizontalalignment','right')
...
hp = find(tr.x<max(xdata) & tr.x>min(xdata) & ...
...
if numel(hp)
    ...
    yBoxPos = (.02+(yPos-Ylim(1))/diff(Ylim))*aPos(4)+aPos(2);
...
while numel(hp)
    ...
    permuta = 1:tr.numLabels;
    permuta(children) = [];
    ipermuta = zeros(1,length(permuta));
...
va = [va{:}];
switch find(va(1:3))
    case 1; args = {'type','phy'};
    case 2; args = {'type','cla'};
    case 3; args = {'type','rad'};
end
...
% rather then erase and redraw, we only change specific fields in hlines
% and hdots.
...
hf = get(hw,'children');
h1 = findall(hw,'Label','Pa&ge Setup...');
h2 = findall(hw,'Label','Print Set&up...');
...
uimenu(hw,'Label','Open from Workspace','Position',3,'Callback',@importfromwsdlg)
uimenu(hw,'Label','Save',       'Position',4,'Callback',@saveNewick,'Separator','on')
item0 = uimenu(hw ,'Label','Publish to Figure','Position',5);
    uimenu(item0,'Label','With Hidden Nodes','Callback',{@publishdlg,1,0});
    uimenu(item0,'Label','Only Displayed',   'Callback',{@publishdlg,0,0});
item1 = uimenu(hw,'Label','Export to New Tool','Position',6);
...
item2 = uimenu(hw,'Label','Export to Workspace','Position',7);
...
hf = get(hw,'children');
h1 = findall(hw,'Label','&Zoom In');    set(h1,'Callback',{@changeEditMode,gcbo});
h2 = findall(hw,'Label','Zoom &Out');   set(h2,'Callback',{@changeEditMode,gcbo});
h3 = findall(hw,'Label','Pa&n');        set(h3,'Callback',{@changeEditMode,gcbo});
h4 = findall(hw,'Label','Reset View');  set(h4,'Callback',{@myResetView,gcbo});
h5 = findall(hw,'Label','Options');
set([h1,h4],'separator','off')
delete(setxor(hf,[h1,h2,h3,h4,h5]))
delete(findall(h5,'Label','Display Cursor as Datatip'))
delete(findall(h5,'Label','Display Cursor in Window'))
...
uimenu(hw,'Label','Phylogenetic Tree Tool Help','Position',2,'Callback',...
       ['helpview(fullfile(docroot,''mapfiles'',''bioinfo_ug.map'')' ...
...
item0 = uimenu(hcm2,'Label','Publish to Figure','Separator','on');   
...
fig = figure('WindowStyle','modal','Color',[0.831373 0.815686 0.784314],...
             ...
             'Name','Publish Phylogenetic Tree to Figure','IntegerHandle','off' );
h1=uibuttongroup;h2=uibuttongroup;
set(h1,'Position',[.08 .35 .35 .55],'Title','Rendering Type')
set(h2,'Position',[.52 .35 .39 .55],'Title','Display Labels')
ui1=uicontrol(h1,'style','radiobutton','Position',[5 70 90 20],'string','Dendrogram','value',1);
ui2=uicontrol(h1,'style','radiobutton','Position',[5 40 90 20],'string','Cladogram');
ui3=uicontrol(h1,'style','radiobutton','Position',[5 10 90 20],'string','Radial Tree');
ui4=uicontrol(h2,'style','checkbox','Position',[5 70 109 20],'string','Branch Nodes');
ui5=uicontrol(h2,'style','checkbox','Position',[5 40 109 20],'string','Leaf Nodes');
ui6=uicontrol(h2,'style','checkbox','Position',[5 10 109 20],'string','Terminal Nodes','value',1);
uicontrol(fig,'style','pushbutton','Position',[70 20 60 30],'string','Publish','Callback',{@doPublishFigure,AllNodes,callerIsContextMenu});
...
uicontrol(fig,'style','text','Position',[20 190 140 20],'string','Select phytree object:','Horizontal','left')
