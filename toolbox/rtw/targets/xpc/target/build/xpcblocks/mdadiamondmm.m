function [range, reset, initValue, baseDec, maskDisplay, maskDescription] = mdadiamondmm(phase, channel, range, reset, initValue, sampleTime, base);

% Mask Initialization function for Diamond Systems MM D/A driver blocks
% Copyright 1996-2002 The MathWorks, Inc.
% $Revision: 1.3 $  $Date: 2002/11/25 20:10:32 $  

    vendorName   = 'Diamond';
    deviceName   = 'MM';
    description  = 'Analog Output';
    maskType     = 'dadiamondmm';
    resolution   = 4096;
    maxChan      = 2;
	...
	if ~isa(range, 'double') || any(range <= 0)
      error('Range vector elements must be of class double and > 0');
	end
	if all(size(range) == [1 1])
      range = range * ones(size(channel));
	elseif ~all(size(range) == size(channel))
      error('Range must be a scalar or have the same number of elements as the Channel vector');
	end
	...
	if ~isa(initValue, 'double') 
      error('Initial value vector entries must be of class double');
	end
