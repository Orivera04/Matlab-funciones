% Script p2_7_23b.m; max altitude climb in given xf, A/C w. parabolic
% lift-drag polar using FOP0; s=[V,gamma,h]'; u=alpha; h in l, V in 
% sqrt(g*l), l=2m/(rho*S*Cla); alm=1/12; eta=1/2; T=.2; Vo=Vf=7, ga0=
% gaf=0; indpt. variable=horiz. distance x;             2/97, 8/10/02
%
xf=120; s0=[5 0 0]'; load p2_7_23b; k=-3e-5; told=1e-5; tols=5e-5;
mxit=10; c=180/pi; name='climbx0'; al0=al0/c;
[x1,al,s]=fop0(name,tu,al0,xf,s0,k,told,tols,mxit);
V=s(:,1); ga=c*s(:,2); h=s(:,3); al=c*al; N1=length(x1); 
es=3.464^2/2;  
%
figure(1); clf; plot(V.^2/2,h,[es 12.5],[6.50 0],'r--',[es 12.5],...
   [20.57 14.07],'r--',[es es],[20.57 6.50],'r--',12.5,0,'bo',...
   V(N1)^2/2,h(N1),'bo'); grid; axis([0 22 0 22]); axis('square')
