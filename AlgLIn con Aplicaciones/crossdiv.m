...
%   The definition of DCDs is based on that of ICDs. The symbol for an ICD 
...
% $ Version: 1.2 $
% CODE      by:                 Paolo de Leva (IUSM, Rome, IT) 2005 Aug 18
% COMMENTS  by:                 Code author                    2005 Sep 27
% OUTPUT    tested by:          Code author                    2005 Aug 18
% -------------------------------------------------------------------------
...
if nargin == 2
   first3 = min( find(size(b)==3) );  % First dimension of length 3
...
x = cross(b, c, dim);      % Vector(s) aligned with Anorm
...
basize = [ones(1,dim-1), N];        % Size of block array
