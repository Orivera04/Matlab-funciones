% Script p3_5_09b.m; DTDP for max uf with gravity and (vf,yf) spec.
% using DOPCB; s=[u,v,y,x]'; t in tf, (u,v) in a*tf, (x,y) in 
% a*tf^2, g in a;                                     3/97, 5/27/02
%
N=50; s0=[0 0 0 0]'; tf=1; sf=[.486 0 .2 .198]; nu=[-2.871 8.737];
p0=[sf nu]; t=[0:N]/N; name='dtdpgc'; thf=-1.2;  
optn=optimset('Display','Iter','MaxIter',500);
p=fsolve('dopcb',p0,optn,name,thf,s0,tf,N);
[f,s,th]=dopcb(p,name,thf,s0,tf,N); c=180/pi; thh=c*[th th(N)];
u=s(1,:); v=s(2,:); x=s(4,:); y=s(3,:); 
%
...
grid; axis([0 1 0 .5]); xlabel('t/t_f')
