...
%   $Revision: 1.6 $  $Date: 2003/04/25 21:12:14 $

[v,d] = fnbrk(f,'var','dim');
if v>1
   error('SPLINES:FNMIN:onlyuni','This only works for univariate functions.')
end
if length(d)>1||d>1
   error('SPLINES:FNMIN:onlyscalar',...
   'This only works for scalar-valued functions.')
end
