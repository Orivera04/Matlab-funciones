function [formatsend, formatrec, dtypeout, maskdisplay, maskdescription, inPorts, outPorts, numInPorts, numOutPorts, formatSendInfo, formatRecInfo, WaitArg, numEOM, TimeoutArg] = mnigpib(flag, block, direct, gpibid)

% MNIGPIB - InitFcn and Mask Initialization for NI GPIB-232CT-A

% Copyright 1996-2002 The MathWorks, Inc.
% $Revision: 1.5 $ $Date: 2002/03/25 04:10:25 $
...
% General
if ~isfield(block,'Address')
  block(1).Address=[];
end
if ~isfield(block,'RdLength')
  block(1).RdLength=[];
end
if ~isfield(block,'Command')
  block(1).Command=[];
end
...
if ~isfield(block,'EOM')
  ...
  if (gpibid > 0)
    addrmsg.Command = ['caddr ',num2str(gpibid),'\r'];	  
    addrmsg.SendData='';
	addrmsg.Address=[];
    addrmsg.RdLength=[];
	addrmsg.Ack = '';
    addrmsg.Timeout=0.049;
    addrmsg.InputPorts=[];
    addrmsg.RecData='';
    addrmsg.OutputPorts=[];
    addrmsg.OutputDataTypes=[];
    addrmsg.Wait=0;
    addrmsg.EOM=2;
...
for i = 1:numblocks
  if isempty(block(i).Command) & isa(block(i).Command,'double')
    block(i).Command=''; % default
  end
  ...
  if strcmp(block(i).Command,'rd')
	block(i).Command = 'rd #LENGTH ADDR';
  end
  if strcmp(block(i).Command,'wrt')
	block(i).Command = 'wrt ADDR';
  end
  if isempty(block(i).Wait)
    block(i).Wait=0; % default
  end

  if ~isempty(block(i).Address)
    currentAddress=block(i).Address;
  else
    block(i).Address = currentAddress;
  end
  if ~isempty(block(i).RdLength)
    currentlength = block(i).RdLength;
  else
    block(i).RdLength = currentlength;
  end
  %if isempty(block(i).Timeout)
  %  block(i).Timeout=0; % default
  %end
  if isempty(block(i).EOM)
    block(i).EOM=2; % default (normally 1 for rs232 but usually 2 for gpib)
  end
...
for i = 1:numblocks
  ...
  %Add Gpib Start Message
  convString = '';
  outString = '';

  %Construct send message
  %sendpreamble
  if ~isempty(block(i).Command)
	convString = block(i).Command;
  end
  ...
  if length(convString) > 2
    if ~ismember(convString(length(convString)-1:length(convString)),tokens) 
      if ~isempty(findstr('wrt',convString))
	    convString = [convString,sprintf('\n')];
      end
	end
  end
  ...
  if length(convString) > 2
    if ~ismember(convString(length(convString)-1:length(convString)),tokens) 
      if ~isempty(convString)
	    convString = [convString,sprintf('\r')];
	  end
    end
  end

  if ~isempty(findstr(convString,'ADDR')) & isempty(block(i).Address)
	error('Address is not defined')
  end
  if ~isempty(findstr(convString,'LENGTH')) & isempty(block(i).RdLength)
	error('Length is not defined')
  end
  
  convString = strrep(convString,'ADDR',num2str(block(i).Address));
  convString = strrep(convString,'LENGTH',num2str(block(i).RdLength));

  if length(block(i).OutputDataTypes) > length(block(i).OutputPorts)
	error('The number of output data types in a block cannot exceed the number of output ports');
  end
  ...
  if ismember(direct,[4]);
	  ...
	  convString = strrep(convString,'GPIBID',num2str(gpibid));
  end
  ...
  if length(block(i).OutputDataTypes) ~= length(find(block(i).OutputPorts>0))
	error('The number of output data types must be empty (default type double assumed), a scalar (scalar expension applies) or a row vector with the same number of element as the non negative output ports');
  end
  ...
  if ismember(direct,[4]);
    if ~isempty(block(i).Ack)
      ...
      if length(outString) > 2
        if ~ismember(outString(length(outString)-1:length(outString)),tokens) 
		  outString = [outString,sprintf('\r'),sprintf('\n')];
		end
	  end
    end

    if ~isempty(findstr(outString,'ADDR')) & isempty(block(i).Address)
	  error('Address is not defined')
    end
    if ~isempty(findstr(outString,'LENGTH')) & isempty(block(i).RdLength)
	  error('Length is not defined')
    end

    outString = strrep(outString,'ADDR',num2str(block(i).Address));
    outString = strrep(outString,'LENGTH',num2str(block(i).RdLength));
	outString = strrep(outString,'GPIBID',num2str(gpibid));
	...
	if ~isempty(findstr('rd',convString))
      outString = [outString,'%d',sprintf('\r'),sprintf('\n')];
	  block(i).OutputPorts = [block(i).OutputPorts,-1];
	end
...
maskdisplay='disp(''GPIB-232CT-A\nNational Instruments\n';

description=['GPIB-232CT-A',10,'National Instruments',10];
