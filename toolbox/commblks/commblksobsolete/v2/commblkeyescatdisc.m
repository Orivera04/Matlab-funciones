function varargout = commblkeyescatdisc(block, action, varargin)
% COMMBLKEYESCATDISC Eye/Scatter Scope dynamic dialog helping function
% Copyright 1996-2002 The MathWorks, Inc.
%   $Revision: 1.1.6.1 $  $Date: 2003/01/26 18:45:48 $
...
function retDiagType = cbDiagram_type(block)
    ...
    prevEn  = En([idxEye_line idxScatter_line]);
    prevVis = Vis([idxEye_line idxScatter_line]);
   ...
   % in new values for the Enable or visibility variables
    if ( ~isequal(prevEn,  En([idxEye_line idxScatter_line])) | ...
         ~isequal(prevVis, Vis([idxEye_line idxScatter_line])) )
