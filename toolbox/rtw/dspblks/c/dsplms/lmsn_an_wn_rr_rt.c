/**********************/  
/* 
 * lmsn_an_wn_rr_rt.c - Signal Processing Blockset Normalized LMS adaptive filter run-time function 
 * 
 * Specifications: 
 * 
 ...
 *  $Revision: 1.1.6.1 $Date: 2003/12/06 15:57:20 $ 
 */  
#include "dsp_rt.h"
#include "dsplms_rt.h" 

EXPORT_FCN   void MWDSP_lmsn_an_wn_RR(   const real32_T *inSigU, 
...
for (i=0; i<FrmLen; i++) 
{
  ...
  /* Step-4:Update wgtIC for next input sample */  
       ...
       for (j=buffStartIdx[0]; j < FilterLength; j++) 
          {*wgtBuff = *errY * ((*(inBuff + j))/ (*enrgInBuff + FLT_EPSILON)) *muU + LkgFactor* (*wgtBuff);wgtBuff++;}
