...
%------------------------------------------------------------
% ZOOM CHANGED BY HANS VEREECKEN 23/03/2001
% BASF Antwerp ; Belgium
% 003235613411
% ZOOMING OCCURS ON ALL THE X-AXES IN THAT FIGURE
% ESPECIALLY IMPORTANT FOR ZOOMING IN SUBPLOTS AND PLOTYY's

% Zooming on all axes works only on the x-axes.

% This function uses a memory persistent variable
persistent zoom_on_all;
% 1 = zoom on all axes of the current figure
% 0 = zooms only on the current axis = normal behaviour of zoom
% This variable can be toggled on and off
% zoom one  = zoom only on the current axis
%           = default in the normal zoom function, but NOT in this one
% zoom all  = zoom on all axes
%           = made the default
...
%------------------------------------------------
% Inserted by Hans Vereecken 23/03/2001
% to toggle zooming on all axes or on one axis
case 'one',
   zoom_on_all = 0; % zoom only on one axis
   return
case 'all',
   zoom_on_all = 1; % zoom on all axes
   return
% End new statements   
%--------------------------------------------------   
...
% BEGIN NEW STATEMENTS

if isempty(zoom_on_all), % no toggle statement used until now --> default = zoom on all axes
   allAxes = findobj(datachildren(fig),'flat','type','axes');
   % find all axes on the current figure
   set(allAxes,'xlim',a(1:2));
   % This command changes the xlimits of ALL the axes in that figure
   set(allAxes,'YLimMode','auto'); set(allAxes,'YTickMode','auto');
   % --> auto-update of all the y-axes
else % toggle functions (zoom one or zoom all) already used
      if zoom_on_all ==1,
          allAxes = findobj(datachildren(fig),'flat','type','axes');
			 % find all axes on the current figure
   		 set(allAxes,'xlim',a(1:2));
...
% OLD STATEMENT
% set(ax,'xlim',a(1:2))
% This command just changes the axis your have clicked on
% END OLD STATEMENT
%------------------------------------------------------------------
