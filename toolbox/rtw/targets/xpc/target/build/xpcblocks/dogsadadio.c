/* $Revision: 1.14.6.1 $ $Date: 2003/11/20 11:57:31 $ */
/* dogsadadio.c -- Digital output module for the General Standards PMC-ADADIO */
/* multifunction board. */
/* Copyright 1994-2003 The MathWorks, Inc. */

#define 	S_FUNCTION_LEVEL 	2
#undef 		S_FUNCTION_NAME
#define 	S_FUNCTION_NAME 	dogsadadio
...
#define IN_ENABLE_ARG           ssGetSFcnParam(S,1)
#define OUT_ENABLE_ARG          ssGetSFcnParam(S,2)
#define SAMP_TIME_ARG           ssGetSFcnParam(S,3)
#define SLOT_ARG                ssGetSFcnParam(S,4)
#define BCRINIT_ARG             ssGetSFcnParam(S,5)
#define INIT_ARG                ssGetSFcnParam(S,6)
#define CAL_ARG                 ssGetSFcnParam(S,7)
#define DACHANS_ARG             ssGetSFcnParam(S,8)
#define DARESET_ARG             ssGetSFcnParam(S,9)
#define DAIVAL_ARG              ssGetSFcnParam(S,10)
#define DOCHANS_ARG             ssGetSFcnParam(S,11)
#define DORESET_ARG             ssGetSFcnParam(S,12)
#define DOIVAL_ARG              ssGetSFcnParam(S,13)
...
static void mdlStart(SimStruct *S)
{
    ...
    //printf("ADADIO dig out start init\n");
    ...
    if ((int_T)mxGetPr(INIT_ARG)[0])
    {
        ...
        while( temp & BOARD_RESET )
            readLongIO( BCR, temp );
        writeLongIO( BCR, (uint32_T)mxGetPr(BCRINIT_ARG)[0] );
        readLongIO( BCR, temp );
    ...
    ///printf("ADADIO dig out DO values\n");
    // This is the digital output block.  Set the digital direction.
    // Set the outputs to the values in the digital output initial value vector
    {
        int ndochans = (int_T)mxGetN(CHANNEL_ARG);
        int output = 0;
        //printf("ndochans = %d\n", ndochans );
        for( i = 0 ; i < ndochans ; i++ )
        {
            int chan = (int_T)mxGetPr(CHANNEL_ARG)[i] - 1;
            // Make sure only the LSB of ival is used.
            int ival = 1 & (int_T)mxGetPr(DOIVAL_ARG)[i];
            //printf("chan = %d, ival = %d\n", chan, ival );
            if( ival == 1 )
                output |= 1 << chan;
        }
        writeLongIO( DIGIO, output | DIGITAL_DIR_OUTPUT );
        readLongIO( BCR, temp );
    }
    //printf("ADADIO dig out init end\n");
#endif
}
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    /* write data to DO register */
    if( enabled )
    {
        output = 0;
        for (i = first_chan ; i < mxGetN(CHANNEL_ARG) + first_chan ; i++ )
        {
            uPtr = (real_T *)ssGetInputPortSignal(S,i);
            channel = (int_T)mxGetPr(CHANNEL_ARG)[i-first_chan] - 1;
            if (uPtr[0] >= THRESHOLD) {
                output |= 1 << (channel);
            } else {
                output &= ~(1 << (channel));
        ...
        output |= DIGITAL_DIR_OUTPUT;
        writeLongIO( DIGIO, output );
        readLongIO( BCR, output );
    }
#endif
}
...
{   
    ...
    // This is the digital output block.
    // Reset the outputs to the values in the digital output
    // initial value vector
    {
        int ndochans = (int_T)mxGetN(CHANNEL_ARG);
        int output;

        readLongIO( DIGIO, output );
        for( i = 0 ; i < ndochans ; i++ )
        {
            int chan = (int_T)mxGetPr(CHANNEL_ARG)[i] - 1;
            int reset = (int_T)mxGetPr(DORESET_ARG)[i];
            // Make sure only the LSB of ival is used.
            int ival = 1 & (int_T)mxGetPr(DOIVAL_ARG)[i];
            if( reset == 1 )
            {
