function [X, map, alpha] = readras(filename)
%READRAS Read image data from a RAS file.
%
%   [X,MAP,ALPHA] = READRAS(FILENAME) reads image data from the specified
%   file.  X is always a uint8 array, MAP is always a double array and ALPHA
%   is always a uint8 array.
%
%   For bitmap images, X is an M-by-N matrix and MAP and ALPHA are empty.
%   These are stored with 1 bit per pixel.
%
%   For indexed images, X is an M-by-N matrix, MAP is a K-by-3 matrix and
%   ALPHA is empty.  These are stored with 8 bits per index value, 8 bits
%   per color component and 1 or 3 color components per pixel.
%
...
%
%   For true color images with alpha channel, X is an M-by-N-by-3 array, MAP
%   is empty and ALPHA is an M-by-N matrix.  These are stored with 8 bits
%   per color component, 4 color components per pixel.
%
...
%  $Revision: 1.1.6.2 $  $Date: 2004/02/01 22:04:52 $

   info = imrasinfo(filename);
   ...
   if maplength
      ...
      if count < maplength
         fclose(fid);
         error('MATLAB:readras:unexpectedEOF', ...
               [filename ': file ended while reading color map.']);
      end

      % Convert the color map to MATLAB type color map.
      switch info.MapType
         case RMT_NONE
            warning('MATLAB:readras:badMapLength', ...
                    [filename ': maplength should be 0 when maptype is 0.']);
         case RMT_EQUAL_RGB
         ...
         case RMT_RAW
            ...
            map = map(:,[1 1 1])/255;
         otherwise
            error('MATLAB:readras:badMapType', ...
                  [filename ': invalid RAS maptype.']);
      end
   ...
   switch info.BitDepth
      ...
      case 32
         ...
         numBytes = paddedByteWidth * height;
      otherwise
         error('MATLAB:readras:badBitDepth', ...
   ...
   if info.Type ~= RT_BYTE_ENCODED && datalength ~= numBytes
      ...
      warning('MATLAB:readras:badDataLength', ...
              [filename ': data length value is not as expected.' ...
   ...
   if count < datalength
      warning('MATLAB:readras:unexpectedEOF', ...
   ...
   if fsize > headersize + maplength + datalength
      warning('MATLAB:readras:tooMuchData', ...
              [filename ': too much image data. Possibly corrupt image.']);
   end
   ...
   switch info.Type
      ...
      case RT_FORMAT_TIFF
         error('MATLAB:readras:tiffRasNotSupported', ...
               [filename ': tiff RAS type is not supported.']);
      case RT_FORMAT_IFF
         error('MATLAB:readras:iffRasNotSupported', ...
               [filename ': iff RAS type is not supported.']);
      case RT_EXPERIMENTAL
         error('MATLAB:readras:experimentalRasNotSupported', ...
               [filename ': experimental RAS type is not supported.']);
      otherwise
         error('MATLAB:readras:invalidRasType', ...
               [filename ': invalid RAS type.']);
   end
