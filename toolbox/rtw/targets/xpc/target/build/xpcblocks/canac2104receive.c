/* $Revision: 1.8.6.1 $ $Date: 2004/04/08 21:01:40 $*/
...
#define         S_FUNCTION_NAME         canac2104receive
        ...
        for (i=0;i<mxGetN(CAN_INDEX_ARG);i++) {
            ...
            switch ((int_T)mxGetPr(BOARD_ARG)[0]) {
              case 1:
                ...
                break;
              case 2:
                frc = CANPC_read_rcv_data_mb2(index, p_data, &time);
                break;
              case 3:
                frc = CANPC_read_rcv_data_mb3(index, p_data, &time);
                break;
            }
			_asm{popf};
            if (frc>0 || frc==-1 || frc==-2) {
        ...
        for (i=0;i<mxGetN(CAN_INDEX_ARG);i++) {
            ...
            switch ((int_T)mxGetPr(BOARD_ARG)[0]) {
              case 1:
                ...
                break;
              case 2:
                frc = CANPC_read_rcv_data2_mb2(index, p_data, &time);
                break;
              case 3:
                frc = CANPC_read_rcv_data2_mb3(index, p_data, &time);
                break;
            }
			_asm{popf};
            if (frc>0 || frc==-1 || frc==-2) {
