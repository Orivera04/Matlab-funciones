...
% $Revision: 1.1.6.4 $ $Date: 2004/04/11 00:37:59 $

% Get handles of associated packages and classes
hCreateInPackage = findpackage('ParameterEstimatorData');

% Construct class
c = schema.class(hCreateInPackage, 'TransientExperiment');

% Proxy to ParameterEstimator.TransientExperiment properties
schema.prop(c, 'Model',         'string');
schema.prop(c, 'InputData',     'handle vector');
schema.prop(c, 'OutputData',    'handle vector');
schema.prop(c, 'InitialStates', 'handle vector');
