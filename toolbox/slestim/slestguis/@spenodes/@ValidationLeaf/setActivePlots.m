...
% $Revision: 1.1.6.4 $ $Date: 2004/04/19 01:33:07 $
...
hExps = this.Experiment; % Numerical experiment object
...
if isempty(hExps) || isempty(hEsts)
  ...
  uiwait( warndlg( warnmsg, 'Validation Plot Warning', 'modal') );
end

% Get copy of estimation objects
Estimations = handle( NaN(size(hEsts)) );
for ct = 1:length(hEsts)
  Estimations(ct) = copy( hEsts(ct).Estimation );
  Estimations(ct).Experiment = hExps;
end
...
% ATTN: To trigger creating a new plot
