function [sys,x0,str,ts] = saeroanim3dof(t,x,u,flag,Config)
%SAEROANIM3DOF S-Function for displaying 3DoF trajectories with Target motion
%
% See also: Simulink model 'aeroblk_guidance'

%   Copyright 1990-2003 The MathWorks, Inc.
%   J.Hodgson
%   Modified by: S.Gage  
%   $Revision: 1.3.2.4 $  $Date: 2004/04/06 01:04:22 $
...
otherwise
  ...
  %%%%%%%%%%%%%%%%%%%%
   ...
   error('aeroblks:saeroanim3dof:invalidflag',['Unhandled flag = ',num2str(flag)]);
...
%
   ...
   a = size(vert,1);
   dum   = (attitude*vert'+ repmat([u(1) 0 u(2)]',1,a))';
...
%
        ...
        set(handle.target,'vertices',x+ repmat([u(4) 0 u(5)],length(x),1));
...
%  count=1;                        % Counter for number of individual patches
   xcraft = cell(num,1);           % Preallocate for speed
   ycraft = cell(num,1);           % Preallocate for speed
   zcraft = cell(num,1);           % Preallocate for speed
