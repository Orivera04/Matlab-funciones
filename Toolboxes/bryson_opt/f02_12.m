% Script f02_12.m; semi-analytic solution for min dist on a sphere
% (Tokyo to New York); p=[thm,al];             8/97, 2/98, 3/23/02
%
c=pi/180; th0=35.7*c; thf=40.7*c; tf=2*pi-c*(139.7+73.8); 
p0=c*[70 -30]; optn=optimset('Display','Iter','MaxIter',20);
p=fsolve('geoa',p0,optn); un=ones(1,101); thm=p(1); al=p(2); 
ph=tf*[0:.01:1]; th=atan(tan(thm)*sin(ph-al*un));
be=acos(cos(thm)*un./cos(th)); for i=52:101, be(i)=-be(i); end
%
