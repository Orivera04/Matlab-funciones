function out = pdezoom(varargin)
%PDEZOOM   Zoom in and out on a 2-D plot.
%   Note: special version of ZOOM for the PDE Toolbox
%   preserves the WindowButtonMotion-callback.
%
%   PDEZOOM with no arguments toggles the zoom status.
%   PDEZOOM ON turns zoom on for the current axis.
%   PDEZOOM OFF turns zoom off in the current axis.
%   PDEZOOM OUT returns the plot to its initial (full) zoom.
%   PDEZOOM XON or PDEZOOM YON turns zoom on for the x or y axis only.
%   PDEZOOM RESET clears the zoom out point
%
...
%   the point at which zoom was first turned for this figure.  Note
%   that turning zoom on, then off does not reset the zoom point.
%   This may be done explicitly with PDEZOOM RESET.
%
%   PDEZOOM(FIG,OPTION) or PDEZOOM(FIG) applies the zoom command to the
%   figure specified by FIG. OPTION can be any of the above arguments.

%   Clay M. Thompson 1-25-93
%   Revised 11 Jan 94 by Steven L. Eddins
%   PDEZOOM add's: M Ringh 07-26-94, MR 10-01-96.
%   Copyright 1994-2003 The MathWorks, Inc.
%   $Revision: 1.9.4.1 $  $Date: 2003/11/18 03:12:12 $

%   Note: pdezoom uses the userdata of the zlabel of the axis and
%   the figure buttondown and buttonmotion functions
%
%   PDEZOOM XON zooms x-axis only
%   PDEZOOM YON zooms y-axis only
...
ax = findobj(get(fig,'Children'),'flat','Tag','PDEAxes');
if isempty(ax), error('PDE:pdezoom:NoAxis', 'PDE Toolbox axis not found.'), end
...
if any(get(ax,'view')~=[0 90]) && ~(strcmp(zoomCommand,'scale') || strcmp(zoomCommand,'fill')),
  return % Do nothing
end
...
if strcmp(zoomCommand,'down'),
  % Activate main axis if it is clicked in; else return
  ZOOM_found = 0;
...
elseif strcmp(zoomCommand,'on'),
  ...
  set(fig,'windowbuttondownfcn','pdezoom down', ...
...
elseif strcmp(zoomCommand,'xon'),
  pdezoom(fig,'on') % Set up userprop
  set(fig,'windowbuttondownfcn','pdezoom xdown', ...
          'windowbuttonupfcn','ones;', ...
          'buttondownfcn','',...
          'interruptible','on');
  set(ax,'interruptible','on')
...
elseif strcmp(zoomCommand,'yon'),
  pdezoom(fig,'on') % Set up userprop
  set(fig,'windowbuttondownfcn','pdezoom ydown', ...
          'windowbuttonupfcn','ones;', ...
          'buttondownfcn','',...
          'interruptible','on');
  set(ax,'interruptible','on')
...
elseif strcmp(zoomCommand,'fill'),
  ...
  error('PDE:pdezoom:InvalidOption', ['Unknown option: ',zoomCommand,'.']);
end
...
list = pdezoom(fig,'getconnect'); % Circular list of connected axes.
if zoomx,
