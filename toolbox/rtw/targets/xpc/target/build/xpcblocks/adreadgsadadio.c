/* $Revision: 1.11.6.1 $ $Date: 2003/11/20 11:57:25 $ */
/* adreadgsadadio.c - A/D input block for the General Standards ADADIO */
/* multifunction board.                                                */
/* Copyright 1994-2003 The MathWorks, Inc. */

#define 	S_FUNCTION_LEVEL 	2
#undef 		S_FUNCTION_NAME
#define 	S_FUNCTION_NAME 	adreadgsadadio
...
#define NCHANNELS_ARG           ssGetSFcnParam(S,0)
#define RANGE_ARG               ssGetSFcnParam(S,1)
#define COUPLING_ARG            ssGetSFcnParam(S,2)
#define IN_ENABLE_ARG           ssGetSFcnParam(S,3)
#define OUT_ENABLE_ARG          ssGetSFcnParam(S,4)
#define SAMP_TIME_ARG           ssGetSFcnParam(S,5)
#define SLOT_ARG                ssGetSFcnParam(S,6)
#define BCRINIT_ARG             ssGetSFcnParam(S,7)
#define INIT_ARG                ssGetSFcnParam(S,8)
#define CAL_ARG                 ssGetSFcnParam(S,9)
#define DACHANS_ARG             ssGetSFcnParam(S,10)
#define DARESET_ARG             ssGetSFcnParam(S,11)
#define DAIVAL_ARG              ssGetSFcnParam(S,12)
...
#define SCALE_R_IND             (0)   // Scaling factor, in real work vector
...
#include "util_xpcimport.c"
    ...
    {
        ssSetNumOutputPorts(S, (int)mxGetPr(NCHANNELS_ARG)[0]);
        for (i=0;i<(int)mxGetPr(NCHANNELS_ARG)[0];i++)
        {
    ...
    {
        ssSetNumOutputPorts(S, 1+(int)mxGetPr(NCHANNELS_ARG)[0]);
        ssSetOutputPortWidth(S, 0, 1);  // Set enable port width.
        ssSetOutputPortDataType(S, 0, SS_BOOLEAN); // Set enable port data type.
        for (i=0;i<(int)mxGetPr(NCHANNELS_ARG)[0];i++)
        {
            ssSetOutputPortWidth(S, i+1, 1);  // Set A/D port widths.
        }
    }
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    if( enabled )
    {
        ...
        while( !(tmp & INTERRUPT_REQUEST) )
            readLongIO( BCR, tmp );

        // If the board was not triggered first, then this routine will wait
        // forever.
        ...
        for ( i = first_chan ; i < (int_T)mxGetPr(NCHANNELS_ARG)[0] + first_chan ; i++ )
        {
            y = (real_T *) ssGetOutputPortSignal(S,i);
            readLongIO( AIN, tmp );
            y[0] = (real_T)tmp * scale;
        }
    }
#endif
}
