function [out1,out2,savepts] = flatplrp(mstruct,in1,in2,object,direction,savepts)

%FLATPLRP  McBryde-Thomas Flat-Polar Parabolic Pseudocylindrical Projection
%
%  This is an equal area projection.  Scale is true along the 45 deg,
%  30 min parallels, and is constant along any parallel and between any
...
if nargin == 1                  %  Set the default structure entries
	  ...
	  mstruct.mapparallels = angledim(4530,'dms',mstruct.angleunits);
...
%  Projection transformation
	 ...
	 x = sqrt(6) * radius * long .* (2*cos(2*theta/3) - 1) / sqrt(7);
...
% Inverse projection
	 ...
	 long = sqrt(7) * x ./ (sqrt(6) * radius * (2*cos(2*theta/3) - 1) );
