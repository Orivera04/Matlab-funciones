/* $Revision: 1.1 $ */
// dadsrubymm1612.c - xPC Target, non-inlined S-function driver for
// analog output section of the Diamond Systems Ruby-MM-1612 D/A 
// Copyright 2002 The MathWorks, Inc.

#define     S_FUNCTION_LEVEL  2
#undef      S_FUNCTION_NAME
#define     S_FUNCTION_NAME   dadsrubymm1612
...
#define RANGE_ARG     ssGetSFcnParam(S,1) // vector of {-2.5,-5,-10,2.5,5,10}
...
#define BASE_ARG      ssGetSFcnParam(S,5) // 0x220, 0x230, .. 0x3e0
...
static void mdlStart(SimStruct *S)
{
    ...
    for( i = 0 ; i < nChans ; i++ ) {
        rangecode = (int_T) (10 * mxGetPr(RANGE_ARG)[i]);
        switch (rangecode) {
            case 25: // 0V to 2.5V
                RWork[GAIN_R_IND + i]   = RESOLUTION / 2.5;
                RWork[OFFSET_R_IND + i] = 0;
                break;
            case 50: // 0V to 5V
            ...
            case 100: // 0V to 10V
                ...
                RWork[OFFSET_R_IND + i] = 0;
                break;
            case -25: // -2.5V to 2.5V
                RWork[GAIN_R_IND + i]   = RESOLUTION / 5.0;
                RWork[OFFSET_R_IND + i] = RESOLUTION / 2;
                break;
            case -50: // -5V to 5V
                RWork[GAIN_R_IND + i]   = RESOLUTION / 10.0;
                RWork[OFFSET_R_IND + i] = RESOLUTION / 2;
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    for( i = 0 ; i < nChans ; i++ ) {
        ...
        voltage = *uPtrs[0];
        count = (int_T) (voltage * RWork[GAIN_R_IND+i] + RWork[OFFSET_R_IND+i]);
        ...
        rl32eOutpB(base + 0, count & 0xff);  // LSB
        rl32eOutpB(base + 2, channel & 0xf); // channel number
        rl32eOutpB(base + 1, count >> 8);    // MSB
    }
