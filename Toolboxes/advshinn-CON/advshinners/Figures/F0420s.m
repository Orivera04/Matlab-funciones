function [ret,x0,str]=F0420S(t,x,u,flag);
%F0420S	is the M-file description of the SIMULINK system named F0420S.
%	The block-diagram can be displayed by typing: F0420S.
%
%	SYS=F0420S(T,X,U,FLAG) returns depending on FLAG certain
%	system values given time point, T, current state vector, X,
%	and input vector, U.
%	FLAG is used to indicate the type of output to be returned in SYS.
%
%	Setting FLAG=1 causes F0420S to return state derivatives, FLAG=2
%	discrete states, FLAG=3 system outputs and FLAG=4 next sample
%	time. For more information and other options see SFUNC.
%
%	Calling F0420S with a FLAG of zero:
%	[SIZES]=F0420S([],[],[],0),  returns a vector, SIZES, which
...
simver(1.2)
if(0 == (nargin + nargout))
     set_param(sys,'Location',[13,56,375,231])
     open_system(sys)
end;
set_param(sys,'algorithm',		'RK-45')
set_param(sys,'Start time',	'0.0')
set_param(sys,'Stop time',		'8')
...
set_param([sys,'/','C(s)'],...
		...
		'Vmax','2.200000',...
		'Hmax','16.000000',...
		'Window',[401,0,640,229])
open_system([sys,'/','C(s)'])
set_param([sys,'/','C(s)'],...
		'position',[305,22,325,48])
...
set_param([sys,'/','C*(s) '],...
		...
		'Vmax','2.200000',...
		'Hmax','16.000000',...
		'Window',[402,227,640,453])
open_system([sys,'/','C*(s) '])
set_param([sys,'/','C*(s) '],...
		'position',[305,77,325,103])
add_line(sys,[140,35;170,35])
add_line(sys,[220,35;295,35])
add_line(sys,[50,35;95,35])
add_line(sys,[220,35;230,35;230,90;235,90])
add_line(sys,[280,90;295,90])
