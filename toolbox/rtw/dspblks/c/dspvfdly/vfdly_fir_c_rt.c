/*
 *  vfdly_fir_c_rt Run-time function for Variable Fractional Delay block.
 *
 *  This function implements the case of FIR interpolation mode, 
 *  Date-type is :- single-precision complex. 
 * 
 *  Copyright 1995-2003 The MathWorks, Inc.
 *  $Revision: 1.3.2.2 $  $Date: 2004/04/12 23:49:39 $
 *
 *  
 */
...
EXPORT_FCN void MWDSP_Vfdly_FIR_C(
     ...
     const real32_T          frac)
{ 
    const creal32_T *in_curr = buffptr;     /* Store new input samples: */
    ...
    if (ti+1 >= filtlen) {
        ...
        int_T   kn;
        const real32_T *filt = (const real32_T *)(filtptr + phase * filtlen); 
        (*yptr)->re = (*yptr)->im = 0.0;
        ...
        const creal32_T *in_last = in_curr-ti+buflen-1;
        int_T kn;
        (*yptr)->re = (*yptr)->im = 0.0;
        for(kn=0; kn<k1; kn++) {
            (*yptr)->re += in_curr[-kn].re * *filt;
