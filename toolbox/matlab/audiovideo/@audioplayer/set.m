...
%SET Configure or display audioplayer object properties.
%
%    SET(OBJ) displays property names and their possible values for all 
%    configurable properties of audioplayer object OBJ. OBJ must be a 1-by-1
%    audioplayer object.
%
%    PROP_STRUCT = SET(OBJ) returns the property names and their 
%    possible values for all configurable properties of audioplayer object OBJ. 
...
%    for the specified property, PropertyName, of audioplayer object OBJ. 
%
...
%    PropertyName, to the specified value, PropertyValue, for audioplayer 
...
%    PV, for the audioplayer object OBJ. PN must be a vector. 
%
...
%       load handel;
%       p = audioplayer(y, Fs);
%       set(p) % Display all configurable properties and their possible values
%       set(p, 'Tag', 'My audioplayer object')
%
%    See also AUDIOPLAYER, AUDIOPLAYER/GET.

%    JCS
%    Copyright 2003 The MathWorks, Inc. 
%    $Revision: 1.1.6.1 $  $Date: 2003/12/04 19:00:48 $
        ...
        if (nargout == 0) % e.g., "set(OBJ,'PN',PV,....)"
            if nargin == 2 && ischar(varargin{1}) % e.g., "set(OBJ,'PN')"
                ...
                if ~isempty(strfind(hProp.Name, 'Fcn')),
                    ...
                    fprintf('string -or- function handle -or- cell array\n');
                elseif isempty(out),
                    fprintf(audioplayererror('MATLAB:audioplayer:propnotenumtype', hProp.Name));
                else
