...
%   $Revision: 1.10 $  $Date: 2002/04/10 05:10:59 $
...
switch action
 case 'init'
  % If the mouse is selecting this constraint, move pointer to margin origin 
  ...
  % Save initial margin data
  Constr.AppData = struct('Origin0', Constr.OriginPha);
  
 case 'restrict'
  % Restrict displacement Y0 -> Y to account for constraints.
  if Ylinabs
    Y = max(Y, 1e-3*max(PlotAxes.Ylim'));  % prevent negative gain
  end
  
 case 'update'
  % Gain margin origin should sit at -180 + 360k (in deg) for some k,
  % so that the initial gain margin origin is closest to the mouse position.
  dX = unitconv(X-X0, XUnits, 'deg') + Constr.AppData.Origin0;
  sgnPhaOrig = (dX >= 0) - (dX < 0);
  
  % Update gain margin origin (in deg)
  ...
  LocStr = sprintf('Gain margin moved from %0.3g to %0.3g %s.', ...
		   Origin0, Origin, XUnits);
