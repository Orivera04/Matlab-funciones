function post=mod_disc(model,prior,llike_fcn,data)
%
% MOD_DISC Posterior distribution for discrete models.
%	POST = MOD_DISC(MODEL,PRIOR,LLIKE_FCN,DATA) returns a vector of posterior 
%	probabilities. MODEL is the vector of parameter values, PRIOR is the 
%	corresponding vector of prior probabilities, LLIKE_FCN is the name of the
%	function containing the log likelihood and DATA is the vector of data
%	corresponding to the likelihood.

like=feval(llike_fcn,model,data);
like=100*exp(like-max(like));
