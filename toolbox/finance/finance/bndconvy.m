function [YearConvexity, PerConvexity] = bndconvy(varargin)
%BNDCONVY Convexity of Bond Given Yield.
%   This function calculates the convexity of NUMBONDS fixed income securities
%   given yield to maturity for each bond. This function will determine the
...
%
%   See also BNDCONVP, BNDDURP, BNDDURY.

%   Author(s): C. Bassignani, 07-29-98 
%   Copyright 1995-2002 The MathWorks, Inc. 
%   $Revision: 1.7 $   $Date: 2002/04/14 21:57:35 $ 
...
% Call cfamounts to get cash flows and time factors for all the bonds
...
%Set the frequency on zero coupon bonds to "2"
Period(Period == 0) = 2;

%Set the frequency on all non semi-annual pay bonds to "2"
Period(Period == 1) = 2;
Period(Period == 12) = 2;
Period(Period == 6) = 2;
Period(Period == 4) = 2;
...
for (i = 1 : NumBonds)
     ...
     a = TF1(i, :) .* TF2(i, :) .* CF(i, :) ./ ...
          ((1 + YLD(i, :) ./ PeriodMat(i, :)) .^ TF1(i, :));
     
     SumInd  = find(~isnan(a));
     
     A(i, 1) = sum( a(SumInd), 2 );
          
     c = CF(i, :) ./ ((1 + YLD(i, :) ./ PeriodMat(i, :)) .^ ...
          TF1(i, :));
     
     SumInd = find(~isnan(c));
     
     C(i, 1) = sum( c(SumInd), 2);
end
...
YearConvexity = PerConvexity ./ Period .^ 2;
