function rsdout=aoptimise(rsd,bindtogui, initfcn, updatefcn, stopfcn);
% DES_RESPSURF/AOPTIMISE   Optimise design using a-optimality.
%   D=AOPTIMISE(D) optimises the design in D using a-optimality.
...
%   $Revision: 1.5.2.3 $  $Date: 2004/02/09 07:03:28 $
...
while ~brk & next
   ...
   case 'optimal'
      ...
      [rsd,newpsi]=aaugment(rsd,p,oldpsi,0);
   end
   ...
   case 'optimal'
      ...
      [rsd, newpsi]=adelete(rsd,newpsi,p,0);
   end
   
   % ask if stop criterion met
   [brk, rvrt,rsd]=stop(rsd,'a',oldpsi,newpsi);
...
rsd=optimisetype(rsd,'A-optimal');
rsd=DesignType(rsd,1,'A-optimal');
