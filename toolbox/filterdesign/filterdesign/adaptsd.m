function [y,e,S] = adaptsd(x,d,S)
%ADAPTSD Sign-data FIR adaptive filter.
%   Y = ADAPTSD(X,D,S) applies a sign-data FIR adaptive filter
...
%
%   [Y,E]   = ADAPTSD(...) also returns the prediction error E.
%
%   [Y,E,S] = ADAPTSD(...) returns the updated structure S.
%
%   ADAPTSD can be called for a block of data, when X and D are vectors,
...
%   end
% 
%   In lieu of assigning the structure fields manually, the INITSD function can be 
...
%
%   See also INITSD, ADAPTSE, ADAPTSS, ADAPTLMS, ADAPTNLMS, and ADAPTRLS.
...
%   $Revision: 1.7 $  $Date: 2002/11/21 16:14:35 $

warning(generatemsgid('obsolete'), ...
        ['ADAPTSD is obsolete and will be removed in future versions of \n',...
...
[y,e,S] = adaptfirfilt(x,d,S,@updatesd);
