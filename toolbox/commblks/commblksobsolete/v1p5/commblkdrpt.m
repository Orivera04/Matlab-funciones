function varargout = commblkdrpt(block, varargin)
% COMMBLKDRPT Communications Blockset Derepeat block helper function.

% Copyright 1996-2002 The MathWorks, Inc.
% $Revision: 1.1.6.1 $ $Date: 2003/01/26 18:44:31 $
...
if(strcmp(action,'UpdateFields'))
   feval(mfilename,block,'cbFBFlag',1);
...
%
% Description:		Deal with the frame based mode of operation
%
% Inputs:			current block
...
if(strcmp(action,'cbFBFlag'))
   ...
   if(strcmp(Vals{FBFlag},'off'))
      En{NumChan} = 'off'; 
      En{Mode}    = 'off'; 
      En{IC}      = 'on'; 
      set_param(block,'MaskEnables',En);
   elseif(strcmp(Vals{FBFlag},'on'))
      En{NumChan} = 'on'; 
      En{Mode}    = 'on'; 
...
if(strcmp(action,'default'))
   ...
   Cb{Drpt}   	= '';
   Cb{IC} 	   = '';
   Cb{FBFlag}	= [mfilename '(gcb,''cbFBFlag'');'];
   Cb{NumChan}	= '';
