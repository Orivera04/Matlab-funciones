/*
 * fft_r2dit_tblm_z_rt.c - Signal Processing Blockset FFT run-time library components.
 *
 *  Copyright 1995-2003 The MathWorks, Inc.
 *  $Revision: 1.7.2.3 $  $Date: 2004/04/12 23:43:45 $
 */
 ...
 * Input is in bit-reverse order, output is in linear order
 * Table-based memory-optimized twiddle computation (TBLM)
 * Complex double-precision input (Z)
 *
 * twiddleTable is assumed to be in linear order, with N/4+1 elements
 */
EXPORT_FCN void MWDSP_R2DIT_TBLM_Z(
    creal_T      *y,
    int_T         nChans,
    const int_T   nRows,
    const int_T   fftLen,
        ...
        /* Remaining butterflies: */
        {
            ...
            while(k>0) {
                ...
                for(j=0; j<nHalf; j+=kratio) {
                    int_T   i1 = istart;
                    int_T   i;
                    creal_T twid;  /* conjugate twiddle */

                    twid.re = (j>nQrtr) ? -twiddleTable[nHalf-j] : twiddleTable[j];
                    twid.im = twiddleTable[ (j>nQrtr) ? j-nQrtr : nQrtr-j];
