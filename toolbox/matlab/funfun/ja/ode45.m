%ODE45  中程度の精度でスティッフでない微分方程式を解きます
%
% [T,Y] = ODE45(ODEFUN,TSPAN,Y0) は、TSPAN = [T0 TFINAL] のとき、
...
%   
% [T,Y] = ODE45(ODEFUN,TSPAN,Y0,OPTIONS) は、デフォルトの積分パラ
...
% [VALUE,ISTERMINAL,DIRECTION] = EVENTS(T,Y) によって出力される3つの
% ベクトルです、I番目のイベント関数に対して、VALUE(I) は、積分がこの
% イベント関数のゼロで終了する場合は関数 ISTERMINAL(I)=1の値で、そう
% でない場合は0です。すべてのゼロが計算される(デフォルト)場合は、
...
%
% SOL = ODE45(ODEFUN,[T0 TFINAL],Y0...) は、T0 と TFINAL 間の任意
% の点で、解を計算するため、DEVAL で使用可能な構造体を出力します。
% ODE45 で選択されたステップは、行ベクトル SOL.x に出力されます。
...
% 注意:
% 以下では、ODE45 の v5 シンタックスを記述します。
%
% [T,Y] = ODE45('F',TSPAN,Y0) では、TSPAN = [T0 TFINAL] の場合、初期条件を
...
%   
% [T,Y] = ODE45('F',TSPAN,Y0,OPTIONS) は、ODESET 関数で作成された
...
% が 1e-6 ) です。
%   
% [T,Y] = ODE45('F',TSPAN,Y0,OPTIONS,P1,P2,...) は、付加的なパラメータ
...
% TSPAN または Y0 が空の場合、ODE45 は、ODE45 引数リストで与えられない値
% を得るために、ODE ファイルをコールします。[TSPAN,Y0,OPTIONS] = F([],[],'init')% コールリストの最後の空の引数は、たとえば、ODE45('F') のように省略すること
% ができます。
%   
% ODE45 は、非特異質量行列 M をもつ問題  M(t,y)*y' = F(t,y) を
...
%   
% [T,Y,TE,YE,IE] = ODE45('F',TSPAN,Y0,OPTIONS) では、OPTIONS の Events
...
% ODE45 は、Dormand , Prince による陽的 Runge-Kutta (4,5) 法
% (RK5(4)7FM, DOPRI5, DP(4,5), DP54 などとも呼ばれる）を実行します。
% これは、Dormand, Prince から私的に得た、4 次の "free" interpolant を使用
% します。局所的な補外が行われます。
