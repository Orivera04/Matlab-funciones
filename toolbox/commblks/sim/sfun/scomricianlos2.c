 ...
 *       required K-factor.
 *
 * If the Doppler 'spur' frequency is zero, the offset required for the
 * Rician distribution will be added directly to the real path. (i.e.,
 * the trig terms are not computed).
 *
 *  Copyright 1996-2003 The MathWorks, Inc.
 *  $Revision: 1.5.4.2 $  $Date: 2004/04/12 23:03:37 $
 */


#define S_FUNCTION_NAME scomricianlos2
...
/* --- Define i/o signals */
enum {
    INPORT = 0, NUM_INPORTS
};
enum {
    OUTPORT = 0, NUM_OUTPORTS
};

/* --- Define mask parameters */
enum {
    DOPPLER_A = 0, K_FACTOR_A, OP_POWER_A, SAMPLE_TIME_A, NUM_CHANS_A, NUM_ARGS
};
...
/* --- Define work vectors */
enum {
    START_PHASE = 0, PHASE_DELTA, K_OFFSET, K_SIGMA, NUM_DWORK
};
					  ...
					  const DimsInfo_T * dimsInfo)
{
    ...
    if (ssGetInputPortWidth(S, INPORT) == DYNAMICALLY_SIZED) {
        ssSetInputPortMatrixDimensions(S, INPORT, dimsInfo->dims[0], dimsInfo->dims[1]);
    } else {
        THROW_ERROR(S, "Port width propagation error.");
    }
}
