/*
 * SDSPMPROD Matrix "prod" (row/column product) block.
 *
 *  The Matrix Prod block multiplies elements along the rows or columns
 *  of the input matrix.
 *
 *   
 *  Copyright 1995-2002 The MathWorks, Inc.
 *  $Revision: 1.12 $  $Date: 2002/04/14 20:42:39 $
 */
#define S_FUNCTION_NAME  sdspmprod
...
static void mdlCheckParameters(SimStruct *S) {
    ...
    if (!IS_FLINT_IN_RANGE(DIM_ARG, 1, 2)) {
        THROW_ERROR(S, "Matrix product dimensions must be 1=rows or 2=columns.");
    }
...
static void mdlInitializeSizes(SimStruct *S)
{
     ...
     *  - prod across rows, but input is a column
     *  - prod down columns, but input is a row
            ...
            * ROW PRODUCT
                */
                ...
                for (i=1; i++ < cols; ) {
                    int_T j;
                    for (j=0; j<rows; j++) {
                        const creal_T yj_old = y[j];
                        creal_T       u      = *((creal_T *)(*uptr++));
                        
                        y[j].re = CMULT_RE(yj_old, u);
                        y[j].im = CMULT_IM(yj_old, u);
                    }
                }
#else
                ...
                for (j=0; j<rows; j++) {
                    ...
                    for (i=1; i<cols; i++) {
                        const creal_T s_old = s;
                        creal_T       u     = *((creal_T *)uptr[i+rows]);
