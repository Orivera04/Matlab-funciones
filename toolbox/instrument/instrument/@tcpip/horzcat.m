...
%   $Revision: 1.2.2.3 $  $Date: 2004/01/16 20:01:46 $
    ...
    if ~isempty(varargin{i}),
        if isempty(c),
            ...
            if ~isa(varargin{i},'instrument'),
                error('instrument:horzcat:nonInstrumentConcat', 'Instrument objects can only be concatenated with other instrument objects.')
            end
            c=varargin{i};
        else
...
if length(c.jobject) ~= numel(c.jobject)
   error('instrument:horzcat:nonMatrixConcat', 'Only a row or column vector of instrument objects can be created.')
end
