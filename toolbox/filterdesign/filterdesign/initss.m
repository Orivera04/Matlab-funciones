function S = initss(varargin)
%INITSS Initialize structure for sign-sign FIR adaptive filter.
%   S = INITSS(W0,MU) returns the fully populated structure that 
%   must be used when calling ADAPTSS. W0 is the initial value of
%   the filter coefficients.  Its length should be equal to the filter
%   order plus one. MU is the step size.
%
%   S = INITSS(W0,MU,Zi) specifies the filter initial conditions.
%   It can be specified as empty, [], to get the default values, i.e.,
%   a zero vector of length one less than length(w0).
%
%   S = INITSS(W0,MU,Zi,LF) specifies the leakage factor LF. If specified
%   as empty, it defaults to one.
%
%   See also ADAPTSS, INITSE, INITSD, ADAPTLMS, ADAPTRLS, INITLMS, and INITNLMS.
...
%   $Revision: 1.4 $  $Date: 2002/11/21 16:14:39 $

warning(generatemsgid('obsolete'), ...
        ['INITSS is obsolete and will be removed in future versions of \n',...
