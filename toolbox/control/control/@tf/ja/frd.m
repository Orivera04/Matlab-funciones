...
%
% 周波数応答データ(FRD)モデルは、実験応答データを含むLTIシステムの周波数
% 応答をストアするのに有効です。
%
% 作成:
% SYS = FRD(RESPONSE,FREQS) は、FREQS の周波数点での応答データ RESPONSE 
% をもつ FRD モデル SYS を作成します。出力引数 SYS はFRDモデルです。
...
% 表現します。
%
% NU 入力 NY 出力で、周波数点 NF の MIMO FRDモデルに対して、RESPONSE は、
...
% (SYS,UNITS) を用いて、FRDモデルの周波数単位を変更すると、必要な変換が
% 実行されます。
%
% FRDモデルの配列:
% 上記の RESPONSE に対して、ND 配列を利用することで、FRDモデルの配列を
...
% です。各々の FRD モデルは、NU 入力 NY 出力で、FREQS の各々の周波数点
% でのデータをもちます。
%
% 変換:
