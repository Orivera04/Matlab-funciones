...
%WINDTRANDEMO Window Transition Generator Demo
%    WINDTRANDEMO - launches a demo that uses windows functions
%    to create smooth transitions between arbitrary vectors.  
%
%   See also WINDOW, PSD
...
%   $Revision: 1.12.4.2 $  $Date: 2004/04/12 23:51:48 $

% Last Modified by GUIDE v2.0 25-Oct-2001 10:09:03
...
%    handles.impposlin = line(1,1,'Parent',handles.posplot,'Color','r','Marker','x');
    ...
    h1 = handle(handles.wintypepop);
    h2 = handle(handles.winparam);
    listener = handle.listener([h1 h2], [h1.findprop('Value') h2.findprop('Value')], ...
     'PropertyPostSet', @window_listener);
    setappdata(fig, 'Listeners', listener);
...
if isempty(handles.wvtool),
    ...
    listener = handle.listener(handles.wvtool, 'WVToolClosing', @wvtool_listener);
