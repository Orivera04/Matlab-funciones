...
%   $Revision: 1.1.6.4 $  $Date: 2004/04/12 23:36:32 $
...
if isempty(ckts)
    ...
    error(id, 'Hybrid network needs the property ''CKTS'' in order to do analysis.'); 
end
...
for i=1:nckts
    ...
    if isnonlinear(ckt)
        ...
        error(id, 'All the circuits in a Hybrid network must be linear.');
    end
    if get(ckt, 'nPort') ~= 2
        id = sprintf('rf:%s:checkproperty:TwoPortOnly', strrep(class(h),'.',':'));
        error(id, 'All the circuits in a Hybrid network must be 2-port.');
    end
end
