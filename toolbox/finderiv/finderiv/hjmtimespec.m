function TimeSpec = hjmtimespec(varargin)
%HJMTIMESPEC Specify time structure for an HJM tree.
%   Set the number of levels and node times for an HJM tree, and
...
%
% Output:
%   TimeSpec - Structure specifying the time layout for HJMTREE.  The
...
%   $Revision: 1.10 $  $Date: 2002/04/14 16:39:52 $

%-----------------------------------------------------------------------
% Argument parsing and defaults
%-----------------------------------------------------------------------

% Insure that there are 5 argument variables
if nargin<2,
  error('ValuationDate and Maturity are required');
elseif nargin>5
...
[ValuationDate, Maturity, Compounding, Basis, EOM] = ... 
    finargparse(ClassList, varargin{:}, EndArgs{:});

% parse sizes
Maturity = sort(Maturity(:));

% everyone else should be scalar

% handle defaults
EOM( isnan(EOM) ) = 1;
Basis( isnan(Basis) ) = 0;
Compounding( isnan(Compounding) ) = 1;

%-----------------------------------------------------------------------
% Store data 
%-----------------------------------------------------------------------
TimeSpec = classfin('HJMTimeSpec');
TimeSpec.ValuationDate = ValuationDate;
TimeSpec.Maturity = Maturity;
TimeSpec.Compounding = Compounding;
TimeSpec.Basis = Basis;
TimeSpec.EndMonthRule = EOM;
