 ...
 *
 * Abstract:
 *   Utility functions to manage data format between simulation and target
 *   processor. 
 *
 *
 * $Revision: 1.9.4.2 $
 * $Date: 2004/04/19 01:28:07 $
 *
...
#define SWAPBYTES (!CommIsSameEndian())
...
void ProcessConnectResponse1(ExternalSim *ES, MsgHeader *msgHdr)
{
    ...
    esSetSingleTargetToHostFcn(ES, (void *) Generic_TargetToHost); /* assume 32 bit */
    esSetSingleHostToTargetFcn(ES, Generic_HostToTarget); /* assume 32 bit */
    
    esSetInt8TargetToHostFcn(ES, (void *) Generic_TargetToHost);
    esSetInt8HostToTargetFcn(ES, Generic_HostToTarget);

    esSetUInt8TargetToHostFcn(ES, (void *) Generic_TargetToHost);
    esSetUInt8HostToTargetFcn(ES, Generic_HostToTarget);

    esSetInt16TargetToHostFcn(ES, (void *) Generic_TargetToHost);
    esSetInt16HostToTargetFcn(ES, Generic_HostToTarget);

    esSetUInt16TargetToHostFcn(ES, (void *) Generic_TargetToHost);
    esSetUInt16HostToTargetFcn(ES, Generic_HostToTarget);

    esSetInt32TargetToHostFcn(ES, (void *) Generic_TargetToHost);
    esSetInt32HostToTargetFcn(ES, Generic_HostToTarget);

    esSetUInt32TargetToHostFcn(ES, (void *) Generic_TargetToHost);
...
{
#if 1 == 0
    int DataSize = ssGetDataTypeSize(S,Type);
    Generic_TargetToHost(&ES,(void *)targetBuffer,(const char *)sourceBuffer,(const int)DataSize,(const int)Elements);
#else
    int DataSize;
    switch (Type) {
    ...
    Generic_TargetToHost(&ES,(void *)targetBuffer,(char *)sourceBuffer,(const int)Elements,(const int)DataSize);
#endif
}

void pilMarshallOutput(
    void *targetBuffer,
    void *sourceBuffer,
    int Elements,
    DTypeId Type,
...
#if 1 == 0
    int DataSize = ssGetDataTypeSize(S,Type);
    Generic_HostToTarget(&ES,(void *)targetBuffer,(const char *)sourceBuffer,(const int)Elements,(const int)DataSize);
#else
    int DataSize;
    switch (Type) {
      case SS_DOUBLE: /* real_T    */
...
void pilMarshallInitialize(SimStruct *S)
{
    esSetSwapBytes(&ES,1);
    esSetDoubleTargetToHostFcn(&ES, Double_TargetToHost);
    esSetDoubleHostToTargetFcn(&ES, Double_HostToTarget);
