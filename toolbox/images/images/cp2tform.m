...
%   Note
%   ----
%   When either either INPUT_POINTS or BASE_POINTS has a large offset with
...
%
%  See also CPSELECT, CPCORR, CPSTRUCT2PAIRS, IMTRANSFORM.

%   Copyright 1993-2003 The MathWorks, Inc. 
%   $Revision: 1.10.4.4 $  $Date: 2003/08/01 18:08:42 $
...
switch method
  ...
  case 'linear conformal'
    findT_fcn = 'findLinearConformalTransform';
  case 'polynomial'
...
if ~needToShift
    ...
    [tshifted, output] = feval(findT_fcn,applyShift(uv,uvShift),...
...
if rank(X) >= 2*K 
    ...
    msg = 'At least 2 unique points needed to infer linear conformal transform.';
...
% interior points.
hull_indices = convhullx(x,y,tri);
OnHull = zeros(size(x));
...
if size(uv,2) ~= 2 | size(xy,2) ~= 2
    error(sprintf('Images:%s:invalidControlPointMatrix', mfilename),...
                  'In function %s, control point matrices must be M-by-2.', mfilename);
end

if size(uv,1) ~= size(xy,1)
    error(sprintf('Images:%s:needSameNumControlPoints', mfilename),...
...
switch nargs_to_go
  ...
  case 1
    ...
    elseif strcmp(method,'lwm')
        ...
        error(sprintf('Images:%s:tooManyInputs', mfilename),...
...
switch method
  ...
  case 'polynomial'
    order = options.order;
    % validate order
    if ~isnumeric(order) | prod(size(order))~=1 | sum(order==2:4)~=1
        error(sprintf('Images:%s:invalidPolynomialOrder', mfilename),...
  ...
  case 'lwm'
    ...
    if isempty(N)
        ...
        % validate N
        if ~isnumeric(N) | prod(size(N))~=1 | rem(N,1)~=0 | N<options.K
            error(sprintf('Images:%s:invalidInputN', mfilename),...
    ...
    options.N = N;
  otherwise
    error(sprintf('Images:%s:internalProblem', mfilename),...
...
function method = GetMethod(method_string)
    ...
    if ischar(method_string)
        ...
        switch length(indx)
          case 0
            msg = sprintf('Unrecognized TRANSFORMTYPE ''%s'' in function %s.',method_string,mfilename);
            eid = sprintf('Images:%s:unrecognizedTransformType',mfilename);
            error(eid,msg);
          case 1
            method = methods{indx};
          otherwise
            msg = sprintf('Ambiguous TRANSFORMTYPE ''%s'' in function %s.',method_string,mfilename);
            eid = sprintf('Images:%s:ambiguousTransformType',mfilename);
            error(eid,msg);
        end
    else
        msg = sprintf('TRANSFORMTYPE must be a string in function %s.',mfilename);
