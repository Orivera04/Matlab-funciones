...
% $Revision: 1.1.6.6 $  $Date: 2004/02/01 22:05:55 $
...
if ~isempty(uddValid),
    % Delete the group(s) from the parent property. This is done by replacing
    % the Group property with the groups which are not being removed from
    % the parent.
    parentObj = uddValid(1).Parent;
    origParentGroup = get(parentObj,'Group');
    origGroupUDD = getudd(origParentGroup);
    [newGroupUDD, grpInd] = setdiff(origGroupUDD, uddValid);
    % Now how do we use the grpInd values? SUBSREF call!!!
    if isempty(newGroupUDD),
        newGroup = [];
    else
        newGroup = subsref(origParentGroup, struct('type','()', 'subs',grpInd));
    end
    
    udsetchild(getudd(parentObj), newGroup);
