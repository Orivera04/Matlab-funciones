%COMMBLKDFEEQRLS
% Mask error checks for the RLS DFE block

%   Copyright 1996-2003 The MathWorks, Inc.
%   $Revision: 1.1.6.2 $  $Date: 2003/12/01 18:59:25 $
...
if ~isnumeric(sigConst) || ~isvector(sigConst)
    error('commblks:dfeqrls:sigConst',...
...
% adjust the inverse correlation matrix by the forgetting factor. This is
% to compensate for the 1 sample delay in the fliter. This ensures that
% the inverse correlation matrix is exactly what the user thinks it is at
...
if ~ any(isnumeric(initWeights) )
      error('commblks:dfeqrls:initweights',...
