function [score, alignment, startat] = swalign(seq1,seq2,varargin)
...
%
%   See also BLOSUM, NT2AA, NWALIGN, PAM, SEQDOTPLOT, SHOWALIGNMENT.
...
%   $Revision: 1.21.6.7 $  $Date: 2004/01/24 09:19:10 $
                ...
                case 1  % scoring matrix
                    if isnumeric(pval)
                        ...
                        try
                            ...
                            error('Bioinfo:InvalidScoringMatrix',...
                                'Invalid scoring matrix.');
                        end
                    end
...
if (isAminoAcid && ~(isaa(seq1) && isaa(seq2))) || (~isAminoAcid && ~(isnt(seq1) && isnt(seq2)))
    error('Bioinfo:InvalidInputSequences',...
...
n = length(seq2);
if ~n||~m
    error('Bioinfo:InvalidLengthSequences',...
        'Length of input sequences must be greater than 0');
end
...
% calculate indices of maximum score
[maxrows , rowindices] = max(F(:,:,1));
[maxval , colindex] = max(maxrows);
score = maxval;

i = rowindices(colindex);
j = colindex;

% trace back through the pointer matrix
%i = rowindices; j = colindex;
...
if setGapExtend
    ...
    while (i>1 || j>1)
        ...
        elseif bitget(pointer(i,j,state),2)
            laststate=2;
        elseif bitget(pointer(i,j,state),3)
            laststate=3;
        else
            break %end of run
        end
    ...
    while (i > 1 || j > 1)
        ...
        switch pointer(i,j)
            ...
            case 6 % up or left --> up (favors gaps in seq2)
                j = j - 1;
            case 0 % end of run (local alignment)
                break
            otherwise 
                %3 diagonal or up         --> diagonal (favors no gaps)
                %5 diagonal or left       --> diagonal (favors no gaps)
...
path(step+1:end,:) = [];
if step == 1 % this is the case for any empty alignment attempt
    alignment = char(zeros(3,0));
    startat = zeros(2,1);
    warning('Bioinfo:EmptyAlignment',...
            'The query sequences can not be aligned with a positive score, use NWALIGN to force a global alignment.')
...
if showscore
    ...
    ylabel('Sequence 2')
    hold on
    plot(path(1:end-1,1),path(1:end-1,2),'k.')
end
