...
%
%   NEWPATH = TRIMPATH(INPATH) trims the given path INPATH and returns the
%   trimmed version NEWPATH.  The trimming includes removing empty directory
%   strings, duplicate directories, non-existing directories and trailing file
%   separators from each directory.
%
%   If no input path is given, the current search path is used.  If no output
%   argument is specified, the new path is set as the new search path.

%   Author:      Peter J. Acklam
%   Time-stamp:  2003-11-04 00:41:16 +0100
