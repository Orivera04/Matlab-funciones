/*
 *  vfdly_fir_r_rt Run-time function for Variable Fractional Delay block.
 *
 *  This function implements the case of FIR interpolation mode, 
 *  Date-type is :- single-precision, real. 
 * 
 *  Copyright 1995-2003 The MathWorks, Inc.
 *  $Revision: 1.3.2.2 $  $Date: 2004/04/12 23:49:41 $
 *
 *  
 */
...
EXPORT_FCN void MWDSP_Vfdly_FIR_R(
     ...
     const real32_T         frac)
{
    ...
    if (ti+1 >= filtlen) {
        ...
        **yptr       = 0.0;
        ...
        const real32_T *in_last = in_curr-ti+buflen-1;
        int_T kn;
        **yptr = 0.0;
        for(kn=0; kn<k1; kn++) {
            **yptr += in_curr[-kn] * *filt++;
        ...
        for(kn=0; kn<k2; kn++) {
            **yptr += in_last[-kn] * *filt++;
        }
        ++(*yptr);
    }
}

/* [EOF] vfdly_fir_r_rt.c */
