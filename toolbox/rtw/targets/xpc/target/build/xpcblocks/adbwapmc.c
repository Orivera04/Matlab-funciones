/* $Revision: 1.1.6.2 $ $Date: 2004/04/08 21:01:30 $ */
/* adbwapmc.c - xPC Target, non-inlined S-function driver for the A/D function */
...
#define         S_FUNCTION_NAME         adbwapmc
...
extern long g_verbose;  // Set to 1 in xpcHelpers.c to get verbose printouts from pmc_aud routines.

extern arraydes DspProg;

// AudPMCBoards is shared with dabwapmc.c as well.
...
// The next 4 are indexed in a loop as (CHAN_1_BUF + i) for i = 0,1,2,3
#define CHAN_1_BUF              (2)
#define CHAN_2_BUF              (3)
#define CHAN_3_BUF              (4)
#define CHAN_4_BUF              (5)
...
#define NUMBUFS                  3  /* must agree with the value in the DA driver */
...
#include "time_xpcimport.c"
    ...
    outformat = (int)mxGetPr( CONVERT_ARG )[0];
    switch( outformat )
    {
      case 1:
        // outformat == 1 means 32 bit integer
        datafmt = SS_INT32;
        break;

      case 2:
        // outformat == 2 means double
        datafmt = SS_DOUBLE;
        break;
    }
    ...
    for ( i = 0 ; i < nchans ; i++ )
    {
        if( !ssSetOutputPortMatrixDimensions( S, i, width, 1 ) )
            return;
        ssSetOutputPortDataType( S, i, datafmt );
        if( width > 1 )
...
//printf("AD mdlStart running\n");
    ...
    for( boardNum = 0 ; boardNum < NUM_AUDPMC_BOARDS ; boardNum++ )
    {
        ...
        if( checkBoard->proc[0] != 0 && checkBoard->proc[1] != 0 )
        {
            ...
            {
                thisBoard = boardNum;
//printf("AD found existing board at slot %d\n", thisBoard );
    ...
    if( thisBoard == -1 )  // We didn't find it above.
    {
        int pnum;
        boardInfo *info;
//printf("AD Board not previously initialized, starting configuration\n");
...
//printf("info: %d has 0x%x, 0x%x, %d, %d\n", boardNum, info->proc[0], info->proc[1], info->bus, info->slot );
...
//printf("ConstructProc returned 0x%x for processor %d\n", processor, pnum );
            if( processor == 0 )
            {
    ...
    // Do the next section always.  A/D buffer allocation.
    if( !xpceIsModelInit() )
    {
        ...
        for( i = 0 ; i < 4 ; i++ )
        {
            ...
            desc_addr += (i%2);  // word addresses on DSP
            ...
            do
            {
                ...
                if( size >= 4096 )
                {
                    ...
                    if( (int)(raw_buffer) + 4096 != (int)rl32eGetDevicePtr( (void *)(raw_buffer+4096), size - 4096, RT_PG_USERREADWRITE ) )
                        continue;  // Try again.
                }
                // Check the end of the buffer since it may have crossed
                // a page boundary.
                if( (int)(raw_buffer) + size-8 != (int)rl32eGetDevicePtr( (void *)(raw_buffer+size-8), 8, RT_PG_USERREADWRITE ) )
                    continue;  // Try again.
            } while( virtual != raw_buffer );  // should work on any system
//printf("AD: raw = 0x%x, addr = 0x%x\n", raw_buffer, desc_addr );

            for( j = 0 ; j < size/4 ; j++ )
                *((int *)raw_buffer + j) = 0;  // Clear the buffers.

            direct_buffer = ((int)raw_buffer + 7) & ~0x7;
...
//printf("%d orig = 0x%x, return = 0x%x\n", i, direct_buffer, test );
    ...
    if( doinit == 1 )
    {
        // Start both processors, after setting buffer addresses
        boardInfo *info = &AudPMCBoards[ thisBoard ];
        int pnum;
        int desc_addr;
        double dsp1, dsp2;

        desc_addr = dsp21k_get_addr( info->proc[0], "_avg_idle" );
        ...
        if(0)  // In the R12.1 version, start interrupts here.
        {
...
//    uint_T convert = mxGetPr( CONVERT_ARG )[0];
    host_desc *buffer;
    int i;
    int j;
    int channel;
    ...
    // All buffers are in sync, so just get last_frame from the first buffer.
    // This reduces the number of writes to the host that the DSP has to make.
    buffer = (host_desc *)ssGetIWorkValue( S, CHAN_1_BUF );
    buffer = (host_desc *)(((int)buffer + 7) & ~7);
    if( buffer != 0 )
    {
        last_frame[0] = buffer->lower;  // [0, NUMBUFS-1]
        overrun[0] = buffer->upper;
    }
    else
    {
        last_frame[0] = 0;  // [0, NUMBUFS-1]
        overrun[0] = 0;
    }
    ...
    if( buffer != 0 )
    {
        last_frame[1] = buffer->lower;
        overrun[1] = buffer->upper;
    }
    else
    {
        last_frame[1] = 0;  // [0, NUMBUFS-1]
        overrun[1] = 0;
    }
    ...
    for( i = 0 ; i < nchans ; i++ )
    {
        ...
        channel = mxGetPr(CHANNEL_ARG)[i] - 1;  // convert 1 based channel to 0 based
        outputType = ssGetOutputPortDataType( S, channel );
        pnum = channel / 4;      // processor 0 or 1
        lr = channel & 1;        // Even channel is first, odd is second
        channel = channel / 2;   // round down to the left channel of the pair
        // Get the raw buffer address, then align on 8 byte boundary.
        buffer = (host_desc *)ssGetIWorkValue( S, CHAN_1_BUF + channel );
        if( buffer == 0 )
            continue;  // No buffer, don't copy anything.
        buffer = (host_desc *)(((int)buffer + 7) & ~7);  // Align
//printf("%d, %d, %d, 0x%x\n", pnum, lr, channel, buffer );
        // if this is the second channel in a buffer, add 4 bytes to the address.
        // Remember that the first entry in the buffer is last_frame, hence the
        // +8 here to skip the first 2 ints.
        buffer = (host_desc *)((int)buffer + 8 + 4 * lr);
        buffer += last_frame[pnum] * width;  // Point to the correct part of the buffer.

        switch( outputType )
        {
          case SS_DOUBLE:
            y = (real_T *) ssGetOutputPortSignal(S,i);
            for( j = 0 ; j < width ; j++ )
                y[j] = 1/(double)(0x7fffffff) * (double)(buffer + j)->lower;
            break;

          case SS_INT32:
            ui = (int *)ssGetOutputPortSignal( S, i ); // u;
            for( j = 0 ; j < width ; j++ )
            {
                ui[j] = (buffer + j)->lower >> 8;
            }
            break;

          default:
            // ERROR: illegal data type of output data
            sprintf( msg, "Illegal output data type, %d", outputType );
            ssSetErrorStatus( S, msg );
            return;
        }
    }
...
//printf("AD mdlTerminate starting\n");
...
//printf("desc_addr for avg_idle = %x\n", desc_addr );
...
//printf("desc_addr for host_buffers = %x\n", desc_addr );
            for( j = 0 ; j < 4 ; j++ )
...
//            printf("dsp1 average idle = %f\n", dsp21k_ul_flt( checkBoard->proc[1], desc_addr ) );
    ...
    // free the buffers that this driver allocated
    for( j = 0 ; j < 4 ; j++ )
    {
