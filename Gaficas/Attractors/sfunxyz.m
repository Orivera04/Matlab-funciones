...
for i=1:nmax,
    set(ud.XYLine,'Xdata',x_data(i,:),'Ydata', y_data(i,:),'Zdata', z_data(i,:),'LineStyle','-','Color',cord(1+mod(i-1,6),:));
end
...
if ishandle(FigHandle),
  ...
  for i=1:size(ud.XYAxes,1),
      set(ud.XYLine,'Xdata',ud.XData(i,:),'Ydata', ud.YData(i,:),'Zdata', ud.ZData(i,:),'LineStyle','-','Color',cord(1+mod(i-1,6),:));
  end
...
FigHandle = figure('Units',          'pixel',...
                   ...
                   'Toolbar',        'none',...
                   'Menubar',        'none',...
                   'DeleteFcn',      'sfunxyz([],[],[],''DeleteFigure'')');
%
