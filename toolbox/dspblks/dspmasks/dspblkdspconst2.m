...
% DSPBLKDSPCONST2 Signal Processing Blockset DSP Constant block helper function.

% Copyright 1995-2004 The MathWorks, Inc.
% $Revision: 1.1.6.2 $ $Date: 2004/04/12 23:06:20 $
...
function set_discrete_samp_time(blk)
  ...
  if strcmp(outMode,FRAME_POPUP)
    dspsafe_set_param(const_blk,'SampleTime','framePeriod');
  else
    dspsafe_set_param(const_blk,'SampleTime','sampTime');
  end
...
function set_continuous_samp_time(blk)
  fullblk   = getfullname(blk);
  const_blk  = [fullblk '/Constant'];
  dspsafe_set_param(const_blk,'SampleTime','inf');
