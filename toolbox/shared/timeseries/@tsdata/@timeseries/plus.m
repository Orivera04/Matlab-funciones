...
%PLUS Overloaded addition
%
%   Author(s): James G. Owen
%   Copyright 1986-2004 The MathWorks, Inc.
%   $Revision: 1.1.6.4 $ $Date: 2004/04/11 00:33:51 $
...
if norm(ts1timevec-ts2timevec)/intervalLen>1e-6
    error('timeseries:plus:badsizes', 'Time vectors do not match')
end

% Load the data so its not extracted for each pass of the loop
[data1,data2,s1,s2] = tsAlignSizes(ts1.Data,ts1.DataInfo.GridFirst, ...
    ts2.Data,ts2.DataInfo.GridFirst);
if ~isequal(s1,s2)
...
try
   ...
   try 
       ...
       errstr = sprintf('%s\n%s','Plus failed for this type of ordinate data object.', ...
...
set(tsout,'Name',['Sum_' ts1.Name '_' ts2.Name]);
...
if ~isempty(ts1.qualityInfo.Code) && ~isempty(ts2.qualityInfo.Code)
...
% Merge events
addevent(tsout,[ts1.Events,ts2.Events]);
