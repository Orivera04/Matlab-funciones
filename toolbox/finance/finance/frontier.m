...
%   [RISK,ROR,WTS] = FRONTIER(ASSET,RET,PTS,TARGET) returns the standard
%   deviations,RISK,and rates of return,ROR,that comprise the efficient frontier
%   of a given portfolio, plus the weights of each asset,WTS,for each point on
%   the frontier.  ASSET is an M-by-N matrix of time series data where each
%   column represents a single asset.  RET is a 1-by-N vector of the rates of
%   return for asset.  PTS specifies the number of efficient frontier points
%   to be calculated.  By default, PTS = 10.  TARGET specifies the desired rate
%   of return based on the ASSET and RET data.  When entering a target rate of
%   return, enter PTS as an empty matrix.  RISK and ROR are PTS-by-1 vectors,
%   and WTS is a PTS-by-(number of assets) matrix. 
% 
%   FRONTIER(ASSET,RET) plots the efficient frontier without returning any
%   data to the MATLAB workspace. 
%        
%   [RISK,ROR,WTS] = FRONTIER(ASSET,RET,PTS) returns the standard deviations,   
%   rates of return, and weights of each point on the efficient frontier. 
% 
%   [RISK,ROR,WTS] = FRONTIER(ASSET,RET,[],TARGET) returns the efficient 
%   frontier data associated with a specific rate of return on the frontier. 
% 
...
%       $Revision: 1.11 $   $Date: 2002/04/14 21:58:17 $ 
