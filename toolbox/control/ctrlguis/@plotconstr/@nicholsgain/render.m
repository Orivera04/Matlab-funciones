...
%   $Revision: 1.11 $ $Date: 2002/04/10 05:11:30 $
...
% Get margin info
Margin = Constr.MarginGain; % in dB
...
YData = linspace(-Margin, Margin, nP);  % Magnitude in dB
...
XLims = PlotAxes.Xlim;
XDataL = XData - (XLims(2) - XLims(1)) / 2 * Constr.Thickness;
XDataR = XData + (XLims(2) - XLims(1)) / 2 * Constr.Thickness;
...
set(Constr.HG.Markers(1), 'XData', XData(nP), 'YData', YData(nP), ...
    'Zdata', Constr.Zlevel);
set(Constr.HG.Markers(2), 'XData', XData(1),  'YData', YData(1), ...
    'Zdata', Constr.Zlevel);
