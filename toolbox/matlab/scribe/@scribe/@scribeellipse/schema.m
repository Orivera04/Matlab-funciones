function schema
%SCHEMA defines the scribe.SCRIBEELLIPSE schema
%
%  See also PLOTEDIT
...
h = schema.class(pkg, 'scribeellipse', hgPk.findclass('hggroup'));

p = schema.prop(h,'ShapeType','ScribeShapeType');
p.AccessFlags.PublicSet = 'off';
p.AccessFlags.Init = 'on';
p.FactoryValue = 'ellipse';
p.Visible = 'off';
...
p = schema.prop(h,'Ellipse','handle');
p.AccessFlags.Serialize = 'off';
p.Visible='off';
