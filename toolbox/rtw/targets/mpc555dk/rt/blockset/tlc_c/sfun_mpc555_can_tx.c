/*
 * File: sfun_mpc555_can_tx.c
 *
 * Abstract:
 *   
 *
 * $Revision: 1.7.2.5 $
 * $Date: 2004/04/19 01:30:21 $
 *
...
#define S_FUNCTION_NAME sfun_mpc555_can_tx 
...
enum { E_MODULE = 0, E_SAMPLE_TIME, E_TX_MODE, E_BUFFERS, E_FIFO_QUEUE_LENGTH, P_NPARMS };
...
#define TX_MODE_PRIORITY_QUEUE 1
#define TX_MODE_DIRECT 2
#define TX_MODE_FIFO 3
...
static void mdlSetInputPortDataType(SimStruct *S, int_T port, DTypeId dataType) {
    ...
    if ( port == 0 ) {
        if( isAcceptableDataType( S, dataType ) ) {
            ...
            ssSetErrorStatus(S,"The TouCAN Transmit block cannot be grounded or left unconnected. "
...
static void mdlInitializeSizes(SimStruct *S)
{
        ...
        if ( (P_TX_MODE == TX_MODE_PRIORITY_QUEUE) || (P_TX_MODE == TX_MODE_FIFO ) ) {
            /* Dynamically sized inport if using shared transmit buffer */
       ...
       if (P_TX_MODE == TX_MODE_FIFO) {
        ...
        if( !ssGetInputPortConnected(S, 0)){
            /* Do not allow the user to use this block in an
               unconnected state.
               */
            ssSetErrorStatus(S,"The input to this block must be connected to a non-virtual source.");
        }
