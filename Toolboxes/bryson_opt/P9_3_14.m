% Script p9_3_14.m; min time pick-and-place motion of two-link robot
...
%
figure(1); clf; subplot(411), plot([0 .5],[1 1],[.5 .5],[1 -1],'b',...
   [.5 1],[-1 -1],'b'); grid; axis([0 1 -1.2 1.2]); ylabel('u_e'); 
subplot(412), plot([0 t1],[-1 -1],[t1 t1],[-1 1],'b',[t1 .5],[1 1],...
   'b',[.5 .5],[1 -1],'b',[.5 1-t1],[-1 -1],'b',[1-t1 1-t1],[-1 1],...
   'b',[1-t1 1],[1 1],'b'); grid; axis([0 1 -1.2 1.2]); ylabel('u_s');
e=180/pi; subplot(413), plot(te/tf,e*the); grid;
ylabel('\theta_e (deg)'); subplot(414), plot(te/tf,e*ths); grid 
ylabel('\theta_s (deg)'); xlabel('t/tf')
%
figure(2); clf; subplot(211), plot(te/tf,ome-oms); grid;
ylabel('\omega_e-\omega_s'); subplot(212), plot(te/tf,oms); grid
ylabel('\omega_s'); xlabel('t/tf');
%
figure(3); clf; for i=1:length(te), 
 x1=[0 xe(i)]; y1=[0 ye(i)]; x2=[xe(i) xt(i)]; y2=[ye(i) yt(i)];
 plot(x1,y1,'b',x2,y2,'b',xt(i),yt(i),'ro',0,0,'ro'); 
 axis([-2 2 -3 1]); axis('square'); axis off; pause(.1); hold on
end; hold off 
