...
%    $Revision: 1.10.2.6 $  $Date: 2004/03/30 13:03:43 $
...
try
    ...
    nout=nargout;
    if (nout<=1)
        varargout{1}=daqmex(obj,'getdata',varargin);
    else
        [varargout{1:nout}]=daqmex(obj,'getdata',varargin);
    end
catch
    error('daq:getdata:unexpected', lasterr)
end
