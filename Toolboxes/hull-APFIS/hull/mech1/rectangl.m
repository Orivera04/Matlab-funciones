...
elseif strcmp(req,'comp') 
  result(1,1)=rectangl(b,h,'area');
  result(1,2)=rectangl(b,h,'circ');
  result(1,3)=rectangl(b,h,'centx');
  result(1,4)=rectangl(b,h,'centy');
  result(1,5)=rectangl(b,h,'ix');
  result(1,6)=rectangl(b,h,'iy');
elseif strcmp(req,'draw')
  xcentroid=rectangl(b,h,'centx');
  ycentroid=rectangl(b,h,'centy');
  X=[0 b b 0 0];
  Y=[0 0 h h 0];
  fill(X,Y,'r')
  hold on;
  ColA=strvcat('Area','Circumference','Centroid X','Centroid Y','Ix','Iy');
  ColB=makecol(rectangl(b,h,'comp')');
