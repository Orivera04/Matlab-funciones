num = 10;
den = [conv([0.3 1],[0.1 1]) 0];
w = logspace(-1,1);
[g p] = bode(num,den,w); g = 20*log10(g);
%
clf; sbplot(211);
axis([-210 -45 -10 40]); plot(p,g); grid; hold on; axis([-210 -45 -10 40]);
...
plot(nnp,nnm,'g--');
%
w=[.15 .4 1.5 2.5 3.5 7]; [g p]=bode(num,den,w); g=20*log10(g);
plot(p,g,'*');
for ii=1:length(w);
...
%
dm=crosser('back_lsh',[.8 .99],[],[],nm,dn);
n=back_lsh(dm); nnm=20*log10(abs(1/n)); nnp=imag(log(-1/n))*180/pi;
plot(nnp,nnm,'*');
w = polymag(nm,dn,abs(1/n));
ii = find(real(w) > 0); w = real(w(ii));
...
plot(nnp,nnm,'g--');
%
w=[.06 .15 .4 .8 1.5 2.3 4 6]; [g p]=bode(num,den,w); g=20*log10(g);
plot(p,g,'*');
for ii=1:length(w);
  text(p(ii),g(ii)-1,[' W=' num2str(w(ii))]);
end;
