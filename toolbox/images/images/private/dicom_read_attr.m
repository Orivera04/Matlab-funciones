function [attr, data, file] = dicom_read_attr(file, info, varargin)
%DICOM_READ_ATTR  Read the next attribute from a DICOM file.
%   [ATTRIBUTE, DATA] = DICOM_READ_ATTR(FILE, INFO) reads the
%   next ATTRIBUTE and its DATA from the currently open FILE.  INFO
%   should be the previously read metadata for the message.
%
%   [ATTRIBUTE, DATA] = DICOM_READ_ATTR(FILE, INFO, VR) reads
...
%   $Revision: 1.8.4.6 $  $Date: 2003/12/13 02:48:36 $
...
if (nargin == 3)
    ...
    [attr, data] = read_elt(file, info, VR);
    
else
    
    [attr, data] = read_elt(file, info);
...
elseif ((~isequal(attr.VR, 'SQ')) && ...
    ...
    data = read_items(file, info, attr.Length, attr.VR);
    return
...
elseif (isequal(attr.VR, 'SQ'))
    ...
    data = read_seq(file, info, attr.Length);
    return
...
function [attr, data] = read_elt(file, info, varargin)
...
try
    ...
    error('Images:dicom_read_attr:attrDetails', lasterr)
...
if (rem(attr.Length, 2) == 1)
    ...
    error('Images:dicom_read_attr:attrLength', err_msg);
...
if (~tf)
    ...
    msg = sprintf('Attribute (%04x,%04x) has inconsistent length %d.', ...
                  attr.Group, attr.Element, attr.Length);
    dicom_warn(msg);
...
if (~isempty(findstr(precision, 'char')))
    ...
    data = char(data');
    data = fliplr(my_deblank(fliplr(my_deblank(data))));
...
elseif (findstr(attr.Name, 'xx_Creator'))
    ...
    case {'AT'}
        ...
        % Some ACR-NEMA files stored only 1 value; try to be nice.
    ...
    if (~isempty(findstr('OverlayData', attr.Name)))
        ...
        if (isfield(info, ['NumberOfFramesInOverlay_' ol_num]))
            ol_frames = info.(['NumberOfFramesInOverlay_' ol_num]);
        else
            ol_frames = 1;
        end
        
        if ((ol_width * ol_height * ol_frames) > 0)
            
            % Overlays are padded to 16-bit boundaries, so there may be
            % some extraneous data.  Remove it.
            if (numel(data) < (ol_width * ol_height * ol_frames))
                
                dicom_close_msg(file);
                err_msg = sprintf('Attribute (%04x,%04x) has too few data elements: %d.', ...
                                  attr.Group, attr.Element, numel(data));
                error('Images:dicom_read_attr:notEnoughData', err_msg);

            elseif (numel(data) > (ol_width * ol_height * ol_frames))
                
                data = data(1:(ol_width * ol_height * ol_frames));
                
            end
            
            data = reshape(data, [ol_width, ol_height, ol_frames]);
...
function items = read_items(file, info, VR)
...
while (more_items)
    ...
    % Read the next item.
    [attr, data, file] = dicom_read_attr(file, info, VR);
...
function seq = read_seq(file, info, seq_length)
...
if (isequal(file.Location, 'Local'))
    ...
    file = dicom_close_msg(file);
    error('Images:dicom_read_attr:localOnly', 'Network access is not yet supported.');
...
case {'UN'}
    ...
    % Always return 'UN' type as uint8 for symmetry when writing and ease
    % of casting to correct type when more information is available.
    precision = 'uint8=>uint8';
    datasize = 1;
...
if (isempty(person_string))
    person_name = make_person(pn_parts);
    return
end

people = tokenize(person_string, '\\');  % Must quote '\' for calls to STRREAD.
...
function attr = find_attr_details(attr)
%FIND_ATTR_DETAILS  Get the attribute's name

attr_dict = dicom_dict_lookup(attr.Group, attr.Element);

if (isempty(attr_dict))
    if ((rem(attr.Group, 2) == 1) && (attr.Group < 65534))  % 0xFFFE == 65534.
        
        % Private attribute.  Try to handle it.
        %
        % Technically attribute groups 1, 3, 5, and 7 are not allowed.
        % Some manufacturers allow them, though, so let's try to read
        % them.
        ...
        error('Images:dicom_read_attr:unknownAttr', msg);
...
function person_struct = make_person(pn_parts)
%MAKE_PERSON  Make an empty struct containing the PN fields.

for p = 1:numel(pn_parts)
    person_struct.(pn_parts{p}) = '';
end
