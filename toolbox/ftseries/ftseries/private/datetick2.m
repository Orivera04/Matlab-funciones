function datetick2(a,b,c,xtic,timeData)
%DATETICK Date formatted tick labels. 
%
%   NOTE: THIS IS A MODIFIED VERSION OF THE DATETICK FUNCTION.
%   THIS FUNCTION SHOULD NOT BE USED OUTSIDE OF FTS TBX.
%
...
%   Author(s): C.F. Garvin, 4-03-95, Clay M. Thompson 1-29-96, P. Wang
%   Copyright 1984-2002 The MathWorks, Inc. 
%   $Revision: 1.2 $  $Date: 2002/01/18 21:50:49 $

% Set up some variables
a= cellstr(a); % tic axis
b = num2cell(1); % dateform
c = cellstr(c); % keeplimits

%[axh,nin,ax,dateform,keep_ticks,keep_limits] = parseinputs(varargin);
[axh,nin,ax,dateform,keep_ticks,keep_limits] = parseinputs([a b c]);
labels = [];
...
set(gca,[ax,'tick'],xtic,[ax,'ticklabel'],labels, ...
    [ax,'lim'],[min(xtic) max(xtic)])
ticks = xtic;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ##-Jan-#### hh:mm
if timeData
    labels1 = datestr(ticks,1);
    labels15 = datestr(ticks,15);
    labels = cellstr([labels1, repmat(' ',size(labels1,1),1), labels15]);
else
    labels = datestr(ticks,dateform);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
