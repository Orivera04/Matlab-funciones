/* File MWDSP_CIC_INT_LAT_TPLT.C
 *
 * CIC Interpolation Nonzero-Latency Filter algorithm template
 *
 * Copyright 1995-2003 The MathWorks, Inc.
 * $Revision: 1.1.6.2 $ $Date: 2003/07/10 19:50:28 $
 */
    ...
    for (chanCount = 0; chanCount < nChans; chanCount++) {
        ...
        for (frameCount = 0; frameCount < nFrames; frameCount++) {
            ...
            for (sampCount = 0; sampCount < R; sampCount++) {
                /* Compute first stage output here (note extra latency) */
                int32_T stageOutput = *statesTmp;

                /* Initialize first stage accumulator with present state */
                int32_T accIntegrators = *statesTmp;

                /* Upsample and add into first integrator stage accum */
                if (sampCount == phase) {
                    ...
                    stgInpWLsTmp++; /* No need to add zero - just increment the pointer */
                }
                ...
                *statesTmp++ = accIntegrators;
                ...
                for (stageCount = 1; stageCount < N; stageCount++)
                {
                    ...
                    accIntegrators = stageOutput + *statesTmp;
                    ...
                    /* Compute this stage output here (note extra latency) */
                    stageOutput = *statesTmp;

                    /* Update this stage state */
                    *statesTmp++ = accIntegrators;
                }

                /* Compute interpolated output value for this frame */
                *out = (MWDSP_CIC_OUTTYPE)(stageOutput >> finalOutShiftVal);
                out += ioStride;
