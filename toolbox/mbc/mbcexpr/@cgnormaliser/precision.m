...
%   $Revision: 1.2.2.2 $  $Date: 2004/02/09 07:14:12 $
...
case 'Signed Floating Point 4 Bytes'
   BP = single(get(T,'breakpoints'));
case 'Unsigned 8 bit integer';
   BP = uint8(get(T,'breakpoints'));
case 'Signed 8 bit integer';
   BP = int8(get(T,'breakpoints'));
case 'Unsigned 16 bit integer'
   BP = uint16(get(T,'breakpoints'));
case 'Signed 16 bit integer';
   BP = int16(get(T,'breakpoints'));
...
set(T,'matrix',{M,'Alter table by precision'});
