...
%
%
% [A,B,C,D] = SSDATA(SYS) は、状態空間モデル SYS に関する行列データ 
% A,B,C,D を抽出します。SYS が状態空間モデルでない場合、あらかじめ状態空間
% 表現に変換します。
%
...
%
% 同じ次数(状態の数)のSSモデルの配列の場合、SSDATA は、多次元配列 A,B,C,D を
% 出力します。 ここで、 A(:,:,k), B(:,:,k), C(:,:,k), D(:,:,k) は、k 番目の
% モデル SYS(:,:,k) の状態空間行列を与えます。
%
% 次数の異なるLTIモデルに対して、セル配列の中の可変サイズの行列 A,B,C, D次数
% を求めるためには、つぎの書式を利用してください。 
