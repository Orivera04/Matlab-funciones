function errWorst=look1_func_plot(xdata,ydata,funcStr,xMin,xMax,xdt,xscale,ydt,yscale,rndMode,varargin)
%LOOK1_FUNC_PLOT plot an ideal function and its lookup approximation. 
%
%ERRWORST = LOOK1_FUNC_PLOT( XDATA, YDATA,
%                                  FUNCSTR, XMIN, XMAX,
%                                  XDT, XSCALE,
%                                  YDT, YSCALE, RNDMETH)
...
%   LOOK1_FUNC_PLOT(xdata, ydata, 'sin(2*pi*x)', 0, 0.25, ...
...
ip = ip+1;
subplot(ipmax,1,ip)
plot( x_fine_grid, y_ideal_fine_grid, 'r-' ); 
hold on
stairs( x_fine_grid, y_fix_look_fine_grid, 'b-' ); % why not stairs?
hold off
title(titleStr);
xy=axis;
