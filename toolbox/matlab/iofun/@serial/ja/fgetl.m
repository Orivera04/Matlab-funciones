% FGETL   行の終端子なしでデバイスからデータの1行を読み込み
%
% TLINE=FGETL(OBJ) は、serial port オブジェクト OBJ に接続している
% デバイスからテキストの1行を読み込み、TLINE に出力します。出力データは、
% テキストのラインに終端子を含みません。終端子を含むには、FGETS を使用
% してください。
%   
% FGETL は、つぎの事柄の内の一つが生じるとブロックされます。:
...
%
% [TLINE,COUNT]=FGETL(OBJ) は、COUNT に読み込んだ値の番号を出力します。
% COUNT は、終端子を含みます。
%
% [TLINE,COUNT,MSG]=FGETL(OBJ) は、FGETL が完全にうまく機能しない場合、
...
%       delete(s);
%
% 参考 : SERIAL/FGETS, SERIAL/FOPEN, SERIAL/RECORD.
