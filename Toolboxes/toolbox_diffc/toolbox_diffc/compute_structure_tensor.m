...
if isfield(options, 'use_grad_renormalization')
    use_grad_renormalization = options.use_grad_renormalization;
else
    use_grad_renormalization = 0;
end
if isfield(options, 'use_tensor_renormalization')
    use_tensor_renormalization = options.use_tensor_renormalization;
else
    use_tensor_renormalization = 1;
end
...
h = build_gaussian_filter([m m],sigma1/(lambda*n),[n n]);
...
if ~use_anisotropic
    ...
    h = build_gaussian_filter([m m],sub*sigma2/(lambda*n),[n n]);
