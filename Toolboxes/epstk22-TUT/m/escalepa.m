...
%         startValue,valueStep,endValue,vForm,vVisible,fontSize,lineWidth,...
...
function longTicAngle=escalepa(epsFile,x,y,minRadius,maxRadius,angle1,angle2,...
  ...
  if (nargin~=19)
    eusage(...
   'longTicAngle=escalepa(epsFile,x,y,minRadius,maxRadius,angle1,angle2,startValue,valueStep,endValue,vForm,vVisible,fontSize,lineWidth,shortTicLength,longTicLength,nValuesMax,space,color)');
  end
  ...
  if rem(signOfDelta*valueStep,3)==0
    nTics=3;
  else
    nTics=5;
  end
  %start ticNo and offset
  nShortTics=rem(startValue,signOfDelta*valueStep)/valueStep*nTics;
  if nShortTics<0
    nShortTics=5+nShortTics;
  end
  if rem(nShortTics,1)>0
      i=fix(nShortTics)+1;
      ticOffset=(i-nShortTics)*valueStep/nTics;
  else 
    i=nShortTics; 
    ticOffset=0;
  end
