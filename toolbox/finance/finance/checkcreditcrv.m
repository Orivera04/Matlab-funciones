function CheckedCreditCurve = checkcreditcrv(InputCreditCurve, MinDate, MaxDate)
%CHECKCREDITCRV Checked Credit Curve from Input Credit Curve
%
...
%$Revision: 1.6 $   $Date: 2002/04/14 21:57:17 $ 
...
if (~isempty(InputCreditCurve))
     if (~isa(InputCreditCurve, 'struct'))
          error('Credit curve must be a structure!')
     end
else
     error('Credit curve cannot be empty!')
end
...
if (isfield(InputCreditCurve, 'CheckCreditFlag'))
     if (InputCreditCurve.CheckCreditFlag)
          CheckedCreditCurve = InputCreditCurve;
          return
     end
end


CheckedCreditCurve = checkstruct(InputCreditCurve, 'creditcurvestand');
CurveDates = makedatenumeric(CheckedCreditCurve.CurveDates);
CurveRates = CheckedCreditCurve.CreditRates;
...
CheckedCreditCurve.CurveDates = CurveDates;
CheckedCreditCurve.CreditRates = CurveRates;


%Set a flag indicating that the zero curve structure has been checked
CheckedCreditCurve = rmfield(CheckedCreditCurve, 'CSCheckFlag');

CheckedCreditCurve.CheckCreditFlag = 1;
