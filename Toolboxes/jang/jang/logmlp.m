...
for i = 1:max_epoch,
	% Forward pass
	X1 = logistic([X0 one]*W1);	% Output of layer 1 (hidden layer)
	X2 = logistic([X1 one]*W2);	% Output of layer 2 (output layer)
	diff = T - X2;	% error
	...
	dE_dX1 = dE_dX2.*(1-X2).*X2*W2(1:hidden_n,:)';	% dE/dX1
	...
	if debug==1,
		...
		for j = 1:length(W1(:));
			...
			new_X1 = logistic([X0 one]*new_W1);
			new_X2 = logistic([new_X1 one]*W2);
	...
	%	tmp = max(max(abs(dE_dW1-num_dE_dW1)));
		...
		for j = 1:length(W2(:));
			...
			new_X1 = logistic([X0 one]*W1);
			new_X2 = logistic([new_X1 one]*new_W2);
			new_diff = T - new_X2;	% error
			new_E = sum(sum(new_diff.^2));
