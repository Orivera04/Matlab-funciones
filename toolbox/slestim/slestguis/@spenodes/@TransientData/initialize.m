...
% $Revision: 1.1.6.8 $ $Date: 2004/04/11 00:39:49 $
...
if strcmp( hEvent.propertyName, 'MODEL_CHANGED' )
  ...
  % Send notification to TransientDataLeaf UDD objects.
...
try
  ...
  if isempty(this.Experiment)
    % Create experiment
    this.Experiment = ParameterEstimator.TransientExperiment( model );
  else
    % Update experiment
