...
%   $Revision: 1.4.4.4 $  $Date: 2004/04/04 03:29:44 $
...
msub= setcode(m.userdefined,Bnds,g,Tgt);

[msub,OK]= gui_globalmodsetup(msub);
if OK
	
	msub= xinfo(msub,xinfo(m));
	msub= yinfo(msub,yinfo(m));

    % set new coding (both levels of model)
	Tgt= repmat(recommendedTgt(msub),nfactors(m),1);
    m  = setcode(m,Bnds,g,Tgt);
    
    Tgt= gettarget(m);
	g(:)= {''};
	m.userdefined= setcode(msub,Tgt,g,Tgt);
