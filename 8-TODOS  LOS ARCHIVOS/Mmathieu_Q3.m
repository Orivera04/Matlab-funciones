function f1r=Mmathieu_Q3(kf,m,Q,x,varargin)
%
%     ==============================================================
...
bj2(k,kr)=besselk(fix(k)-1,u2(kr));
end;
%===================================================================
...
f1r(kr)=f1r(kr)+2*i/pi.*(-1).^(fix(ic)+1).*(fg(kr,fix(k)).*bj1(fix(k),kr).*bj2(fix(k),kr));
elseif(kd==2 | kd==3);
%%f1r(kr)=f1r(kr)+(-1).^(fix(ic)+fix(k)).*fg(fix(k)).*(bj1(fix(k)-1+1,kr).*bj2(fix(k)+1,kr)+(-1).^fix(kd).*bj1(fix(k)+1,kr).*bj2(fix(k)-1+1,kr));
f1r(kr)=f1r(kr)+2/pi.*(-1).^(fix(ic)+1).*fg(kr,fix(k)).*(bj1(fix(k),kr).*bj2(fix(k)+1,kr)+(-1).^fix(kd).*bj1(fix(k)+1,kr).*bj2(fix(k),kr));
else;
%%f1r(kr)=f1r(kr)+(-1).^(fix(ic)+fix(k)).*fg(fix(k)).*(bj1(fix(k)-1+1,kr).*bj2(fix(k)+1+1,kr)-bj1(fix(k)+1+1,kr).*bj2(fix(k)-1+1,kr));
f1r(kr)=f1r(kr)+2*i/pi.*(-1).^(fix(ic)).*fg(kr,fix(k)).*(bj1(fix(k),kr).*bj2(fix(k)+1+1,kr)-bj1(fix(k)+1+1,kr).*bj2(fix(k),kr));
end;
