function hgo=subsasgn(hgo,s,rhs)
% SUBSASGN method for HGOBJECTS
%
% HGO(n).pname allowed
...
if nsubs==1 && strcmp(s.type,'.')                             % HGO.Pname = 
   ...
   if isempty(fn)
      error('HGOBJECT:Subsasgn','PName is Ambiguous or Unknown.')
   else
      set(hgo.handles,fn,rhs);
   end
...
elseif nsubs==2 ...
   ...
   elseif length(s(1).subs)==1
      set(hgo.handles(s(1).subs{:}),fn,rhs);
   else
      error('HGOBJECT:Subsasgn','Invalid HGOBJECT Indexing Expression.')
   end
