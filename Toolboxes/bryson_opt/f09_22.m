% Script f09_22.m; F4 min time to climb using inverse optimization 
% with SVIC h>=0; (Vo,ho,gao) and (Vf,hf,gaf) specified; h in lc where
% lc=2W/(g*rho*S), m in mo, T in g*mo=Wo, V in sqrt(g*lc), t in
% sqrt(lc/g);                                           9/94,  3/24/02
% 
tic; Wo=41998; S=530; ep=3*pi/180; c=1600; rho=.002378; gr=32.2;
...
%
titl='F4 Min Time Climb, Mf=1.00, hf=20 km, gaf=0, h>=0';
figure(1); clf; subplot(211), plot(ti,Vi,ti,Vi,'.'); grid;  
ylabel('V (kft/sec)'); ; title(titl); 
subplot(212), plot(ti,gai,ti,gai,'.'); grid;
ylabel('\gamma (deg)'); xlabel('t/tf')
%
figure(2); clf; subplot(211), plot(tbi,alpi,tbi,alpi,'.');
grid; title(titl);  ylabel('\alpha (deg)');
subplot(212), plot(tbi,ngi,tbi,ngi,'.'); grid
ylabel('Load Factor'); xlabel('t/tf');
%         
figure(3); clf;  subplot(211), plot(tbi,Ti,tbi,Ti,'.'); 
grid; title(titl);  ylabel('Thrust/Wo'); 
subplot(212), plot(ti,hi,ti,hi,'.'); grid;axis([0 1 0 70]);
ylabel('Altitude (kft)'); xlabel('t/tf');
%
figure(4); clf; plot(xi,hi,xi,hi,'.'); grid;
axis([0 450 0 70]);  xlabel('x (kft)');
ylabel('h (kft)'); title(titl); 
%
figure(5); clf; plot(Vi,hi,Vi,hi,'.',Vi(2*N+1),hi(2*N+1),'o'); 
grid; hold on; plot(Vi(1),hi(1),'o'); hold off; axis([0 2 0 70]);
xlabel('V (kft/sec)'); ylabel('h (kft)'); 
print -deps2 \book_do\figures\f09_22
