/* $Revision: 1.2 $ $Date: 2002/06/06 15:56:40 $ */
/* xpcbyte2any.c - xPC Target, non-inlined S-function driver for *
 *                 byte-unpacking                                */
/* Copyright 1994-2002 The MathWorks, Inc. */

#define         S_FUNCTION_LEVEL        2
#undef          S_FUNCTION_NAME
#define         S_FUNCTION_NAME         memscramnetinit

#include        <stddef.h>
#include        <stdlib.h>

#include        "simstruc.h"

#ifdef          MATLAB_MEX_FILE
#include        "mex.h"
#endif

#ifndef         MATLAB_MEX_FILE
#include        <windows.h>
#include        "io_xpcimport.h"
#include        "pci_xpcimport.h"
#endif

/* Input Arguments */
#define NUMBER_OF_ARGS          (3)
#define CSRS_ARG				ssGetSFcnParam(S, 0)
#define ACRAM_ARG				ssGetSFcnParam(S, 1)
#define SLOT_ARG				ssGetSFcnParam(S, 2)

#define SAMP_TIME_IND           (0)
#define BASE_ADDR_IND           (0)

#define NO_I_WORKS              (1)
#define BASE_ADDR_I_IND 		(0)

#define NO_R_WORKS              (0)

#define NO_P_WORKS              (0)

#define CSR0 					0x400001
#define CSR1 					0x400080
#define CSR2 					0x400103
#define CSR3 					0x400182
#define CSR4 					0x400205
#define CSR5 					0x400284
#define CSR6 					0x400307
#define CSR7 					0x400386
#define CSR8 					0x400409
#define CSR9 					0x400488
#define CSR10 					0x40050B
#define CSR11 					0x40058A
#define CSR12 					0x40060D
#define CSR13 					0x40068C
#define CSR14 					0x40070F
#define CSR15 					0x40078E

static char_T msg[256];

static int INIT=1;

static void mdlInitializeSizes(SimStruct *S)
{

	#ifndef MATLAB_MEX_FILE
	#include "io_xpcimport.c"
	#include "pci_xpcimport.c"
	#endif

    ssSetNumSFcnParams(S, NUMBER_OF_ARGS);
    if (ssGetNumSFcnParams(S) != ssGetSFcnParamsCount(S)) {
        sprintf(msg,"Wrong number of input arguments passed.\n"
                "%d arguments are expected\n",NUMBER_OF_ARGS);
        ssSetErrorStatus(S,msg);
        return;
    }

    ssSetNumContStates(S, 0);
    ssSetNumDiscStates(S, 0);


	ssSetNumInputPorts(S, 0);
   
    ssSetNumOutputPorts(S, 0);
        
    ssSetNumSampleTimes(S, 1);

    ssSetNumRWork(S, NO_R_WORKS);
    ssSetNumIWork(S, NO_I_WORKS);
    ssSetNumPWork(S, NO_P_WORKS);

    ssSetNumModes(S, 0);
    ssSetNumNonsampledZCs(S, 0);

    ssSetOptions(S, SS_OPTION_EXCEPTION_FREE_CODE | SS_OPTION_PLACE_ASAP);

}


static void mdlInitializeSampleTimes(SimStruct *S)
{
    ssSetSampleTime(S, 0, INHERITED_SAMPLE_TIME);
    ssSetOffsetTime(S, 0, FIXED_IN_MINOR_STEP_OFFSET);
}

#define MDL_START
static void mdlStart(SimStruct *S)
{

#ifndef MATLAB_MEX_FILE


	PCIDeviceInfo pciinfo;
    void *Physical1;
    void *Virtual1;
	void *Physical0;
    void *Virtual0;
    volatile unsigned long  *ioaddress32;
	volatile unsigned long  *ioaddress32pci;
	volatile unsigned short *ioaddress16;
	volatile unsigned char  *ioaddress8;
    char devName[20];
    int  devId;
	uint16_T errors;
	int_T i;

	strcpy(devName,"Scramnet+ SC150");
	devId=0x4750;
	  
    if ((int_T)mxGetPr(SLOT_ARG)[0]<0) {
        /* look for the PCI-Device */
        if (rl32eGetPCIInfo((unsigned short)0x11B0,(unsigned short)devId,&pciinfo)) {
        	sprintf(msg,"%s: board not present", devName);
            ssSetErrorStatus(S,msg);
            return;
        }
    } else {
        int_T bus, slot;
        if (mxGetN(SLOT_ARG) == 1) {
            bus = 0;
           	slot = (int_T)mxGetPr(SLOT_ARG)[0];
        } else {
            bus = (int_T)mxGetPr(SLOT_ARG)[0];
            slot = (int_T)mxGetPr(SLOT_ARG)[1];
        }
        // look for the PCI-Device
        if (rl32eGetPCIInfoAtSlot((unsigned short)0x11B0,(unsigned short)devId,(slot & 0xff) | ((bus & 0xff)<< 8),&pciinfo)) {
        	sprintf(msg,"%s (bus %d, slot %d): board not present",devName, bus, slot );
            ssSetErrorStatus(S,msg);
            return;
        }
    }


    // show Device Information
    //rl32eShowPCIInfo(pciinfo);
   
    Physical1=(void *)pciinfo.BaseAddress[1];
    Virtual1 = rl32eGetDevicePtr(Physical1, 0x1000000, RT_PG_USERREADWRITE);
    ioaddress32=(void *)Physical1;
	ioaddress16=(void *)Physical1;
    ioaddress8=(void *)Physical1;

	Physical0=(void *)pciinfo.BaseAddress[0];
    Virtual0 = rl32eGetDevicePtr(Physical0, 0x100, RT_PG_USERREADWRITE);
    ioaddress32pci=(void *)Physical0;

	//ioaddress32pci[0x13]=0x80000000;
	//printf("%x\n",ioaddress32pci[0x13]);

	

    ssSetIWorkValue(S, BASE_ADDR_I_IND, (uint_T)ioaddress32);

	if (INIT) { 

		uint32_T k=1;

		printf("Scramnet+ SC150 init ...\n");

		/* initialize ACRAM */

		ioaddress16[CSR0]=0x0010;

		for (i=0; i < (int_T)mxGetPr(ACRAM_ARG)[0]; i++) {

			uint32_T n=(uint32_T)mxGetPr(ACRAM_ARG)[k];
			uint32_T address=(uint32_T)mxGetPr(ACRAM_ARG)[k+1]/4;
			uint32_T k1;

			for (k1=0; k1<n; k1++) {
				ioaddress32[address+k1]= (uint32_T)mxGetPr(ACRAM_ARG)[k+2+k1];
				//printf("%x ",(uint32_T)mxGetPr(ACRAM_ARG)[k+2+k1]);
			}
			//printf("\n");

			k+=n+2;

		}

		ioaddress16[CSR0]=0x0000;

		/* setup interface */

		ioaddress16[CSR0]=0xF000;
		ioaddress16[CSR0]=(uint16_T)mxGetPr(CSRS_ARG)[0];
		ioaddress16[CSR2]=(uint16_T)mxGetPr(CSRS_ARG)[2];	
		ioaddress16[CSR3]=(uint16_T)mxGetPr(CSRS_ARG)[3];	
		ioaddress16[CSR5]=(uint16_T)mxGetPr(CSRS_ARG)[5];	
		ioaddress16[CSR8]=(uint16_T)mxGetPr(CSRS_ARG)[8];
		ioaddress16[CSR9]=(uint16_T)mxGetPr(CSRS_ARG)[9];
		ioaddress16[CSR12]=(uint16_T)mxGetPr(CSRS_ARG)[12];

		ioaddress16[CSR1]=0x0000;
		errors=ioaddress16[CSR1];
		errors=ioaddress16[CSR1];
		printf("CSR1 register after init: %x\n",errors);

		//ioaddress16[CSR0]= 0x0100 | (uint16_T)mxGetPr(CSRS_ARG)[0];

		INIT=1;

	}

    
#endif

}





static void mdlOutputs(SimStruct *S, int_T tid)
{

}

static void mdlTerminate(SimStruct *S)
{
}


#ifdef MATLAB_MEX_FILE  /* Is this file being compiled as a MEX-file? */
#include "simulink.c"   /* Mex glue */
#else
#include "cg_sfun.h"    /* Code generation glue */
#endif
