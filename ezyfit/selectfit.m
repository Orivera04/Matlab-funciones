...
%   selected from the active curve by drawing a polygon. Use the left mouse
%   button to draw the polygon. Double click or right-click to close the
%   polygon. Use UNDOFIT or RMFIT to remove the fit. The default display
%   settings are defined in the file FITPARAM.
%
...
%   content as the fit structure returned by FIT (see FIT for details),
...
%      editcoeff(f);
%
%   See also SHOWFIT, FIT, UNDOFIT, RMFIT, PLOTSAMPLE, SELECTDATA, INTERP1.

%   F. Moisy, moisy_at_fast.u-psud.fr
%   Revision: 2.20,  Date: 2006/10/18
...
if ischar(fp.fitcolor) || length(fp.fitcolor)==3
    ...
    if ishandle(f.hdata)
        ...
        if isfield(co,'Color')
            ...
            fitcolor=[0 0 0];   % hist data do not have Color property
        end;
    else
        fitcolor=[0 0 0];
        ...
        switch f.name
            ...
            otherwise
                ...
                if nargin<=1,
                    ...
                    fout=fit(xs,ys, fun, varargin{2:end});
                end;
                fout.hdata = f.hdata;
