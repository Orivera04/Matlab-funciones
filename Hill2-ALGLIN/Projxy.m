function projxy(u)                              %last updated 4/23/02
...
drawnow
hold on
plot(0,0,'*w',[0 xax2(1)],[0 xax2(2)],'-k','erasemode','none')
text(xax2(1),xax2(2),'X','erasemode','none','fontweight','bold')
drawnow
plot([0 yax2(1)],[0 yax2(2)],'-k','erasemode','none')
text(yax2(1),yax2(2),'Y','erasemode','none','fontweight','bold')
drawnow
plot([0 zax2(1)],[0 zax2(2)],'-k','erasemode','none')
text(.7*zax2(1),.7*zax2(2),'Z','erasemode','none','fontweight','bold')
drawnow
tic;while toc<2,end
plot([0 u2(1)], [0 u2(2)],'-g','erasemode','none','linewidth',2)
text(u2(1)/2,u2(2)/2,'U','color','black',...
        'erasemode','none','fontweight','bold')
...
hold off
%THE SECOND FRAME
if FS=='N';subplot(2,2,2),else,clf,end
%Next we draw axes from vector ax in black to set size of
%of axes for future drawing
plot([ax(1) ax(2)],[0 0],'-k',[0 0],[ax(3) ax(4)],'-k','erasemode','none')
axis(axis)
...
drawnow
tic;while toc<2,end
title('Projecting U onto X & Y axes.','fontweight','bold')
tic;while toc<2,end
%plotting the projections on to the x & y axes
plot([u2(1) u(1)*xaxp(1)],[u2(2) u(1)*xaxp(2)],':b','erasemode','none')
drawnow
tic;while toc<2,end
plot([0 u(1)*xaxp(1)],[0 u(1)*xaxp(2)],'-b','linewidth',2,'erasemode','none')
drawnow
tic;while toc<2,end
plot([u2(1) u(2)*yaxp(1)],[u2(2) u(2)*yaxp(2)],':r','erasemode','none')
tic;while toc<2,end
plot([0 u(2)*yaxp(1)],[0 u(2)*yaxp(2)],'-r','linewidth',2,'erasemode','none')
pause(4)
goon=text(.2,-.1,cont,'units','normalized','erasemode','none');
pause
set(goon,'string',' ');
...
hold off
%THE THIRD FRAME
if FS=='N',subplot(2,2,3),else,clf,end
%Next we draw axes from vector ax in black to set size of
%of axes for future drawing
plot([ax(1) ax(2)],[0 0],'-k',[0 0],[ax(3) ax(4)],'-k','erasemode','none')
axis(axis)
...
plot([0 u(2)*yaxp(1)],[0 u(2)*yaxp(2)],'-r','linewidth',2,'erasemode','none')
drawnow
title('Projection of U as lin. comb.','fontweight','bold')
tic;while toc<2,end
...
plot([u(1)*xaxp(1) p(1)],[u(1)*xaxp(2) p(2)],':k','erasemode','none')
drawnow
tic;while toc<2,end
plot([u(2)*yaxp(1) p(1)],[u(2)*yaxp(2) p(2)],':k','erasemode','none')
drawnow
tic;while toc<2,end
plot([0 p(1)],[0 p(2)],'-m','linewidth',2,'erasemode','none')
text(p(1)/2,p(2)/2,'PROJ','erasemode','none','fontweight','bold')
drawnow
tic;while toc<2,end
pause(4)
goon=text(.2,-.1,cont,'units','normalized','erasemode','none');
...
hold off
%THE FOURTH FRAME
if FS=='N';subplot(2,2,4),else,clf,end
%Next we draw axes from vector ax in black to set size of
%of axes for future drawing
plot([ax(1) ax(2)],[0 0],'-k',[0 0],[ax(3) ax(4)],'-k','erasemode','none')
axis(axis)
...
plot([0 p(1)],[0 p(2)],'-m','linewidth',2,'erasemode','none')
text(p(1)/2,p(2)/2,'PROJ','fontweight','bold','erasemode','none')
drawnow
tic;while toc<2,end
title('Drawing the orthogonal projector!','fontweight','bold')
tic;while toc<2,end
plot([u2(1) p(1)],[u2(2) p(2)],'--k','erasemode','none')
drawnow
or=(p3+u)*(.5);  %finding position for word ORTH
or=[or 1]*T;     %half way up the perpendicular
text(or(1),or(2),'S','erasemode','none','fontweight','bold')
drawnow
pause(2)
goon=text(.2,-.1,cont,'units','normalized','erasemode','none');
pause
