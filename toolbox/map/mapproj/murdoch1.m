function [out1,out2,savepts] = murdoch1(mstruct,in1,in2,object,direction,savepts)

%MURDOCH1  Murdoch I Conic Projection
%
%  This is an equidistant projection which is nearly minimum-error.  Scale is
%  true along any meridian and is constant along any parallel.  Scale is also
...
if abs(diff(parallels)) <= epsilon;   parallels(1) = parallels(1) + epsilon;  end
...
n = cos(add);    m = tan(add) * (sin(sub)/sub - add);
