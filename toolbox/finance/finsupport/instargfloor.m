function [Strike, Settle, Maturity, FloorReset, Basis, Principal] = instargfloor(varargin)
%INSTARGFLOOR Subroutine for 'Type','Floor' argument validation.  
%   This function is called at the top of processing routines.
%
%   [Strike, Settle, Maturity, Reset, Basis, Principal] = instargfloor(ArgList{:})
%
%   Inputs: 
...
%   $Revision: 1.10 $  $Date: 2002/04/14 21:39:43 $
...
[FieldList, ClassList, TypeString, SizeList, DefDataList] = instfloor;
...
[Strike, Settle, Maturity, FloorReset, Basis, Principal] = deal(ArgCells{:});
...
if any(isnan(Strike))
  error('Strike is required for all Floor instruments')
end

if any(isnan(Settle))
  error('Settle is required for all Floor instruments')
end

if any(isnan(Maturity))
  error('Maturity is required for all Floor instruments')
end

% Settle <= Maturity
if any( Settle > Maturity )
  error('Settle must be less than or equal to Maturity for all Floors')
end
...
if( any(FloorReset ~= 1 & FloorReset ~= 2 & FloorReset ~= 3 & FloorReset ~= 4 & FloorReset ~= 6 & FloorReset ~= 12))
