function [board, boardType, devName, reset, initValue, maskdisplay, maskdescription]= mdaueipd2aox(boardtype, channel, reset, initValue, pciSlot)
...
maskdescription=[description,10,'United Electronic Industries',10,'Analog Output'];
...
% check for channel elements smaller than 1 or greater than maxDAChannel
if ~isempty(find(channel<1)) | ~isempty(find(channel>maxDAChannel))
  error(['Channel Vector elements must be in the range 1..',num2str(maxDAChannel)]);
end
