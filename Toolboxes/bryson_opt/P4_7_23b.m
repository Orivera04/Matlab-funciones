% Script p4_7_23b.m; min time climb, A/C w. parabolic lift-drag polar
% using FOPTB; s=[V ga h x]'; al=control; V in sqrt(g*l), alm=1/12;
% l=2m/(rho*S*Cla); eta=1/2; T=.2; Vo=Vf=7, ga0=gaf=0; hf=11.3;
%                                                  4/97, 3/28/02
%
sf=[7 0 11.8020 150.7879]; name='climbt'; nu=[-17.4749 -7.2864 ...
      -2.1121]; tf=30.0; p0=[sf nu tf]; s0=[7 0 0 0]';
optn=optimset('Display','Iter','MaxIter',50);
p=fsolve('foptb',p0,optn,name,s0);
[f,t,y]=foptb(p,name,s0); eta=.5; c=180/pi; V=y(:,1); ga=y(:,2);
h=y(:,3); x=y(:,4); lv=y(:,5); lg=y(:,6); al=lg./(2*eta*V.*lv); 
es=4.630^2/2; ke=V.^2/2; N1=length(t); tf=t(1);
%
