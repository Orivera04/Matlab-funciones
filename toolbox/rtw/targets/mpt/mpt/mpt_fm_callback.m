function mpt_fm_callback(operation, value)
...
switch (operation)
    ...
    case 'file_select'
        ...
        if value >= 1
            fm.functionList=[];
            for i=1:length(fm.fileFunList{value}.function)
                fm.functionList{i}=fm.fileFunList{value}.function{i}.name;
            end
            set(fm.lh2,'String',fm.functionList,...
                'Value',1);
            mpt_fm_callback('fun_select', 1);
        end
...
fileName = fm.fileFunList{fileSelValue}.name;
funName = fm.fileFunList{fileSelValue}.function{funSelValue}.name;
