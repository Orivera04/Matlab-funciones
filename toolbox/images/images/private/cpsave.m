...
%   $Revision: 1.6.2.5 $  $Date: 2003/08/23 05:53:38 $
...
if ~isa(inputBasePairsVector,'java.util.Vector')
    ...
    error(eid,'%s',eid,'%s','inputBasePairsVector must be a java.util.Vector.')
end

% varNames should be a cell array of strings
if ~iscell(varNames)
    eid = sprintf('Images:%s:varNamesNotCellArray',mfilename);                
    error(eid,'%s','varNames must be a cell array.')
end
