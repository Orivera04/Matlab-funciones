/*
 * acf_fft_interleave_zpad_d_rt.c - Signal Processing Blockset ACF run-time library components.
 *
 *  Copyright 1995-2003 The MathWorks, Inc.
 *  $Revision: 1.3.2.3 $  $Date: 2004/04/12 23:40:36 $
 */

#include "dsp_rt.h"

/*
 * Identical to fft_interleave_d_rt.c except that this one allows for zero padding
    ...
    )
{
     ...
     */
    if (nChans & 0x01) {
        real_T *rbuff = (real_T *)buff;
        int_T j = (N - inRows);

        memcpy(rbuff, inPtr, inRows * sizeof(real_T));
        rbuff += inRows;
        while (j--) {
            *rbuff++ = 0.0;
        }        
...
/* [EOF] acf_fft_interleave_zpad_d_rt.c */
