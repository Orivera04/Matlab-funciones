...
% METHODS - methods for textarrow class
...
subfunctions = {'mouseover','postdeserialize','updateXYData', ...
                'ispointon','beginedit','tbdown','setselected',...
                'endedit'};
...
if ~isempty(h.Text) && ishandle(h.Text)
  pos = [PX(1) PY(1)]./ppos(3:4);
  set(h.Text,'units','points');
  ext = get(h.Text,'Extent');
  ext = ext(3:4)./ppos(3:4);
  set(h.Text,'units','data');
  if strcmp(get(h,'VerticalAlignmentMode'),'auto')
    if abs(theta-(-pi/2)) < pi/3
      set(h.Text,'VerticalAlignment','bottom');
    elseif abs(theta-(pi/2)) < pi/3
      set(h.Text,'VerticalAlignment','top');
    else
      set(h.Text,'VerticalAlignment','middle');
    end
  end
  if abs(theta) < pi/3
    offset = -ext(1)/2;
  ...
  pos(1) = pos(1) + offset;
  switch get(h.Text,'HorizontalAlignment')
   case 'right'
    offset = ext(1)/2;
   case 'left'
    offset = -ext(1)/2;
...
lineMethods(h,'setselected',onoff);
...
set(h.Text,'Editing','off');
str = get(h.Text,'String');
% % remove special characters
% str = deblank(get(h.Text,'String'));
