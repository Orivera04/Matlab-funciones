...
%   clossenes. If C='levels' (default) then the first criteria is branch
...
% $Revision: 1.1.6.5 $ $Author: batserve $ $Date: 2004/03/09 16:15:23 $
           ...
           switch(k)
               case 1  % reference
                   if islogical(pval)
                       ...
                       pval = find(pval);
                       ReferenceIs = 'node';
                       if (numel(pval) ~= 1) && all(pval~=1:numLabels)
                           error('Bioinfo:IncorrectReferenceNode',...
                               'Incorrect reference node')
                       else
                           ReferenceNode = pval;
                   ...
                   elseif isnumeric(pval)
                       ...
                       else error('Bioinfo:InvalidReference',...
                             'Invalid string for reference.');
                       end
                   end
                case 2  % criteria
                   ...
                   else error('Bioinfo:InvalidCriteria',...
                              'Invalid string for criteria.');
                   end
               case 3 % threshold
                   V = pval;
                   if (~isnumeric(V) || ~isreal(V) || numel(V)>1)
                       error('Bioinfo:InvalidThreshold',...
                             'Invalid value for V.');  
                   end
               case 4 % exclude
                   ...
                   if numel(h)
                       ...
                       else error('Bioinfo:InvalidExcludeOption',...
               ...
               case 5 % propagate
                   ...
                   if numel(h)
                       ...
                       else error('Bioinfo:InvalidPostPropagate',...
...
switch ReferenceIs
    ...
    case 'node'
        ...
        dist2Ref = pdist(phytree(tr),'SquareForm',true,'nodes','all')...
                   * refVector;
        tr.dist = ones(numLabels,1); % to count now levels !
        levels2Ref = pdist(phytree(tr),'SquareForm',true,'nodes','all') ...
                   * refVector;
end
