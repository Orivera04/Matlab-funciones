% Script p5_3_21.m; helicopter acceleration from hover (9th order order
% model of OH-6A); xdot=Ax+Bd; x=[u w q theta v r p phi psi]'; d=[de dc
% da dr]'; x in ft, sec, crad, d in deci-in;              11/90, 7/3/02 
%
...
Q=1*diag([0 0 0 0 0 0 0 0 1]); N=zeros(9,4); R=eye(4);
Qf=1e3; Mf=eye(9); x0=zeros(9,1); tf=3; Ns=40; Ts=tf/Ns;
psi=[10 0 0 0 0 0 0 0 0]'; [Ad,Bd,Qd,Nd,Rd]=cvrtj(A,B,Q,N,R,Ts);
[x,u]=tdlqs(Ad,Bd,Qd,Nd,Rd,x0,Mf,Qf,psi,Ts,Ns);
uh=[u u(:,Ns)]; t=tf*[0:1/Ns:1];
%
figure(1); clf; subplot(211), plot(t,x([1 2 5 6],:)); grid
axis([0 3 -3 10]); text(1.25,7,'u (ft/sec)'); text(1.7,1,'v (ft/sec)')
text(2.1,-1,'w (ft/sec)'); text(.9,1,'r (crad/sec)'); subplot(212)
zohplot(t,uh); grid; axis([ 0 3 -8 12]); xlabel('Time (sec)')
ylabel('deci-inches'); text(.3,-4.5,'\delta a') 
text(.3,-1.5,'\delta e'); text(.3,1.5,'\delta r') 
text(.3,9,'\delta c')
%
figure(2); clf; subplot(211), plot(t,x([4 8 9],:)); grid
xlabel('Time (sec)'); ylabel('crad'); text(1.55,-3,'\psi')
