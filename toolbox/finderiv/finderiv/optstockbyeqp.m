function [Price, PriceTree] = optstockbyeqp(BinStockTree, varargin)
%OPTSTOCKBYEQP Price options on stocks by an EQP binomial tree.
%
...
%   $Revision: 1.1.6.1 $  $Date: 2003/08/29 04:46:48 $

%-------------------------------------
%Checking input arguments
%-------------------------------------
if nargin<5 
    str1 = 'The function optstockbyeqp requires five input arguments:';
    str2 = 'EQPTree, OptSpec, Strike, Settle and ExerciseDates';
    msg = sprintf('%s\n%s', str1, str2);
    error('finderiv:optstockbyeqp:InvalidInputs',msg)
end

if ~isafin(BinStockTree, 'BinStockTree') | ~strcmpi(BinStockTree.Method, 'EQP')
    error('finderiv:optstockbyeqp:InvalidTree','EQPTree must be a Binary Stock Tree created with EQPTREE')
end
