/*
 * ifft_dbllen_trig_cbr_cbr_oop_rt.c - Signal Processing Blockset FFT run-time library components.
 *
 *  Copyright 1995-2003 The MathWorks, Inc.
 *  $Revision: 1.5.2.3 $  $Date: 2004/04/12 23:44:11 $
 */
 ...
 *
 */
EXPORT_FCN void MWDSP_Ifft_DblLen_TRIG_Cbr_Cbr_Oop(
    creal32_T       *y,
    const creal32_T *x,
    const int_T      fftLen
    )
{
    const int_T N2=((uint_T)fftLen)>>1;
    const int_T N4=((uint_T)N2)>>1;
...

    {
        ...
        if (N2 > 1) {
             ...
             */
            theta = (real32_T)DSP_TWO_PI / (real32_T)fftLen;
            W.re  = cosf(theta);
            W.im  = sinf(theta);
...

        {
            ...
            for (i=2; i<N4; i++) {
                ...
                theta = (real32_T)DSP_TWO_PI * ((real32_T)i/(real32_T)fftLen);
                W.re  = cosf(theta);
                W.im  = sinf(theta);
...
/* [EOF] ifft_dbllen_trig_cbr_cbr_oop_rt.c */
