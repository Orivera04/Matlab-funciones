...
% $Revision: 1.1.6.5 $ $Date: 2004/04/11 00:42:28 $
...
for ct = 1:length(this)
  ...
  if strcmp(h.PortType, 'Signal')
    fprintf('\n(%d) Frequency-domain data for output port %d of %s block:\n', ...
            ct, h.PortNumber, LocalSetHtmlLine(Block) );
  else
    fprintf('\n(%d) Frequency-domain data for %s block %s:\n', ...
            ct, h.PortType, LocalSetHtmlLine(Block) );
  end
  
  if ~isnan(h.Fs)
    fprintf('\n    Sampling frequency: %g rad/sec.\n', h.Fs);
  else
