...
void validateInputs(int nrhs, const mxArray *prhs[])
{
    if (nrhs != 2)
    {
        mexErrMsgIdAndTxt("map:ind2rgb8:wrongNumInputs",
                          "IND2RGB8 expected two input arguments.");
    }

    if (!mxIsDouble(prhs[0]) && !mxIsUint8(prhs[0]) && !mxIsUint16(prhs[0]))
    {
        mexErrMsgIdAndTxt("map:ind2rgb8:invalidInputType",
                          "X must be uint8, uint16, or double.");
    }

    if (mxGetNumberOfDimensions(prhs[0]) != 2)
    {
        mexErrMsgIdAndTxt("map:ind2rgb8:inputNot2D",
                          "X must be two-dimensional.");
    }

    if (mxIsSparse(prhs[0]))
    {
        mexErrMsgIdAndTxt("map:ind2rgb8:sparseInput",
    ...
    if ((mxGetNumberOfDimensions(prhs[1]) != 2) ||
        (mxGetN(prhs[1]) != 3))
    {
        mexErrMsgIdAndTxt("map:ind2rgb8:invalidMapSize",
                          "CMAP must be a P-by-3 matrix.");
    }
    
    if (!mxIsDouble(prhs[1]))
    {
        mexErrMsgIdAndTxt("map:ind2rgb8:invalidMapType",
                          "CMAP must be double.");
    }

    if (mxIsSparse(prhs[1]))
    {
        mexErrMsgIdAndTxt("map:ind2rgb8:sparseMap",
                          "CMAP must not be sparse.");
    }

    if (mxGetM(prhs[1]) < 1)
    {
        mexErrMsgIdAndTxt("map:ind2rgb8:emptyMap",
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    switch (mxGetClassID(prhs[0]))
    {
      ...
      default:
        mexErrMsgIdAndTxt("map:ind2rgb8:unexpectedType",
                          "Invalid input type.");
    }
