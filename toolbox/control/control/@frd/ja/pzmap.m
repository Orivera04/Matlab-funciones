...
% 
% PZMAP(SYS1,SYS2,...) は、複数のLTIモデル SYS1,SYS2,...の極と零点を
% 一つのプロット図上に表示します。ユーザは、各モデルに対して、別々な
% カラーをつぎのようにして設定できます。
% 
%   pzmap(sys1,'r',sys2,'y',sys3,'g')   
% 
% 左辺に出力引数を指定した場合、[P,Z] = PZMAP(SYS) は、システムの極と零
% 点を列ベクトル P と Z で出力します。画面にはプロットしません。 
%
% 関数 SGRID または ZGRID を用いて、減衰率および固有周波数をs平面または
% z平面にプロットすることができます。
%
% LTIモデルの配列 SYS に対して、PZMAP は配列の各モデル毎の極と零点を
% 同じダイアグラム上にプロットします。
%
