function handle = cgsl2exprgetpriorblock(b,outportnum)
% Returns destination block regardless of subsystems

%  Copyright 2000-2004 The MathWorks, Inc. and Ford Global Technologies, Inc.

%   $Revision: 1.3.6.2 $  $Date: 2004/02/09 07:17:40 $
if nargin==1
    outportnum=1;
end
...
if length(handle)==1
    ...
    case 'SubSystem'
        if ~cgsl2expristabletype(handle)
            ...
            handle = cgsl2exprgetpriorblock(subsysinport,1);
        end
    case 'Outport'
        ...
        if ~isempty(parent)
            if ~strcmp(get_param(parent,'type'),'block_diagram');
                ...
                handle=cgsl2exprgetpriorblock(parent,str2num(portnum));   
            end
        end
    end
end
