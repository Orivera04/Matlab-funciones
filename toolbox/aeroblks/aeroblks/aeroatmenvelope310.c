/* $Revision.2 $ */
/*
 * File: aeroatmenvelope310.c
 *
 * Abstract:
 *
 *   Functions to calculate Non-Standard atmospheric 
 *   condition implemented from MIL-HDBK-310.  Given 
...
/* Function: InitCalcAtmosEnvelope310 ======================================
 * Abstract:
 *   initialize tables for selected non-standard day envelope
 *
 */
void InitCalcAtmosEnvelope310( SFcnCache *udata,
                               double altitude310[],
                               double temperature310[],
                               double density310[],
                               double pressure310[] )
{
        ...
        switch (udata->epercent) {
          case EXTREME:
            ...
            for ( i = 0; i < NUMENVELOPE1PTS; i++ )
                {
                    temperature310[i] = hte[i];
                    density310[i] = htde[i];
                    altitude310[i]=altitude1[i];
		    udata->numpts = NUMENVELOPE1PTS - 1;
		    udata->lower_alt = altitude310[0];
		    udata->upper_alt = altitude310[NUMENVELOPE1PTS - 1];
               } 
              }
            break;
          case P1:
            ...
            for ( i = 0; i < NUMENVELOPE2PTS; i++ )
                {
                    temperature310[i] = ht1p[i];
                    density310[i] = htd1p[i];
                    altitude310[i]=altitude2[i];
		    udata->numpts = NUMENVELOPE2PTS - 1;
		    udata->lower_alt = altitude310[0];
		    udata->upper_alt = altitude310[NUMENVELOPE2PTS - 1];
                }
              }
            break;
          case P5:
          ...
          case P20:
            ...
            for ( i = 0; i < NUMENVELOPE3PTS; i++ )
                {
                    temperature310[i] = ht20p[i];
                    altitude310[i]=altitude3[i];
		    udata->numpts = NUMENVELOPE3PTS - 1;
		    udata->lower_alt = altitude310[0];
		    udata->upper_alt = altitude310[NUMENVELOPE3PTS - 1];
      ...
      case LOWTEMP:
        switch (udata->epercent) {
          ...
          case P1:
            ...
            for ( i = 0; i < NUMENVELOPE2PTS; i++ )
                {
                    temperature310[i] = lt1p[i];
                    density310[i] = ltd1p[i];
                    altitude310[i]=altitude2[i];
		    udata->numpts = NUMENVELOPE2PTS - 1;
		    udata->lower_alt = altitude310[0];
		    udata->upper_alt = altitude310[NUMENVELOPE2PTS - 1];
        ...
        switch (udata->epercent) {
          ...
          case P1:
            ...
            for ( i = 0; i < NUMENVELOPE4PTS; i++ )
                {
                    pressure310[i] = hp1p[i]*1000.0;
                    altitude310[i] = altitude4[i];
		    udata->numpts = NUMENVELOPE4PTS - 1;
		    udata->lower_alt = altitude310[0];
		    udata->upper_alt = altitude310[NUMENVELOPE4PTS - 1];
                          ...
                          const SFcnCache *udata)
{
    ...
    for (i=0; i<numPoints; i++) {
         ...
         *  if altitude <= altitude310[bottom] then return idx = bottom
         *  if altitude >= altitude310[top]    then return idx = top
         */
        if (altitude[i] <= altitude310[bottom]) {
            idx = bottom;
        } else if (altitude[i] >= altitude310[top]) {
            idx = top;
        } else {
            for (;;) {
                idx = (bottom + top)/2;
                if (altitude[i] < altitude310[idx]) {
                    top = idx - 1;
                } else if (altitude[i] >= altitude310[idx+1]) {
                    bottom = idx + 1;
                } else {
                    /* we have altitude310[idx] <= altitude[i] 
                     *                          < altitude310[idx+1],
          ...
          case P1:
            switch (udata->evar) {
              ...
              case LOWDENSITY:
                ...
                double tempGrad = (temperature310[idx+1]-temperature310[idx])/
                    (altitude310[idx+1] - altitude310[idx]);

                if ((tempGrad != 0.0)&&((altitude[i]>udata->lower_alt)||
                                       (altitude[i]<udata->upper_alt)))
                {
                    temp[i] = temperature310[idx] + 
                        tempGrad * (altitude[i] - altitude310[idx]);
                    density[i] = density310[idx]*
                        pow(temperature310[idx]/temp[i], 
                            (GMR/tempGrad)+1.0 );
                } else {
                    temp[i] = temperature310[idx];
                    if ((altitude[i]>udata->lower_alt)||
                        (altitude[i]<udata->upper_alt)){
                    density[i] = density310[idx]*
                        exp( (-GMR)*(altitude[i] - altitude310[idx]) 
                             / temperature310[idx] );
                    }
                    else{
                    density[i]=density310[idx];
              ...
              case LOWPRESSURE:
                ...
                double presGrad = (pressure310[idx+1]-pressure310[idx])/
                     (altitude310[idx+1] - altitude310[idx]);

                if ((presGrad != 0.0)||(altitude[i]>udata->lower_alt)||
                                       (altitude[i]<udata->upper_alt))
                {
                    pressure[i] = pressure310[idx] + 
                        presGrad * (altitude[i] - altitude310[idx]);
                }
                else
                    { pressure[i] = pressure310[idx];
                    }
          ...
          case P20:
            switch (udata->evar) {
              ...
              case LOWDENSITY:
                ...
                double densGrad = (density310[idx+1] - density310[idx])/
                    (altitude310[idx+1] - altitude310[idx]);
                ...
                if ((densGrad != 0.0)||(altitude[i]>udata->lower_alt)||
                    ...
                    {
                        density[i] = density310[idx] + 
                            densGrad * (altitude[i] - altitude310[idx]);
                    }
                else
                    { 
                        density[i] = density310[idx];
