...
input_n = 3; % Up to input_n inputs are selected
anfis_n = 0;
for i = 1:input_n,
...
for i=1:input_n,
  ...
  for j=1:all_input_n,
    if isempty(input_index) | isempty(find(input_index==j)),
      ...
      trn_data = data(1:2:size(data,1), [current_input_index, size(data,2)]);
      chk_data = data(2:2:size(data,1), [current_input_index, size(data,2)]);
