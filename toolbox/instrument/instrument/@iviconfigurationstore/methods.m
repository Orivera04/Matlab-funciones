...
%   $Revision: 1.1.6.2 $  $Date: 2004/01/16 19:59:38 $

% Error checking.
if (nargout > 1)
    error('iviconfigurationstore:methods:maxlhs', 'Too many output arguments.');
end

if (nargin > 2)
    error('iviconfigurationstore:methods:invalidSyntax', 'Invalid syntax. Type ''help methods'' for more information.');
end

if (nargin == 2)
    if ~strcmp(varargin{1}, '-full')
        error('iviconfigurationstore:methods:invalidArg', ['Invalid second argument. The only valid second argument is ''-full''.',...
