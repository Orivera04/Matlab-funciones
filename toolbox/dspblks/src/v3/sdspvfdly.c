/*
 * SDSPVFDLY A SIMULINK variable fractional delay block.
 *
 ...
 *  $Revision: 1.30.4.1 $  $Date: 2004/01/25 22:39:43 $
 */

/* The polyphase filter bank is generated in MATLAB by the 
 * helper function dspblkvfdly.m
 */

#define S_FUNCTION_LEVEL 2
#define S_FUNCTION_NAME sdspvfdly

#include "dsp_sim.h"

enum {ARGC_MAX_DELAY, ARGC_FILTERS, ARGC_IC, ARGC_CHANNELS, NUM_ARGS};
...
static void mdlStart (SimStruct *S)
{
    ...
    if (ssGetInputPortWidth(S, DelayPort) != frame) {
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    const InputRealPtrsType delayPort = ssGetInputPortRealSignalPtrs(S, DelayPort);
...
static void mdlRTW(SimStruct *S)
{
     ...
     */
    {
        ...
        const real_T    *Fr         = mxGetPr(FILT_ARG);
        const real_T    *Fi         = mxGetPi(FILT_ARG);
        int_T           numFILT     = mxGetNumberOfElements(FILT_ARG);
        int_T	        nChans      = (int_T) (mxGetPr(CHANS_ARG)[0]);
        SFcnCache       *cache      = ssGetUserData(S);
        int_T           nIC1        = cache->nIC1;
        int_T           nIC2        = cache->nIC2;
        real_T          dummy       = 0.0;
        int32_T         numPhases   = (int32_T)mxGetN(FILT_ARG);
	...
	int32_T         i32MaxDelay = (int32_T)mxGetPr(DMAX_ARG)[0];
        ...
        if (!ssWriteRTWParamSettings(S, 7,
                                     ...
                                     SSWRITE_VALUE_DTYPE_NUM,  "NUM_PHASES", 
                                     &numPhases,
                                     DTINFO(SS_INT32,0),
