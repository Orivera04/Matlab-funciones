function varargout = mbsyield2oas(varargin)
...
%
% OAS = mbsyield2oas(ZeroRates, Yield, Settle, Maturity, ...
%  IssueDate, GrossRate)
%
% OAS = mbsyield2oas(ZeroRates, Yield, Settle, Maturity, ...
...
% 
% [Prices, AccruedInterest] = bndprice(Yields, CouponRate, Settle, ...
...
%   $Revision 1.3 $  $Date: 2004/04/06 01:09:00 $

if nargin <6
    error('finfixed:mbsyield2oas:invalidInputs',['Need at least ZeroMatrix, Price,', sprintf('\n'), ... 
    ...
    if any(Settle ~= Settle(1))
        error('finfixed:mbsyield2oas:invalidSettle',['Please make sure that all Settle date are the same, ' ...
...
if nargin <9 | isempty(varargin{9})
    ...
    if any(Interpolation ~=Interpolation(1))
      error('finfixed:mbsyield2oas:invalidInterpolation',['Please make sure that all Interpolation is of ONE type, ',...
...
if customized
    if ~isempty(varargin{10})
      error('finfixed:mbsyield2oas:invalidPrePaySpeed',['Cannot use benchmark when supplying customized prepayment CPR.', ...
             sprintf('\n'),...
    ...
    if isempty(varargin{11})
        error('finfixed:mbsyield2oas:invalidPrePayMatrix',['Please supply a prepayment (SMM) matrix when',... 
   ...
   [Yield Settle, Maturity, IssueDate, GrossRate, ...
    ...
    finargsz(1,Yield(:), Settle(:), Maturity(:), IssueDate(:), GrossRate(:), ...
      CouponRate(:), Delay(:), Interpolation(:));
   ...
   [CFlowAmounts dummy TFactors Factors crc_Maturity] = ...
    ...
    % Call mbscfamounts to get the Amounts and Time 
    % the cash flows occuring when std benchmark used   
    [CFlowAmounts dummy TFactors Factors crc_Maturity] = ...
        mbscfamounts(Settle, Maturity, IssueDate, ...
...
if any(Settle > min(CurveDates))
    error('finfixed:mbsyield2oas:invalidSpotCurve',['Settle is less than the earliest point in the Spot curve.', ...
           sprintf('\n'),... 
...
for i = 1:length(SpotRates)
    if RateCompounding(i)~=12
        ...
        if RateCompounding(i) == -1
            ...
            % This is where OutputCompoundingFORWARD could 
            % replace Output Compounding the old code
            SpotRates(i) = ...
                12 * ((1 + SpotRates(i)/RateCompounding(i)).^...
...
% Calculate the discount factor per period
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                            %
%   Clean Price =  sum( CF(i) / (1+YTM/12)^TF(i) )           %
%                                                            %
%           or                                               %
%                                                            %
%   Clean Price = sum( CF(i) / (1 + spot(i)/12 + K)^TF(i) )  %
%                                                            %
%   given CF, YTM, TF and spot, we can solve for K           %
%                                                            %   
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% subtract the Clean price out on Settle
% so that this matrix has NPVs of 0.
