function d1r=DMmathieu_Q1(kf,m,Q,x,varargin)
%
%     ==============================================================
...
%%dj(k+1)=bj(fix(k)-1+1)-fix(k)./x.*bj(fix(k)+1);
   %%dj1(0+1,kr)=-bj1(1+1,kr);
    %%dj2(0+1,kr)=-bj2(1+1,kr);
      ...
      dj2(k,kr)=dbesseli(fix(k)-1,u2(kr));
...
d1r(kr)=d1r(kr)+i.*(-1).^(fix(ic)+fix(k)).*fg(kr,fix(k)).*(c2.*(bj1(fix(k),kr).*dj2(fix(k)+1,kr)-(-1).^fix(kd).*bj1(fix(k)+1,kr).*dj2(fix(k),kr))-c1.*(dj1(fix(k),kr).*bj2(fix(k)+1,kr)-(-1).^fix(kd).*dj1(fix(k)+1,kr).*bj2(fix(k),kr)));  
       else;
