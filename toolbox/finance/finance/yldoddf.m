function yld =  yldoddf(sd,md,id,fd,rv,price,cpn,per,basis,maxiter) 
...
%       $Revision: 1.7 $   $Date: 2002/04/14 21:53:56 $ 
...
if nargin < 7 
  error(sprintf('Missing one of SD, MD, ID, FD, RV, CPN, and YLD.')) 
end 
...
if checksiz([size(sd);size(md);size(id);size(fd);size(rv);size(price);... 
             size(cpn);size(per);size(basis);size(maxiter)],mfilename) 
  return 
end 
...
if checkrng(str2mat('sd','md','cpn','per'),[sd(i),md(i),cpn(i),per(i)],... 
            [id(i) fd(i) 0 0],[fd(i) inf inf inf],str2mat('id','fd','0','0'),... 
