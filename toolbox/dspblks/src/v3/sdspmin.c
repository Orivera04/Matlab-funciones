/*
 * SDSPMIN DSP Blockset S-Function for minimum computation,
 * with value and index outputs. S-Function also computes the 
 * running minumum with reset input.
 *
 *  Copyright 1995-2002 The MathWorks, Inc.
 *  $Revision: 1.31 $  $Date: 2002/04/14 20:42:50 $
 */
#define S_FUNCTION_NAME sdspmin
...
static void mdlInitializeSizes(SimStruct *S)
{
...

    {
        ...
        switch (ftype) {
            case fcnRunning:
            {
                /* Running minimum: 2 inputs (Data, Reset), 1 output (Val) */
                /* Inputs: */
...
static void mdlInitializeConditions(SimStruct *S)
{
     ...
     * value *is* the min value).  After that, we only look at the
     * reset input port to determine if we must reset the min value.
     * However, in a re-enabling subsystem, we might have to reset
     * the min value as well.
     */
    if (running) {
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    switch (ftype) {
        ...
        {
            /*
             * Running Minimum: 2 inputs (data and reset) and 1 output (minimum).
             */
            const int_T     width  = ssGetInputPortWidth(S, INPORT_DATA);
            const boolean_T frame  = (boolean_T)(mxGetPr(FRAME_ARG)[0] != 0.0);
            ...
            if (!c0) {
                ...
                /* Point to array of "last minimum data values" for each input element */
        ...
        case fcnValandIdx:
        {
            ...
            if (!c0) {
                ...
                real_T             m_val = **(uptr++);  /* 1st element is initial min */
                int_T              idx;
                ...
                /* 
                 * fcnVal: 1 output, minimum value 
                 * fcnIdx: 1 output, index of minimum value
                ...
                *y0 = ((ftype == fcnVal) || (ftype == fcnValandIdx)) ? m_val : (real_T)(m_idx + 1);
                ...
                creal_T       *u     = (creal_T *)(*uptr++); /* 1st element is initial min */
                real_T         m_val = CMAGSQ(*u);
                int_T          idx;
                
                for (idx=1; idx<width; idx++) {
                    creal_T *u = (creal_T *)(*uptr++);
                    const real_T val = CMAGSQ(*u);
                    if (val < m_val) {
                        m_val = val;
                        m_idx = idx;
                    }
...
static void mdlSetWorkWidths(SimStruct *S)
{
    ...
    if (running) {
        ...
        if (ssGetInputPortComplexSignal(S, INPORT_DATA) == COMPLEX_YES) {
            /* 
