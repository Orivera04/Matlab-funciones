...
%	This script creates graphs for the simulation script REGROB.
...
iflg_=1;
while(opt_),
fprintf('\n\n --REGROBP--Reduced-Order-Observer-Based Regulator Plotting.\n\n');
...
if m_==1
  fprintf('     <4>  Measurement \n');
else
  fprintf('     <4>  Measurements (1--%g) \n',m_);
end
...
if (opt_==4),
  ...
  if m_==1
    plot(t1,ym);
    title('Measurement ym(t)');
    xlabel('Time (seconds)')
    grid;
  else
    plot(t1,ym(wo_,:));
    title(sprintf('Measurement #%g',wo_))
    xlabel('Time (seconds)')
    grid;
  end
end
...
if opt_==6,
  ...
  if tg_==4
   plot(t1,ym(wo1_,:));hold on
   if m_==1
    tt_='Measurement ym(t)';
   else
     tt_=sprintf('Measurement #%g',wo1_);
   end
  end
  if bg_==1
   plot(t1,x(wo2_,:),'--');
  ...
  if bg_==4
   plot(t1,ym(wo2_,:),'--');
   if m_==1
    bt_='Measurement ym(t)';
   else
     bt_=sprintf('Measurement #%g',wo2_);
   end
  end
...
% ___________________ END OF REGROBP.M ___________________________________
