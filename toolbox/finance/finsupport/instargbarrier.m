function [OptSpec,Strike,Settle,ExerciseDates,AmericanOpt,BarrierSpec, Barrier,Rebate] = instargbarrier(varargin)
% INSTARGBARRIER Subroutine for 'Type','Barrier' argument validation.  
%   This function is called at the top of processing routines.
%
%   [OptSpec, Strike, Settle,ExerciseDates, AmericanOpt, BarrierSpec, ...
%                                   Barrier, Rebate] = instargbarrier(ArgList{:})
%
%   Inputs: 
...
%     ExerciseDates   - NINST x 1 (for European) or NINST x 2(for American) of
%                       exercise dates.
...
%
%     Barrier         - NINST x 1 matrix if barrier levels.
%     Rebate          - NINST x 1 matrix of rebate values.

%
%   See also INSTBARRIER.

%   Author(s): M. Reyes-Kattar 01-Nov-2002
%   Copyright 1998-2003 The MathWorks, Inc. 
%   $Revision: 1.1.6.1 $  $Date: 2003/08/29 04:45:50 $
...
[FieldList, ClassList, TypeString, SizeList, DefDataList] = instbarrier;
...
% Check the number of arguments, 6 to NumFields
if nargin<7,
  error('Seven arguments are required for a barrier');
...
[OptSpec,Strike,Settle,ExerciseDates,AmericanOpt,BarrierSpec,Barrier,Rebate] = deal(ArgCells{:});
...
% BarrierSpec
BarrierSpec = cellstr( lower(BarrierSpec) );
UIMatch = strcmp(BarrierSpec, 'ui');
UOMatch  = strcmp(BarrierSpec, 'uo');
DIMatch  = strcmp(BarrierSpec, 'di');
DOMatch  = strcmp(BarrierSpec, 'do');

Ind = find(~(UIMatch | UOMatch | DIMatch | DOMatch));
if any(Ind)
    BadOpt = unique(strvcat(BarrierSpec(Ind,:)), 'rows')';
    BadOpt = finargcat(1,BadOpt,' ');
    error(['Unrecognized BarrierSpec: ',BadOpt(:)'])
end
...
%--------------------------------------------------------------------
if any(Barrier < 0)
    error('Barrier values cannot be negative')
end

if any(Rebate < 0)
    error('Rebate values cannot be negative')
end
