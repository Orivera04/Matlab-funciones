/*
 * Power System Blockset continuous implementation of state-space system.
 * See simulink/src/sfuntmpl_doc.c
 *
 * By Gilbert Sybille (IREQ) and Roger Champagne (ETS), 
 * Revision: 1.1(ETS revision number)
 * Date: 2000-05-25 10:45:18-04
 *
 ...
 * $Revision: 1.1.6.1 $
 * File : sfun_psbcontc.c
 */


#define S_FUNCTION_NAME sfun_psbcontc
...
#define YISW_PARAM(S) ssGetSFcnParam(S, 6)
...
static void mdlStart(SimStruct *S)
{
    ...
    ssGetPWork(S)[6]=tmp1;
    ssGetPWork(S)[7]=tmp2;
    ssGetPWork(S)[8]=sw_state;
    ssGetPWork(S)[9]=yswitch;
    ssGetPWork(S)[10]=idxSwChng;
    ssGetPWork(S)[11]=swChng;
    ssGetPWork(S)[12]=Chopper;
    ssGetPWork(S)[13]=IndicesSortiesSwitches;
...
static void mdlInitializeConditions(SimStruct *S)
{
    ...
    real_T           *x0       = (real_T*)ssGetContStates(S);

    real_T           *A        = (real_T*)ssGetPWork(S)[0]; 
    real_T           *B        = (real_T*)ssGetPWork(S)[1]; 
    real_T           *C        = (real_T*)ssGetPWork(S)[2]; 
    real_T           *D        = (real_T*)ssGetPWork(S)[3]; 
    int_T            *sw_state = (int_T*) ssGetPWork(S)[8];
    real_T           *yswitch  = (real_T*)ssGetPWork(S)[9];
    int_T            *Chopper  = (int_T*) ssGetPWork(S)[12];
    
    int_T            *IndicesSortiesSwitches  = (int_T*) ssGetPWork(S)[13];
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    int_T             *sw_state = (int_T*) ssGetPWork(S)[8]; 
    real_T            *yswitch  = (real_T*)ssGetPWork(S)[9];
    int_T             *idxSwChng= (int_T*) ssGetPWork(S)[10];
    int_T             *swChng   = (int_T*) ssGetPWork(S)[11];
    int_T             *Chopper  = (int_T*) ssGetPWork(S)[12];
    int_T             *IndicesSortiesSwitches = (int_T*)ssGetPWork(S)[13]; 

    int_T             i, j, k, ki, idx2, idx3, nSw, kSw, indice, nbSwChng=0;
    real_T            accum, temp;
