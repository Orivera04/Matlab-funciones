...
%    $Revision: 1.4.2.4 $  $Date: 2003/08/29 04:42:34 $
...
% Determine if the object or an event structure was passed in.
if isstruct(obj)
   events = obj;
else
...
if nargin == 2
   ...
   elseif isempty(index)
      ...
      error('daq:showdaqevents:invalidindex', 'No events match the specified INDEX.');
   end
...
if ~isequal(fieldnames(events),{'Type';'Data'}) && ...
        ~isequal(fieldnames(events),{'Data';'Type'}),
    error('daq:showdaqevents:invaliddata', 'Invalid event data structure. Structure must contain Type and Data fields.')
end
