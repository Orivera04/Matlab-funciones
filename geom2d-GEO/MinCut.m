...
%%% input - 
%%%   list of vertices to keep togeather and weighted      CLICK !!! <=====
%%% output -
%%%   MinCutGroupsList - two lists of verices, SECOND one contains the sourve vertives
%%%   MinCutWeight - weight on cut

%%% (C) Yohai Devir 2006
%%% <first name> AT WHOEVER D0T COM

    GraphDim = size(WeightedGraph,1);
    SourceVertices = SourceVertices(find(SourceVertices)); %remove zero vertices
...
function [MinCutGroupsList, MinCutWeight] = MinCutNoSeed(WeightedGraph)
    ...
    jj = 1;
    for kk=1:GraphDim
        if isempty(find(MinCutGroup(1,:) == kk)) 
            MinCutGroupsList(2 ,jj) = kk;
            jj = jj + 1;
        end
    end
