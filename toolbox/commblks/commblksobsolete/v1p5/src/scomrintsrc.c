/* $Revision: 1.1.6.1 $ */
/*
 * SCOMRINTSRC   Random integer source
 *
...
#define S_FUNCTION_NAME scomrintsrc
...
static void mdlInitializeSizes(SimStruct *S)
{
  ...
  if (ssGetNumArgs(S) == NUM_ARGS) {
    ...
    if (((mxGetN(MUL_NUM) * mxGetM(MUL_NUM)) != 1) && 
        (mxGetN(MUL_NUM) * mxGetM(MUL_NUM)) != numOutput) {
        
        char_T err_msg[256];
        sprintf(err_msg, "Dimension for boundaries must match the dimension "
                "for seeds.");
        mexErrMsgTxt(err_msg);
    }
...
static void mdlInitializeConditions(real_T *x0, SimStruct *S)
{
    ...
    real_T  *mulNum        = (real_T *)mxGetPr(MUL_NUM);
