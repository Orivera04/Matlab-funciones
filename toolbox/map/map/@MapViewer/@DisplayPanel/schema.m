...
%   $Revision: 1.1.6.3 $  $Date: 2003/12/13 02:49:28 $

pk   = findpackage('MapViewer');
hgPk = findpackage('hg');

c = schema.class(pk,'DisplayPanel',hgPk.findclass('axes'));
...
p = schema.prop(c,'ActiveLayerDisplay','double');
p.AccessFlags.PrivateGet = 'on';
p.AccessFlags.PrivateSet = 'on';
p.AccessFlags.PublicGet  = 'on';
p.AccessFlags.PublicSet  = 'off';

p = schema.prop(c,'MapUnitsDisplay','double');
p.AccessFlags.PrivateGet = 'on';
p.AccessFlags.PrivateSet = 'on';
p.AccessFlags.PublicGet  = 'on';
p.AccessFlags.PublicSet  = 'off';
