function [Price, PriceTree] = barrierbyeqp(BinStockTree, varargin)
%BARRIERBYEQP Price barrier options by an EQP binomial tree.
%
%   [Price, PriceTree] = barrierbyeqp(EQPTree, OptSpec, Strike, Settle, ExerciseDates,...
...
%   $Revision: 1.1.6.2 $  $Date: 2003/09/22 19:13:57 $
...
if nargin<8 
    str1 = 'The function barrierbyeqp requires eight input arguments:';
    str2 = 'EQPTree, OptSpec, Strike, Settle, ExerciseDates, AmericanOpt, BarrierSpec, and Barrier';
    msg = sprintf('%s\n%s', str1, str2);
    error('finderiv:barrierbyeqp:InvalidInputs',msg)
end
    

if ~isafin(BinStockTree, 'BinStockTree') | ~strcmpi(BinStockTree.Method, 'EQP')
    error('finderiv:barrierbyeqp:InvalidTree','EQPTree must be a Binary Stock Tree created with EQPTREE')
end
