/*
 * fft_r2br_z_rt.c - Signal Processing Blockset FFT run-time library components.
 *
 *  Copyright 1995-2003 The MathWorks, Inc.
 *  $Revision: 1.7.2.3 $  $Date: 2004/04/12 23:43:37 $
 */

#include "dspfft_rt.h"

/*
 * In-place digit-reverse data reordering for radix 2 ("bit reversal").
    ...
    while(nChans-- > 0) {
        ...
        for (i = 1; i < fftLen-2; i++) {
            if (i < j) {
                 ...
                 */
                real_T t=y[j].re; y[j].re=y[i].re; y[i].re=t;
