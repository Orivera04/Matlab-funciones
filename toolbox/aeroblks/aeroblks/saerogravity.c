 ...
 *
 *  $Revision: 1.11.2.2 $ $Date: 2004/04/10 23:12:07 $
 */

#define S_FUNCTION_NAME  saerogravity
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    /* if WGS84TAYLORSERIES, nInputPorts= 2, else WGS84CLOSEAPPROX or 
       WGS84EXACT, nInputPorts = 3*/
    nInputPorts = ((TypeIdx) mxGetScalar(TYPE_PARAM(S)) == WGS84TAYLORSERIES) 
        ? 2 : 3;        
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    opt_m2ft = ( udata->units == METRIC) ? 1.0: M2FT;
    ...
    switch ( udata->type ) { 
      case WGS84TAYLORSERIES:
        ...
        for ( i = 0; i < num_inputs; i++ ){
            ...
            phi = *phi_ptr[i] * DEG2RAD;
            h = *height_ptr[i] / opt_m2ft;
