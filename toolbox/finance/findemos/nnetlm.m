function pred = nnetlm(indat,pts,N,neurons)
%NNETLM Neural Net Levenberg-Marquardt demo for Financial Expo.
%   PRED = NNETLM(INDAT,PTS,N,NEURONS) Neural Network Levenberg-Marquardt
%   demonstration for Financial Expo.  INDAT is the input data used for 
%   training and testing.  PTS specifies the total number of data set points
%   to be used.  N is the number of points in PTS which are used to predict
%   the next output.  NEURONS specifies the number of hidden neurons to be 
%   used by the neural network.   
%
%   For example, from a data set of 300 points (PTS = 300), use 10 points 
%   (N = 10) at a time to predict the next point.  That is, the first 10 points
%   are used to predict the 11th point, points 2 through 11 are used to predict 
%   the 12th point and so on.  Increase the number of NEURONS being used for
...
%       $Revision: 1.8 $   $Date: 2002/04/14 21:43:24 $
...
grid on
title('Levenberg-Marquardt Estimation')
