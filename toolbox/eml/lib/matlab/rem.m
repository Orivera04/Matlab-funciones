...
% $INCLUDE(DOC) toolbox/eml/lib/matlab/rem.m $
%   Copyright 2002-2004 The MathWorks, Inc.
%   $Revision: 1.1.6.5 $  $Date: 2004/04/14 23:58:35 $
...
eml_assert(isfloat(x), 'error', ['Function ''rem'' is not defined for values of class ''' class(x) '''.']);
eml_assert(isfloat(y), 'error', ['Function ''rem'' is not defined for values of class ''' class(y) '''.']);
...
for i = 1:size(new_x,1)
    for j = 1:size(new_x,2)
        out(i,j) =scalar_rem(new_x(i,j), new_y(i,j));
    end
end
