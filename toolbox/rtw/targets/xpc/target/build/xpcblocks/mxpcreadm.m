function [noutputsout, dimensionsout, datatypesout]=mxpcreadm(noutputs, dimensions, datatypes)

% MXPCREADM - Mask Initialization function for Broadcast Memory ReadM driver block
% Copyright 1996-2002 The MathWorks, Inc.
% $Revision: 1.3 $ $Date: 2002/03/25 04:10:39 $
...
if (length(dimensions)~=noutputs)
  error('length of dimension cell array is not equal to number of output ports');
end

if (length(datatypes)~=noutputs)
  error('length of datatypes cell array is not equal to number of output ports');
end
...
for i=1:noutputs
  ...
  if size(dimension,1)>1 | size(dimension,2)>2
    error('dimensions elements must be aither scalar or a row vector with two elements');
  end
