/*
 *   Module Name: sis95rcdec.c
 * 
 *  Description:
 *      Uses the Viterbi algorithm for optimum decoding of the convolutionally 
 *      encoded information sequence with rate 1/2 and 1/3.It involves a search 
 *      through the trellis for the most probable sequence. The corresponding 
 *      metric in the trellis search is an euclidean metric. 
 * 
 *  Inputs:
 *      Rate
 *          Integer scalar, this represents the rate for which the Viterbi  
 *          decoding is to be performed.
 *      Frame In
 *          Real vector representing the received frame data.
 *
 *  Outputs:
 *      Frame Out
 *          Binary vector, representing the decoded frame.
 *      Metric
 *          Real scalar, containing the final path metric of the Viterbi  
 *          decoder.
 *
 *  Parameters:
 *      Channel Type
 *          Logical Channel Type (TRAFFIC/ACCESS): 1 for TRAFFIC and 2 
 *          for ACCESS.
 *      Rate Set 
 *          Rate Set 1 or Rate Set 2.
 *      Trace-Back-Delay to Frame-Length Ratio
 *          This provides the ratio of the trace-back length for the Viterbi 
 *          algorithm  to the input frame size. This may be left empty if  
 *          full trace-back is desired or may be specified to set the 
 *          trace-back length.
 *
 *  Copyright 1999-2002 The MathWorks, Inc. and ALGOREX, Inc.
 *  $Revision: 1.13.2.1 $  $Date: 2004/04/12 22:59:50 $
 */
#define S_FUNCTION_NAME sis95rcdec
...
typedef struct TurbDec_s{
    ...
    int                    frameread;   /*Index that points to the input ports which each increment is 
                                        done in terms of the code rate*/
                      ...
                      tPar *params)
{
    ...
    *pCodeWd = 0;
    ...
    
    {
        ...
        /*psMask = btManMsk(numdelay, 0);
        int btManMsk( int numMaskBit, int startPos)*/
        {
            int i, mask=0;
                      ...
                      int lastFrameFlag)
{
    ...
    int i,index=0,readPath,bestState=0;
...
static void mdlCheckParameters(SimStruct *S) {
    ...
    if ( (mxGetM(TRACEBACKLEN_ARG)*mxGetN(TRACEBACKLEN_ARG)) != 1 || 
        ...
        msg = "Ratio of trace-back length to frame length must be a positive scalar less than 1.0";
        goto ERROR_EXIT;
    }
    ...
    if ( (mxGetM(DECODELEN_ARG)*mxGetN(DECODELEN_ARG)) != 1 || 
        ...
        msg = "Ratio of decoding length to frame length must be a positive scalar less than 1.0";
        goto ERROR_EXIT;
    }
...
static void mdlInitializeConditions( SimStruct *S)
{
    ...
    P_GENFORW       =  (real_T *) calloc(3 * 2,sizeof(real_T));
    
    if ((rateSet == RATESET2)&(chType == TRAFFIC)) {
        P_NUMFORWGEN    =  2;
        P_GENFORW[0]    = 431.0;    P_GENFORW[1] = 285.0;
    }else {
        P_NUMFORWGEN    =  3;
        P_GENFORW[0]    = 493.0;    P_GENFORW[1] = 411.0;     P_GENFORW[2] = 295.0;
    }
    P_GENBACK       = 1; /* Also currently non-SOVA and Non-SYSTEMATIC */

    /* P_FRAME */
    switch(chType){
    case ACCESS: P_FRAME = 96; break;
