function [LLF, G, H, e, h, LogLikelihoods] = egarchllfn(Parameters, y, R, M, P, Q, X, isConstantInMean, isVarianceInMean, e0, s0, y0)
%EGARCHLLFN Univariate EGARCH process objective function (Gaussian Distribution).
...
%   conditional variances are then fit to an EGARCH model. Gaussian innovations
%   are assumed.
%
%   [LLF, G, H, Innovations, Sigmas, LogLikelihoods] = egarchllfn(Parameters, 
...
%   $Revision: 1.1.4.1 $   $Date: 2003/05/08 21:45:17 $
...
% expected value. For Gaussian innovations, E(|z(t-i)|) = sqrt(2/pi).
%
...
varianceCoefficients  =  Parameters((isConstantInMean + R + M + nX + 1):end);
...
MAD  =  sqrt(2/pi);  % For Gaussian innovations.
