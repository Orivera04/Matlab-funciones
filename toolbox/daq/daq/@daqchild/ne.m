...
%    $Revision: 1.10.2.4 $  $Date: 2003/08/29 04:40:28 $
...
if isa(arg1, 'daqchild') && isa(arg2, 'daqchild')
   ...
   if prod(size(arg1))~=1 && prod(size(arg2))~=1
      for i=1:length(arg1)
         ...
         % Get access to the daqchild object's parent's handle.
         p1=struct(P1);
         p2=struct(P2);
         
         % The object's are not equal if the objects do not have the same 
         % handle and/or their parents do not have the same CreationTime
         % and/or handle.
         isNEQ = ~( strcmp(class(arg1(i)),class(arg2(i))) &...
   ...
   elseif prod(size(arg1))==1
      ...
      for i=1:length(arg2)
         ...
         % handle and/or their parents do not have the same CreationTime
         % and/or handle.
         isNEQ = ~( strcmp(type,class(arg2(i))) & CrTm==get(P2,'CreationTime') &...
   ...
   elseif prod(size(arg2))==1,
      ...
      for i=1:length(arg1)
         ...
         % handle and/or their parents do not have the same CreationTime
         % and/or handle.
         isNEQ = ~( strcmp(class(arg1(i)),type) & get(P1,'CreationTime')==CrTm &...
