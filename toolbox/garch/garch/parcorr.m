...
%   $Revision: 1.6.2.1 $  $Date: 2003/05/08 21:45:38 $
...
if (rows ~= 1) & (columns ~= 1) 
    error('GARCH:parcorr:NonVectorInput' , ' Input ''Series'' must be a vector.');
end
...
if (nargin >= 2) & ~isempty(nLags)
   if prod(size(nLags)) > 1
      error('GARCH:parcorr:NonScalarLags' , ' Number of lags ''nLags'' must be a scalar.');
   end
   if (round(nLags) ~= nLags) | (nLags <= 0)
      error('GARCH:parcorr:NonPositiveIntegerLags' , ' Number of lags ''nLags'' must be a positive integer.');
   end
   if nLags > (n - 1)
      error('GARCH:parcorr:LagsTooLarge' , ' Number of lags ''nLags'' must not exceed ''Series'' length - 1.');
   end
else
...
if (nargin >= 3) & ~isempty(P)
   if prod(size(P)) > 1
      error('GARCH:parcorr:NonScalarP' , ' Number of lags ''P'' must be a scalar.');
   end
   if (round(P) ~= P) | (P < 0)
      error('GARCH:parcorr:NegativeIntegerP' , ' Number of lags ''P'' must be a non-negative integer.');
   end
   if P >= nLags
      error('GARCH:parcorr:PTooLarge' , ' ''P'' must be less than ''nLags''.');
   end
else
   P  =  0;       % Set default.
end
...
if (nargin >= 4) & ~isempty(nSTDs)
   if prod(size(nSTDs)) > 1
      error('GARCH:parcorr:NonScalarSTDs' , ' Number of standard deviations ''nSTDs'' must be a scalar.');
   end
   if nSTDs < 0
      error('GARCH:parcorr:NegativeSTDs' , ' Number of standard deviations ''nSTDs'' must be non-negative.');
   end
else
