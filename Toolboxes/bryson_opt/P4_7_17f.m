% Script p4_7_17f.m; TDP for min time trsfr to Jupiter orbit using FOPTF;
% y=[r u v th lr lu lv lt]'; be=thrust direction angle; 6/97, 3/28/02
%
clear; clear global; global rf; rf=5.2; vf=.4385; la0=[-3.161 .451 ...
  -4.373 0]; tf=8.216; c=180/pi; nu=[-.813 2.630 -.761]; 
p0=[la0 nu tf]; optn=optimset('Display','Iter','MaxIter',10);
name='mart'; s0=[1 0 1 0]';
p=fsolve('foptf',p0,optn,name,s0); [f,t,y]=foptf(p,name,s0);
N=length(t); be=atan2(-y(:,6),-y(:,7)); for i=1:N, if be(i)<-.2, ...
   be(i)=be(i)+2*pi; end; end; r=y(:,1); u=y(:,2); v=y(:,3);
th=y(:,4); xc=r.*cos(th); yc=r.*sin(th); yt=yc+.75*cos(be-th);
th1=[0:6:360]*pi/180; xj=5.2*cos(th1); yj=5.2*sin(th1); xe=cos(th1);
ye=sin(th1); lr=y(:,5); lu=y(:,6); lv=y(:,7);
%
figure(1); clf; plot(xc,yc); grid; hold on; xt=xc+.75*sin(be-th);
plot(xe,ye,'r--',1,0,'ro',xc(N),yc(N),'ro'); for i=1:2:N,
