/* 	program dot1mpi.c */
/* Illustrates dot product via mpi_send and mpi_recv. */     
#include <stdio.h>
...
/* The local dot products are sent and recieved to processor 0. */        
      if (my_rank==0) {
        for (source = 1;source <= p-1; source++) { 
          MPI_Recv(&loc_dots[source],1,MPI_FLOAT,source,50,
                        MPI_COMM_WORLD,&status);
        }
      }else{
         MPI_Send(&loc_dot,1,MPI_FLOAT,0,50,
                             MPI_COMM_WORLD);
      }
