/*
 *  firdn_df_dblbuf_zz_rt.c - FIR Decimation block runtime helper function
 *
 *  Copyright 1995-2003 The MathWorks, Inc.
 *  $Revision: 1.3.2.3 $  $Date: 2004/04/12 23:45:03 $
 *
...
EXPORT_FCN void MWDSP_FIRDn_DF_DblBuf_ZZ(
    const creal_T   *u,
          creal_T   *yout,
          creal_T   *tap0,
          creal_T   *sums,
...
)
{
    ...
    for (k = 0; k < numChans; k++) {
        ...
        while (i--) {
            ...
            *mem = *u++;   
            while (j--) {       
                rsum += (mem->re) * (*cff).re - (mem->im) * (*cff).im;
                isum += (mem->im) * (*cff).re + (mem->re) * (*cff).im;
                cff++;
