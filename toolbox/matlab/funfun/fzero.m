...
elseif (length(x) == 1)
    if trace > 2 
        ...
        header = ' Func-count   left       f(left)          right     f(right)     Procedure';
    end
    % Put first feval in try catch
    try
...
function [xOutputfcn, optimValues, stop] = callOutputFcn(outputfcn,x,state,fcount,iter,intervaliter,  ...
    f,procedure,left,fvalleft,right,fvalright,varargin)
...
optimValues.intervalleft = left;
optimValues.fvalleft = fvalleft;
optimValues.intervalright = right;
optimValues.fvalright = fvalright;
