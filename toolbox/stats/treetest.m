function [cost,secost,ntnodes,bestlevel] = treetest(Tree,TorCorR,X,Y,varargin)
...
%   $Revision: 1.5.4.2 $  $Date: 2003/11/01 04:29:21 $

if nargin<2, error('stats:treetest:TooFewInputs','Not enough arguments.'); end
if ~isstruct(Tree) || ~isfield(Tree,'method')
   error('stats:treetest:BadTree','First argument must be a decision tree.');
end
...
defaults = {10         'se'};
[eid,emsg ncv treesize] = statgetargs(okargs,defaults,varargin{:});
if ~isempty(emsg)
...
if doclass
   ...
   coststruct.group = 1:size(Tree.cost,1);
...
for j=1:ncv
   ...
   Tj = treefit(X(trainrows,:),id(trainrows),...
