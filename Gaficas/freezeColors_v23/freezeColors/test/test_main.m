...
subplot(3,2,1)
imagesc(peaks); axis xy; colormap hot; title('colors not frozen'); colorbar
...
disp('  testing the freezing of an explicitly specified axis.')
disp(' =Hit a key='), pause, disp(' ')

subplot(3,2,2)
imagesc(peaks); axis xy; colormap jet; title('jet colormap, frozen')
freezeColors
freezeColors(colorbar) %tests passing an axis handle 
...
disp('>> freezeColors')
disp('>> freezeColors(colorbar) %tests passing an axis handle')
