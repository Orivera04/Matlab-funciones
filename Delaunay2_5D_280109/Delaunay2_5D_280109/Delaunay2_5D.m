% Delaunay2_5D 
% 
% Simplified version of the Delaunay2_5D algorithm based on 2.5 D delaunay
%   criterion.
% 
...
tic
[ptest,tetr,tetr2t,t2tetr,t]=Connectivity(tetr);
...
function [ptest,tetr,tetr2t,t2tetr,t]=Connectivity(tetr)
...
ptest=zeros(nume,2,'int32');%testpoints for Delaunay test
for i=1:nume

    ptetr=tetr(tetr2t(i,1),:);%points of first neighbour tetraedrom
    for j=1:4
        if (ptetr(j)~=t(i,1) && ptetr(j)~=t(i,2) && ptetr(j)~=t(i,3))
            ptest(i,1)=ptetr(j);
            break
        end
    end
    %Check first tetraedrom
    if tetr2t(i,2)>0
        ptetr=tetr(tetr2t(i,2),:);%points of first neighbour tetraedrom
        for j=1:4
            if (ptetr(j)~=t(i,1) && ptetr(j)~=t(i,2) && ptetr(j)~=t(i,3))
                ptest(i,2)=ptetr(j);%point oppostite to the triangle
                break
            end
        end
    end
