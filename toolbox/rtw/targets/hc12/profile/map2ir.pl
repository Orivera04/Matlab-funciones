...
} # end sub readMapFilewithSummary
...
sub trans2Value {
	...
	if ($type eq "DEC") {
		...
		#$tmp = substr $string, 0, length($string); #get rid of leading "0x"	
		#$tmp = "0x" . $string;	#take care of any length HEX 
		$tmp = $string;
		$ret_hex += hex $tmp;
	} 
...
sub isDWARFsection {
	...
	foreach $eachname(@DWARF) {
		if ($section_name eq $eachname) {
			return 1;
		}
	}									
	return 0;
} # end sub isDWARFsection
...
sub removeOverlapSectionLayout {
	...
	for ($i = 0; $i<=$#SL_addr; $i++) {
		for ($j = 0; $j <= $#SL_addr; $j ++) {
			if (($i != $j) && ($SL_addr[$i] != -1) && ($SL_addr[$j] != -1)) {
				...
				if (($start1 <= $start2) && ($end1 >= $end2) && (!isInList($SL_func[$i], Full_Cross_Table)) ) {
					if ($EchoDebugInfo) {
						print "Will discard symbol $SL_func[$i] for $SL_func[$j]\n";
					}
					$SL_addr[$i] = -1;
				} elsif (($start2 <= $start1) && ($end2 >= $end1) && (!isInList($SL_func[$j], Full_Cross_Table)) ) {
					if ($EchoDebugInfo) {
						print "Will discard symbol $SL_func[$j] for $SL_func[$i]\n";
					}
					$SL_addr[$j] = -1;
				}
			}
	...
	@SL_section = @t6;
	@SL_sectionname = @t7;
} # end sub removeOverlapSectionLayout
	...
	while (<$FILE_HANDLE>) {
		...
		if (/$layout_Regular_EXPR1/) {
			...
			$accurate_section_name = ""; #reinitialize global $accurate_section_name
				...
				push @SL_section, calculateSection($funcaddr, $funcsize);
...
} # end sub searchGenSectionLayout
...
# Here we make the assumption that heap/stack either defined in the same manner
# as other sections in MAP file, or MAP has dedicated lines to describe stack/heap,
# and these lines must be one for beginning, one for ending
# search heap lines
...
if ($ExistSummary) {
	readMapFilewithSummary(MAPFILE);
} else {
	readMapFilenoSummary(MAPFILE);
}
