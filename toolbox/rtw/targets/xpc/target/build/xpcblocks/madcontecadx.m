function [maskDisplay, maskDescription, range, pciBus, pciSlot] = madcontecadx( phase, channel, range, polarity, slot, device )
    ...
    switch device
        case 1  
            maxChan = 16 / polarity;
            deviceName = 'AD12-16(PCI)';
        case 2  
            maxChan = 64 / polarity;
            deviceName = 'AD12-64(PCI)';
        otherwise
            error('madcontecadx: bad device param');
    end
    ...
    elseif length(sort(channel)) < length(channel)
    ...
    elseif ~all(ismember(range, [1, 2, 5, 10, -1, -2, -5, -10]))
        error('Range vector elements must be chosen from the values 1, 2, 5, 10, -1, -2, -5, -10');
...
%   $Revision: 1.2 $  $Date: 2002/06/06 15:56:27 $
