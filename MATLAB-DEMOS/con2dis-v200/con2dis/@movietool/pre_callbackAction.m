...
% callback evaluator.
%
%   PRE_CALLBACKACTION(mt,varargin) determines which callback "action" is relavant during
...
if ~isempty(mt)
    ...
    if mt.play_flag
        ...
        if any(list_current)
            ...
            %-------------------------------
            if  strcmp(callback,'GetCurrentPoint')
                %-------------------------------
                set(varargin{4}.Axis4,'userdata',playdata{1}{index})
                %-------------------------------
            end
            %-------------------------------
