		 ...
		 int nrhs, const mxArray * prhs[])
{
  ...
  // Get a pointer to the transpose of the array
  float *pr = (float *) mxGetPr(prhst);
  
  // Create the tree
  KDTree *tree = new KDTree;
  tree->create(pr,npoints,ndims);
  
 
  // Copy the serialized tree data to a new class
  plhs[0] = mxCreateStructMatrix(1,1,1,fieldNames);
  int length = tree->get_serialize_length();  
  mxArray *tmp = mxCreateNumericMatrix(length,1,mxUINT8_CLASS,mxREAL);
  tree->serialize(mxGetPr(tmp));
 ...
 // Delete the tree 
 // (it has already been serealized & put in the variable)
  delete tree;
