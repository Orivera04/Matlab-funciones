function obj = removeFromExtrapolationMask(obj, varargin)
%REMOVEFROMEXTRAPOLATIONMASK Remove table cells from the extrapolation mask
%
%  OBJ = REMOVEFROMEXTRAPOLATIONMASK(OBJ, ROW, COL) removes the specified
%  table cell from the extrapolation mask.  ROW and COL can be vectors of
%  the same length, N, in which case they will specify N cells to remove
%  from the mask.
%
%  OBJ = REMOVEFROMEXTRAPOLATIONMASK(OBJ, ROW) removes table cells from the
%  mask of a 1D table.
%
%  OBJ = REMOVEFROMEXTRAPOLATIONMASK(OBJ, ROW, COL, DIM3, ...DIMn) removes
%  table cells from an nD table.
%
...
%  $Revision: 1.1.6.2 $    $Date: 2004/02/09 07:12:05 $ 
