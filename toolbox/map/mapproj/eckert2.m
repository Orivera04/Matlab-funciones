function [out1,out2,savepts] = eckert2(mstruct,in1,in2,object,direction,savepts)

%ECKERT2  Eckert II Pseudocylindrical Projection
%
%  This is an equal area projection.  Scale is true along the 55 deg,
%  10 min parallels, and is constant along any parallel and between
%  any pair of parallels equidistant from the Equator.  It is not free
%  of distortion at any point except at 55 deg, 10 min N and S along
%  the central meridian.  The break at the Equator introduces excessive
%  distortion there;  regardless of their appearance, Tissot indicatrices
%  are of indeterminate shape along the Equator.  This novelty projection
...
if nargin == 1                  %  Set the default structure entries
	  ...
	  mstruct.mapparallels = angledim(5510,'dms',mstruct.angleunits);
...
%  Projection transformation
	 ...
	 x = 2 * radius * long .* sqrt(4 - 3*sin(abs(lat)))/sqrt(6*pi);
...
% Inverse projection
	 ...
	 long = sqrt(6*pi) * x ./ (2*radius*sqrt(4 - 3*sin(abs(lat))) );
