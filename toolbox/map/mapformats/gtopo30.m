function [map,maplegend] = gtopo30(varargin)
%GTOPO30 Read 30-Arc-Sec global digital elevation data.
%
% [map,maplegend] = GTOPO30(filename,scalefactor) reads the GTOPO30 files
% and returns the result as a regular matrix map.  The filename is given as
% a string which does not include the extension.  If the files are not
% found on the MATLAB path, they may be selected interactively. Scalefactor
% is an integer, which when equal to 1 gives the data at its full
% resolution.  When scalefactor is an integer n larger than one, every nth
% point is returned. The map data is returned as an  array of elevations
% and associated regular matrix map legend. Elevations  are given in meters
% above mean sea level using WGS 84 as a horizontal datum.
%
% [map,maplegend] = GTOPO30(fname,scalefactor,latlim,lonlim) allows a
...
% concatenates data from multiple files within a GTOPO30 CD-ROM or
% directory  structure. The dirname input is a string with the name of the
% directory  which contains the GTOPO30 tile directories. Within the tile
% directories are the uncompressed files data files. The dirname for
% CD-ROMs distributed  by the USGS is the device name of the CD-ROM drive. 
%
...
%  $Revision: 1.1.6.1 $ $Date: 2003/08/01 18:21:06 $
...
for i = 1:length(do)
	lond = fname{i}(1);
	if lond == 'W'
		lonv = -1*str2num(fname{i}(2:4));
	else
		lonv = str2num(fname{i}(2:4));
	end
	lon = [lon lonv];
	latd = fname{i}(5);
	if latd == 'S'
		latv = -1*str2num(fname{i}(6:7));
	else
		latv = str2num(fname{i}(6:7));
	end
	lat = [lat latv];	
end	
...
for i = 1:length(uniquelats)
	for j = 1:length(uniquelons)
		...
		lonv = strrep(lonv,'-','');
        while length(lonv)< 3; lonv = ['0',lonv]; end
...
if length(uniquelats) == 1 & length(uniquelons) == 1
	[map,maplegend] = gtopo30(nfile{1,1},samplefactor,latlim,lonlim);
end
...
if length(uniquelats) == 1 & length(uniquelons) > 1
	...
	for j = 2:length(uniquelons)
		[map,maplegend] = gtopo30(nfile{1,j},samplefactor,latlim,lonlim);	
		tmap = [tmap map];
	end	
	clear map maplegend
...
if length(uniquelats) > 1 & length(uniquelons) == 1
	...
	for j = 1:length(uniquelats)
		[map{j},maplegend{j}] = gtopo30(nfile{j,1},samplefactor,latlim,lonlim);
...
if length(uniquelats) > 1 & length(uniquelons) > 1
	...
	for i = 1:length(uniquelats)
		tmaprow = [];
		for j = 1:length(uniquelons)
			[map,maplegend] = gtopo30(nfile{i,j},samplefactor,latlim,lonlim);
			tmaprow = [tmaprow map];
		end	
...
if isempty(wld_lat) & size(antarcfile,1) == 1 & size(antarcfile,2) == 1
	[map,maplegend] = gtopo30(antarcfile{1,1},samplefactor,latlim,lonlim);
end
...
if isempty(wld_lat) & size(antarcfile,1) == 1 & size(antarcfile,2) > 1 
	...
	for j = 2:size(antarcfile,2)
		[map,maplegend] = gtopo30(antarcfile{1,j},samplefactor,latlim,lonlim);	
		tmap = [tmap map];
	end	
	clear map maplegend
	map = tmap; maplegend = tmaplegend;
...
if ~isempty(wld_lat) & size(antarcfile,1) == 1 & size(antarcfile,2) == 1
	...
	for j = 1:size(wldfile,1)
		[map{j},maplegend{j}] = gtopo30(wldfile{j,1},samplefactor,latlim,lonlim);
		% need to flip matrix since we're starting in the top and working down
		tmap = [tmap;flipud(map{j})];
	end
	tmaplegend = maplegend{1};
...
% matrix 
if ~isempty(wld_lat) & size(antarcfile,1) == 1 & size(antarcfile,2) > 1
	[map,maplegend] = gtopo30(wldfile{1,1},samplefactor,latlim,lonlim);
	tmaplegend = maplegend;
	tmapcolumn = [];
	...
	for i = 1:size(wldfile,1)
		...
		for j = 1:size(wldfile,2)
			[map,maplegend] = gtopo30(wldfile{i,j},samplefactor,latlim,lonlim);
			tmaprow = [tmaprow map];
		end	
		tmapcolumn = [tmapcolumn;flipud(tmaprow)];
	end	
	...
	for i = 1:size(antarcfile,1)
		...
		for j = 1:size(antarcfile,2)
			[amap,amaplegend] = gtopo30(antarcfile{i,j},samplefactor,latlim,lonlim);
...
for i = 1:length(doEAST)
	lond = Efile{i}(1);
	if lond == 'W'
		lonv = -1*str2num(Efile{i}(2:4));
	else
		lonv = str2num(Efile{i}(2:4));
	end
	lon = [lon lonv];
end	
...
for i = 1:length(doWEST)
	lond = Wfile{i}(1);
	if lond == 'W'
		lonv = -1*str2num(Wfile{i}(2:4));
	else
		lonv = str2num(Wfile{i}(2:4));
	end
	lon = [lon lonv];
end	
...
if ~isempty(wld_lat) & isempty(antarc_lat)
	if length(wld_lat) == 1
		tmap = [];
		[map,maplegend] = gtopo30(wldfile{1,1},samplefactor,latlim,wlonlims{1,1});
		tmap = [tmap map]; tmaplegend = maplegend;
		for j = 2:size(wldfile,2)
			[map,maplegend] = gtopo30(wldfile{1,j},samplefactor,latlim,wlonlims{1,j});	
			tmap = [tmap map];
		end	
		clear map maplegend
		map = tmap; maplegend = tmaplegend;
...
if isempty(wld_lat) & ~isempty(antarc_lat)
	tmap = [];
	[map,maplegend] = gtopo30(antarcfile{1,1},samplefactor,latlim,alonlims{1,1});
	tmap = [tmap map]; tmaplegend = maplegend;
	for j = 2:size(wldfile,2)
		[map,maplegend] = gtopo30(antarcfile{1,j},samplefactor,latlim,alonlims{1,j});	
		tmap = [tmap map];
	end	
	clear map maplegend
	map = tmap; maplegend = tmaplegend;
...
if ~isempty(wld_lat) & ~isempty(antarc_lat)
	...
	for i = 1:size(wldfile,1)
		tmaprow = [];
		for j = 1:size(wldfile,2)
			[map,maplegend] = gtopo30(wldfile{i,j},samplefactor,latlim,wlonlims{i,j});
			tmaprow = [tmaprow map];
		end	
		tmapcolumn = [tmapcolumn;flipud(tmaprow)];
	end	
	...
	for i = 1:size(antarcfile,1)
		tmaprow = [];
		for j = 1:size(antarcfile,2)
			[amap,amaplegend] = gtopo30(antarcfile{i,j},samplefactor,latlim,alonlims{i,j});
			tmaprow = [tmaprow amap];
		end	
		tmapcolumn = [tmapcolumn;flipud(tmaprow)];
	end	
