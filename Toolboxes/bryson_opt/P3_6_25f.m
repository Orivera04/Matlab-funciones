% Script p3_6_25f.m; 727 max hf climbing turn with (Vf,psf) spec. using 
% FOPCF; s=[V ga ps h x y]'; u=[al sg]';                  3/94, 5/28/02
%
la0=[1.0572 .0853 .0366 1 0 0]; nu0=[.4887 .0366]; p=[la0 nu0];
name='maxalt'; tf=2.4; s0=[1 0 0 0 0 0]'; 
optn=optimset('Display','Iter','MaxIter',100); nc=2;
p=fsolve('fopcf',p,optn,name,s0,tf,nc); 
[f,t,y]=fopcf(p,name,s0,tf,nc);
W=180000; S=1560; rho=.002203; g=32.2; lc=2*W/(rho*g*S); Vc=sqrt(g*lc);
tc=lc/Vc; t=tc*t; c=180/pi; V=y(:,1); ga=y(:,2); ps=y(:,3); h=lc*y(:,4);
x=lc*y(:,5); y1=lc*y(:,6); lv=y(:,7); lg=y(:,8); lp=y(:,9); cga=cos(ga);
al1=12*pi/180; a0=.2476; a1=-.04312; a2=.008392; th=a0+a1*V+a2*V.^2;
N1=length(V); b1=-.08617; b2=1.996; c0=.1667; c1=6.231; alc=zeros(1,N1);    
sg=atan(lp./(cga.*lg)); cs=cos(sg); ss=sin(sg); ep=2/c; 
for i=1:N1, al=8/c; z=1;
  while abs(z)>1e-7,
   ca=cos(al+ep); sa=sin(al+ep); if al<al1, c2=0; else c2=-21.65; end
   ...
   za=lv(i)*(-th(i)*ca-V(i)^2*2*b2)+(lg(i)*cs(i)+...
      lp(i)*ss(i)/cga(i))*(-th(i)*sa+2*c2)/V(i); al=al-z/za;
  end; alc(i)=al;
end; V=Vc*V;  
%
