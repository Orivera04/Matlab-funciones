function [Price, PriceTree] = floorbybdt(BDTTree, varargin)
%FLOORBYBDT Price a floor from a BDT interest rate tree.
%
...
%
% See also BDTTREE, CFBYBDT, CAPBYBDT, SWAPBYBDT

%   Author(s): M. Reyes-Kattar 03/11/2001
%   Copyright 1998-2003 The MathWorks, Inc.
%   $Revision: 1.6.2.2 $  $Date: 2004/04/06 01:08:30 $

%---------------------------------------------------------
%Checking the input arguments.
%---------------------------------------------------------
if (nargin < 4)
   error('finderiv:floorbybdt:InvalidInputs','You must enter BDTTree, Strike, Settle and Maturity');
end

if ~isafin(BDTTree,'BDTFwdTree')
  error('finderiv:floorbybdt:InvalidTree','The first argument must be a BDT tree created by BDTTREE');
end
...
[Strike, Settle, Maturity, FloorReset, Basis, Principal] = ...
   instargfloor(varargin{1}, BDTTree.TimeSpec.ValuationDate, varargin{3:end});
...
if ~isempty(FloorSettle) & any(FloorSettle ~= Settle(1))
   warning('finderiv:floorbybdt:IgnoredSettle','Floors are valued at BDT Tree ValuationDate rather than Settle');
end
...
if (~all(ismember(AllTimes, TreeTimes)))
    ...
    if(ConstRate)
        if(ShowWarnings)
            warning('finderiv:floorbybdt:ResultsApproximated','Not all cash flows are aligned with the tree. Result will be approximated.');    
        end    	    
    else        
        if(ShowWarnings)
            warning('finderiv:floorbybdt:RebuildingTree','Not all cash flows are aligned with the tree. Rebuilding tree.');    
        end        
...
function CF = FindCF(BDTTree, EndTime, StartTimes, Strike, Principal, FloorReset)
