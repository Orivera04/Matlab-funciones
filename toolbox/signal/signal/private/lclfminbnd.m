...
%        x = fminbnd(f,2,5);
%
%   See also OPTIMSET, FMINSEARCH, FZERO, @, INLINE.
...
%   $Revision: 1.1.4.1 $  $Date: 2004/04/13 00:18:49 $
...
if nargin < 3
   error('fminbnd requires three input arguments.')
end
...
if ax > bx
   ...
   output = []; xf=[]; fval = [];
   if print > 0
        msg=sprintf(['\nExiting due to infeasibility: the lower bound exceeds the' ...
         ' upper bound.\n']);
        disp(msg)
   end
   return
