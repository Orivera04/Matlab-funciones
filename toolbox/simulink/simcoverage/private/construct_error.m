function varargout = construct_error( ids, errorType, errorMsg, throwFlag )
...
%   $Revision: 1.8 $  $Date: 2002/03/23 02:59:44 $
    ...
    if(throwFlag==-2) 
    	outError = [ 'Stateflow ' errorType ' Warning' ];
    else
       outError = [ 'Stateflow ' errorType ' Error' ];
    end	
    idsLen = length( ids );

    if idsLen,
	   outError = [ outError '( #' sprintf( '%d', ids(1) ) ];
       for i = 2:idsLen, outError = [outError, ', #', sprintf( '%d', ids(i) ) ]; end;
	   outError = [ outError ')' ];
    end
    outError = [ outError ': ' errorMsg ];
    ...
    nag.component   = 'Model Coverage';
    nag.sourceHId   = id;
    nag.ids         = ids;
    nag.blkHandles  = [];% ???? get_chart_block_handle_from_id_l(id);
    if ~isempty(id),  nag.openFcn = ''; end % ??['sf( ''Open'', ' int2str( id ) ' );' ]; end;
