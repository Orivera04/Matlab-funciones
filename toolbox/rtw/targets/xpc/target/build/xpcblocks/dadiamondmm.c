/* $Revision: 1.6.4.1 $ $Date: 2004/04/08 21:01:57 $ */
// dadiamondmm.c - xPC Target, non-inlined S-function driver for
// D/A section of Diamond Systems MM and MM-AT boards  
// Copyright 1996-2003 The MathWorks, Inc.


#define     S_FUNCTION_LEVEL    2
#undef      S_FUNCTION_NAME
#define     S_FUNCTION_NAME     dadiamondmm
...
#define RANGE_ARG              ssGetSFcnParam(S,1) // vector of double
...
#define NUM_R_WORKS            (MAX_CHAN * 2)
#define GAIN_R_IND             (0)
#define OFFSET_R_IND           (MAX_CHAN)
...
static void mdlStart(SimStruct *S)
{
    ...
    for (i = 0; i < numChans; i++) {
        ...
        if (range < 0) {
            RWork[GAIN_R_IND + chan]   = - RESOLUTION / (2 * range);
            RWork[OFFSET_R_IND + chan] = - range;

        } else if (range > 0) {
            RWork[GAIN_R_IND + chan] = RESOLUTION / range;
            RWork[OFFSET_R_IND + chan] = 0.0;

        } else {
            sprintf(msg, "range is too close to 0"); 
            ssSetErrorStatus(S, msg);
            return;
        }
    }
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    for (i = 0; i < numChans; i++) {
        ...
        count = (value + RWork[OFFSET_R_IND + chan]) * RWork[GAIN_R_IND + chan];

        if (count < 0) count = 0;
        if (count > MAX_COUNT) count = MAX_COUNT;

        rl32eOutpB(base + DAC_0_LSB + 2 * chan, count & 0xff);
        rl32eOutpB(base + DAC_0_MSB + 2 * chan, (count >> 8) & 0xf);
    }
#endif
}
