/* $Revision: 1.4 $ $Date: 2002/03/25 03:59:12 $ */
/* adbbpci20019.c - xPC Target, non-inlined S-function driver for analog input section of BB PCI-20019 module  */
/* Copyright 1996-2002 The MathWorks, Inc.
*/

#undef S_FUNCTION_NAME
#define S_FUNCTION_NAME adbbpci20019 
...
static void mdlInitializeConditions(real_T *x0, SimStruct *S)
{
      ...
      return;
       ...
       if (carrierid !=1 && carrierid !=3 && carrierid !=5 && carrierid !=7) {
			 sprintf(msg,"PCI-20019 (0x%x): PCI-20041C-2A,3A carrier board not present",base_addr);
			 ssSetErrorStatus(S,msg);
           		return;
       }
       modulepr=(ioaddress[0] & 0xe0) >> 5;
       if ((modulepr & (1 << (module -1))) !=0) {
			sprintf(msg,"PCI-20019 (Id: %x): not present on carrier board",module);
			ssSetErrorStatus(S,msg);
            return;
       }
       ...
       if (moduleid != 0xe5) {
			sprintf(msg,"PCI-20019 (Id: %x): module has wrong type or software channel selection is not enabled",module);
			ssSetErrorStatus(S,msg);
			return;
       }
