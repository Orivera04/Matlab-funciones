% Script p4_6_09n.m; DTDP for min tf with gravity to vf=0 and spec.
% (uf,yf) with DOPTN;	                             2/97, 3/28/02
%
clear; clear global; global yf g; yf=.2/.4935^2; g=1/3; 
th=[1.384 1.350 1.300 1.221 1.084 .804 .182 -.590 -.990 -1.172];
nu=[-.2567 .7197 -1.0796]; tf=2.0299; s0=[0 0 0 0]';
N=length(th); name='dtdpgt';
optn=optimset('Display','Iter','MaxIter',50);
p0=[th nu tf]; p=fsolve('doptn',p0,optn,name,s0);
[f,s,la0]=doptn(p,name,s0); thh=[th th(N)]*180/pi; 
