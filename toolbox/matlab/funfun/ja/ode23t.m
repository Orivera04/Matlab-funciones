...
%
% [T,Y] = ODE23T(ODEFUN,TSPAN,Y0) は、TSPAN = [T0 TFINAL] のとき、
...
%   
% [T,Y] = ODE23T(ODEFUN,TSPAN,Y0,OPTIONS) は、デフォルトの積分パラ
...
% トではすべての要素が1e-6)です。  
%   
% [T,Y] = ODE23T(ODEFUN,TSPAN,Y0,OPTIONS,P1,P2...) は、
...
% MASS(T,Y) が質量行列の値を出力する場合は、ODESET を使って関数 MASS 
% に 'Mass' プロパティを設定します。質量行列が定数の場合、行列は、
% 'Mass' オプションの値として使われます。状態依存の質量行列をもつ問題
% は、より難しいものになります。質量行列が状態変数 Y に依存せず、関数
% MASS が1つの入力引数 T と共に呼び出される場合は、'MStateDependence' 
% を 'none' に設定してください。質量行列が Ｙ に少し依存する場合は、
...
% またはJacobian プロパティを使ってスパース df/dy を設定します。
% 状態変数に強く依存する M(t,y) の場合、'MvPattern' をスパース行列 S 
% に設定してください。ここで、S は任意の k に対して、M(t,y) の(i,k)要素
% が y のj 要素に依存する場合、S(i,j)  = 1 で、他の場合は0である行列です。
%
% 質量行列が正則な場合、問題の解は、直接的なものになります。例題
% FEM1ODE, FEM2ODE, BATONODE, BURGERSODE を参照してください。
% M(t0,y0) が正則でない場合、問題は、微分代数方程式(DAE)になります。
% ODE23T は、インデックス 1 の DAEs を解きます。DAEs は、y0 が一致する
...
% M(t0,y0) が正則であるか否かのチェックをします。'InitialSlope' プロパ
% ティの値として、yp0 を与えます。デフォルトは、ゼロベクトルです。y0 と
% yp0 に矛盾が存在する場合、ODE23T は推定値としてそれらを取り扱い、
% 推定に近い矛盾のない値を計算しようとします。そして、問題を解きます。
% HB1DAE または AMP1DAE の例題を参照してください。
%
% [T,Y,TE,YE,IE] = ODE23T(ODEFUN,TSPAN,Y0,OPTIONS...) は、OPTIONS の 
% 'Events' プロパティが関数 EVENTS に設定されている場合は上記のように
...
% TSPAN または Y0 が空の場合、ODE23S は、ODE23S 引数リストで与えられない値
% を得るために、 ODE ファイル [TSPAN,Y0,OPTIONS] = F([],[],'init')をコール
% します。コールリストの最後の空の引数は、たとえば、ODE23S('F') のように
% 省略することができます。
%
...
% ODESET を使用して、JConstant を 'on'に設定してください。 F(T,[Y1 Y2 ...]) が
% [F(T,Y1) F(T,Y2) ...]を出力するようにODE ファイルがコードされている場合、
% Vectorized を'on'に設定してください。 
% dF/dy がスパース行列で、ODE ファイルが、 F([],[],'jpattern') が
% 1 と dF/dy の非ゼロを示す 0 のスパース行列を出力するように、コードされて
% いる場合、JPattern を'on'に設定してください。 
% F(T,Y,'jacobian') がdF/dy を出力するようにODE ファイルがコードされている
% 場合、Jacobian を'on' に設定してください。
%   
% ODE23T は、非特異な定数の質量行列 M をもつ問題  M*y' = F(t,y) を
% 解くことができます。F(T,Y,'mass') が定数の質量行列を出力するように、
% ODE ファイルがコードされている場合、'M' に質量を設定するためには、
% ODESET を使用してください。
% 質量のデフォルト値は、'none' です。ODE の他のソルバでは、
% 非定数の質量行列をもつ問題を解くことができます。ODE15S と ODE23T は、
% 特異質量行列をもつ問題を解くことができます。
%   
% [T,Y,TE,YE,IE] = ODE23T('F',TSPAN,Y0,OPTIONS) では、OPTIONS の Events
...
% ODE23S は、"free" interpolant を使用した、新しい修正 Rosenbrock (2,3) 法
% を実行します。局所的な補外は、行われません。 デフォルトでは、ヤコビアンは
% 数値で作成されます。
