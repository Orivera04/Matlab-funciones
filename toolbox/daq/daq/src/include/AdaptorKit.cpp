...
// $Revision: 1.3.4.4 $  $Date: 2003/12/04 18:39:13 $
#include <stdafx.h>
#include <math.h>
...
wTimerID(0)
{
    ...
    wTimerRes = min(max(tc.wPeriodMin, static_cast<UINT>(resolution)), tc.wPeriodMax);
...
//_RPT1(_CRT_WARN,"Error= %lf\n",thisptr->Error);
		...
		UINT newDelay = static_cast<UINT>(max(0.0,floor((thisptr->Period-thisptr->Error)*1000)));
		// Wait for 1ms less than needed
		if ( newDelay > 1 )
			newDelay -= 1;
		else
