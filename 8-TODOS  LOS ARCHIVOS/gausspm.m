function [A,bcol,fcol] = gausspm(A,tol)
...
cf = 0;
while (i <= m) & (j <= n)
   % Find maximal value in column j below row i 
   [p,k] = max(abs(A(i:m,j)));
   k=k+i-1;
   if (p <= tol)
   ...
   % increment counter cf of free variables and assign j-th column
      ...
      for k = [i+1:m]
       ...
       disp(['O(' int2str(k) ',' int2str(j) ',' num2str(-A(k,j)/A(i,j)) ')'])
       disp([blanks(10)])
 ...
 % backward elimination step
   for k = cb:-1:1 
    ...
    if k ~= 1  
     ...
     if A(p,bcol(k)) ~= 0 
     clc,home
     disp(['O(' int2str(p) ',' int2str(bcol(k)) ',' num2str(-A(p,bcol(k))) ')'])
     disp([blanks(10)])
