...
%   $Revision: 1.14.4.1 $  $Date: 2003/01/07 19:32:15 $
...
for ix = 1:length(indstr)
    ...
    elseif nhits1 && nhits2,
        error(sprintf('Name reference ''%s'' matches both a channel and a group.',str))
    elseif nhits2,
        % Group match
        if nhits2>1,
            warning(sprintf('Multiple matches for group name ''%s''.',str))
        end
        for jx=imatch2.'
        ...
        % Channel match
        if nhits1>1,
            warning(sprintf('Multiple matches for channel name ''%s''.',str))
        end
