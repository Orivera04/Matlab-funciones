function bdteditcreditcurvegui(varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
h1 = uicontrol('Parent',h0, ...
	...
	'String','Input Credit Curve Editor', ...
	'Style','text', ...
	'Tag','StaticText1');
h1 = uicontrol('Parent',h0, ...
	'Units','points', ...
	...
	'String','Done', ...
     ...
     strcat(['MainGUIHandle = findobj(''Tag'', ''MainGUI''); '...
          ...
          'CreditCurveLineHandle = findobj(CreditCurveAxesHandle, ''type'', ''line''); '...
          'axes(CreditCurveAxesHandle); '...
          'CurveDates = CreditCurve.CurveDates; '...
          'CreditRates = CreditCurve.CreditRates; '...
          'if isempty(CreditCurveLineHandle), line(CurveDates, CreditRates); '...
          'else, set(CreditCurveLineHandle, ''XData'', CurveDates, ''YData'', CreditRates); '...
          'end;'...
          'plotscale(0.10);'...
...
if (~isempty(CreditCurve))
     ...
     FirstRate = CreditRates(1, 1);
     LastRate = CreditRates(end, 1);
     FirstDate = CurveDates(1, 1);
     LastDate = CurveDates(end, 1);

     WorkCurveDates = linspace(FirstDate, LastDate, 10);
     WorkCreditRates = interp1(CurveDates, CreditRates, ...
          WorkCurveDates);

else
     WorkCurveDates = linspace(Settle, Maturity, 6);
     WorkCreditRates = [150 150 150 150 150 150]; 
     
end

%Call the curve editor GUI
bdteditcreditcurvegui(WorkCurveDates, WorkCreditRates, ...
     [(min(WorkCurveDates)- 10) (max(WorkCurveDates)+ 8) ...
          0 (max(WorkCreditRates) + 300)]);
...
set(CurveAxes, 'ButtonDownFcn', 'bdteditcreditcurvegui(''Drop'')' );
...
Line = line( CurveData.XLine, CurveData.YLine, ...
    'ButtonDownFcn', 'bdteditcreditcurvegui(''CurveLine'')', ...
...
for iline=1:length(XPoints),
  Points(iline) = line(XPoints(iline), YPoints(iline), ...
      ...
      'ButtonDownFcn', 'bdteditcreditcurvegui(''ControlPoint'')', ...
      'EraseMode', 'xor', ...
...
set(gcf,'WindowButtonMotionFcn', 'bdteditcreditcurvegui(''Dragging'')' );
set(gcf,'WindowButtonUPFcn', 'bdteditcreditcurvegui(''Drop'')' );
