function ret = getIRInfo_C6711DSK (modelName, RTWSampleTimes,adcSampleTime,dacSampleTime)

% $RCSfile: getIRInfo_C6711DSK.m,v $
% $Revision: 1.1.6.1 $ 
% $Date: 2004/01/22 18:37:26 $
% Copyright 2001-2003 The MathWorks, Inc.

try
    
    % DSK has fixed CPU clock rate
    CPU_CLOCK_RATE = 150000000;
    
    ADCblock = find_system(modelName,'FollowLinks','on','LookUnderMasks','on','MaskType','C6711DSK ADC');
    if isempty(ADCblock)
        ADCblock = find_system(modelName,'FollowLinks','on','LookUnderMasks','on','MaskType','TI TMDX326040A ADC');
    end;
    DACblock = find_system(modelName,'FollowLinks','on','LookUnderMasks','on','MaskType','C6711DSK DAC');    
    if isempty(DACblock)
        DACblock = find_system(modelName,'FollowLinks','on','LookUnderMasks','on','MaskType','TI TMDX326040A DAC');
    end;
