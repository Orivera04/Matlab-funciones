/* $Revision: 1.2.2.2 $ */
/*
 *  IIR_DF2T_A0SCALE_DZ_RT.C - DSP filter runtime helper function
 *
...
EXPORT_FCN void MWDSP_IIR_DF2T_A0Scale_DZ(const real_T          *u,
                               creal_T               *y,
                               ...
                               const boolean_T       one_fpf)
{
    ...
    for (k=0; k < numChans; k++) {
        ...
        while (i--) {
             ...
             */
            for (j=0; j < lenMIN; j++) {
                filt_mem->re     = next_mem->re     + in * num->re     - CMULT_RE(*y, *den);
                (filt_mem++)->im = (next_mem++)->im + in * (num++)->im - CMULT_IM(*y, *den);
                ++den;
            }
