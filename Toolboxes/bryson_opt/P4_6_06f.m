% Script p4_6_06f.m; DTDP for min tf to vf=0 and specified (uf,yf)
% using DOPTF; (u,v) in uf, (x,y) in uf^2/a, t in uf/a;
%                                                  3/97, 3/28/02
%
clear; clear global; global yf; yf=.2/.6753^2; N=10; s0=[0 0 0 0]'; 
la0=[-.3863 -.9257 -1.2434 0]; u0=1; nu=[-.3863 .9257 -1.2434];
tf=1.4890; name='dtdpt'; 
optn=optimset('Display','Iter','MaxIter',50); p0=[la0 nu tf];
p=fsolve('doptf',p0,optn,name,u0,s0,N);
[f,s,th,la]=doptf(p,name,u0,s0,N); t=tf*[0:1/N:1];
u=s(1,:); v=s(2,:); y=s(3,:); x=s(4,:); thh=[th th(N)]*180/pi;
%
