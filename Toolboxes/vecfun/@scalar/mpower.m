...
%^  Power.
%   U = S^T is S to the T power, where S and T both are
%   scalar functions.
...
findA=any(Aname=='+') | any(Aname=='-') | any(Aname=='*') | any(Aname=='/') | any(Aname=='\');
findB=any(Bname=='+') | any(Bname=='-') | any(Bname=='*') | any(Bname=='/') | any(Bname=='\');
if findA & findB
   C.f=['(' Aname ')^(' Bname ')'];
elseif findA & ~findB
   C.f=['(' Aname ')^' Bname];
