...
%
%   T2 = PRUNE(T1,NODES,'EXCLUSIVE') only the descendants of NODES will be
%   pruned. Then NODES will become leaves as long as they do not have a
%   predecessor in the list NODES. In this case pruning is the process of
%   reducing a tree by turning some branch nodes into leaf nodes, and
%   removing the leaf nodes under the original branch.
%  
%   Examples:
%
...
if nargin==3
    if strmatch(lower(varargin{1}),'exclusive')  %#ok
        exclusiveMode = true;
    else 
        error('Bioinfo:UnknownParameterName','Unknown input argument');
    end
end
