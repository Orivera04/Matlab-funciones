...
% 
% これにより、ZPKモデルの様々なプロパティ(詳細は、LTIPROPS を参照)を設定
% します。既存のLTIモデル REFSYS のすべての LTI プロパティを継承して
...
% * Z と P は、NY*NU のセル配列で、Z{i,j} と P{i,j} は、入力 j から出力i
%   までの伝達関数の零点と極を表します。
...
%
% 極-零点-ゲインモデルの配列:
% 上記の Z, P に対して ND セル配列を、 K に対して ND 配列を利用すること
% で、ZPKモデルの配列を作成することができます。たとえば、Z, P, K がサイ
% ズ [NY NU 5] の3次元配列とすると、
% 
%    SYS = ZPK(Z,P,K) 
% 
...
%
% 変換:
% SYS = ZPK(SYS) は、任意の LTI モデル SYS を ZPK 表現に変換します。結果
% は、ZPKオブジェクトになります。  
%
% SYS = ZPK(SYS,'inv') は、状態空間から ZPK モデルへの変換に高速なアルゴ
