...
%   deviation from 0.
%
%   B = FIRCLS1(N,WO,DP,DS,'high') is a highpass filter.  For this case, the
%   order N must be even.  Otherwise, the order will be incremented by one.
%
...
%   $Revision: 1.11.4.2 $  $Date: 2004/04/13 00:17:52 $
...
if EDGE
    if isempty(wt),
        error('WT cannot be empty.');
    elseif prod(size(wt)) > 1
...
[n,msg1,msg2] = firchk(n,1,pass_type);
error(msg1);
warning(msg2);
