 ...
 * $Revision: 1.1.6.2 $ $Date: 2004/04/08 21:02:57 $         
 */


#define         S_FUNCTION_LEVEL        2
#undef          S_FUNCTION_NAME
#define         S_FUNCTION_NAME         memsbs25x0write
...
static void mdlInitializeSizes(SimStruct *S)
{
   int_T status_port;
    ...
    ssSetInputPortWidth(S, 0, (int_T)mxGetPr(SIZEOF_ARG)[0]>>2);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    uint32_T  *u = (uint32_T *)ssGetInputPortSignal(S,0);
      ...
      for (i=0; i<((uint32_T)mxGetPr(SIZEOF_ARG)[0]/4); i++) { 
          net_ram[((uint32_T)mxGetPr(ADDRESS_ARG)[0]/4)+i]=u[i];
      }
    ...
    if( ssGetNumOutputPorts(S) > 0) {
        uint8_T  *ps = (uint8_T *)ssGetOutputPortSignal(S,0);
