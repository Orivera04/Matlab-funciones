...
%   $Revision: 1.4.2.2 $  $Date: 2004/02/09 07:53:25 $


if nargin<2
   VarName= m.Yinfo.Name;
end
if ~isempty(VarName)
   action='figure';
   if nargin>1
...
bold=[1;1; zeros(length(Expr),1);1;zeros(size(chModel,1),1)];
indent=[0;0;ones(length(Expr),1);0;ones(size(chModel,1),1)];
