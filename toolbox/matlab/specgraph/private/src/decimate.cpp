...
public:
    ...
    vert_info() : Q(Mat4::zero) { norm=0.0; }
};
...
static
void compute_pair_info(Model& m, pair_info *pair)
{
    Vertex *v0 = pair->v0;
    Vertex *v1 = pair->v1;
	...
	pair->cost += pair_mesh_penalty(m, v0, v1, pair->candidate);
...
void decimate_contract(Model& m)
{
	...
	*logfile << "#$cost " << m.validFaceCount << " "
    ...
    m.validVertCount--;  // Attempt to maintain valid vertex information
}
...
void decimate_init(Model& m, real limit)
{
    ...
    //else cout << "  Decimate:  Ignoring non-edge pairs [limit=0]." << endl;

    //cout << "  Decimate:  Designated " << pair_count << " pairs." << endl;
}

void decimate_cleanup()
{
    int i,j,len;
    ...
    for(i=0; i<vinfo.length(); i++)  
      {
	  pair_buffer& pairs = vinfo(i).pairs; 
	  len = pairs.length();
	  for (j=0; j<len; j++) delete_pair(pairs(j));
      }
