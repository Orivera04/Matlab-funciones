...
%
num = 11.7; den = [conv([0.05 1],[0.1 1]) 0];
%
w = [.1:.1:1 2:30 75]; [gain phase] = bode(num,den,w);
hold on; plot(phase,20*log10(gain),'g-'); hold off;
[Gm,Pm,Wcg,Wcp] = margins(num,den);
hold on; plot([0 Pm]-180,[0 0],'y-'); hold off;
text(Pm-180,0,sprintf('Phase margin = %g',Pm));
hold on; plot([-180 -180],[0 -20*log10(Gm)],'y-'); hold off;
text(-180,-20*log10(Gm),sprintf('Gain margin = %g db',20*log10(Gm)));
%
w = [1:3 5 7 12 15 20 25]; [gain phase] = bode(num,den,w);
