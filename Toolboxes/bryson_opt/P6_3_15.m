% Script p6_3_15.m; disc. ctrl. helic. (OH-6A) deceleration to hover w.
...
C=[zeros(4,8) eye(4)]; N=zeros(12,4); Qy=eye(4); Q=C'*Qy*C; R=Qy;
tf=6; Ns=120; Ts=tf/Ns; s0=[10 1 0 -.3555 0 0 0 .1714 0 -15 0 1.5]';
[k,S,ev]=lqrd(A,B,Q,R,Ts); 
[Ad,Bd,Qd,Nd,Rd]=cvrtj(A,B,Q,N,R,Ts); k1=dlqr(Ad,Bd,Qd,Rd,Nd);
t=tf*[0:1/Ns:1]'; s=dinitial(Ad-Bd*k,Bd,eye(12),zeros(12,4),s0,Ns+1);
d=-k*s'; 
%
...
subplot(212), zohplot(t',d(1,:)); hold on; zohplot(t',d(2,:),'r--');
zohplot(t',d(3,:),'k-.'); zohplot(t',d(4,:),'c:'); grid
