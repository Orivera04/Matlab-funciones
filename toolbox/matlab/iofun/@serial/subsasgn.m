...
%   $Revision: 1.7.4.7 $  $Date: 2004/01/16 20:04:45 $
...
if isempty(Obj)
   ...
   elseif length(Value) ~= length(Struct.subs{1})
      ...
      error('MATLAB:serial:subsasgn:index_assign_element_count_mismatch', ...
           ['In an assignment A(I)=B, the number of elements in B and I must be the same.']);
   elseif Struct.subs{1}(1) <= 0
      error('MATLAB:serial:subsasgn:badsubscript', 'Subscript indices must either be real positive integers or logicals.')
   else
      % Ex. s(2) = serial('COM1'); where s is originally empty.
      error('MATLAB:serial:subsasgn:badsubscript', 'Gaps are not allowed in instrument array indexing.');
   end
end
...
errorId  = 'MATLAB:serial:subsasgn:invalidSubscriptExpression';
error4   = 'In an assignment A(I)=B, the number of elements in B and I must be the same.';
errorId4 = 'MATLAB:serial:subsasgn:index_assign_element_count_mismatch';
...
if ~isempty(prop1)
   ...
   if ~(isa(Value, 'instrument') || isempty(Value))
      error('MATLAB:serial:subsasgn:invalidConcat', 'Only instrument objects may be concatenated.');
   end
   
   % Ex. s(1) = [] and s is 1-by-1.
   if ((length(Obj) == 1) && isempty(Value))
       error('MATLAB:serial:subsasgn:invalidAssignment', 'Use CLEAR to remove the object from the workspace.');
   end

   % Error if index is a non-number.
   for i=1:length(index1)
       if ~isnumeric(index1{i}) && (~(ischar(index1{i}) && (strcmp(index1{i}, ':'))))
           error('MATLAB:serial:subsasgn:badsubscript', ['Unable to find subsindex function for class ' class(index1{i}) '.']);
       end
   end
...
try
   ...
   if length(type) ~= numel(type)
       error('MATLAB:serial:subsasgn:nonMatrixConcat', 'Only a row or column vector of instrument objects can be created.')
   end
...
try
   ...
   lasterr('Index exceeds matrix dimensions.', 'MATLAB:serial:subsasgn:badsubscript');
   errflag = 1;
   return;
end
