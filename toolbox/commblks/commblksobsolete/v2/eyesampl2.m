function  [sys, x0, str, ts]  = eyesampl2(t,x,u,flag,...
    timeRange, time_offset, time_decision, boundary, storeLength, ...
        diagType, eyeLine, scatterLine, sampleTime)
% EYESAMPL2 Simulink eye diagram and scatter plot --- sampled time version.
%   [SYS, X0, STR, TS]  = EYESAMPL2(T,X,U,FLAG,TIMERANGE,
%       BOUNDARY,STORELENGTH,EYELINE,SCATTERLINE,SAMPLETIME)
%
...
%       $Revision: 1.1.6.1 $ $Date: 2003/01/26 18:45:51 $
...
timeDead = eps^(2/3) * timeRange(1);
if ( abs(rem(time_offset + sampleTime/2, sampleTime) - sampleTime/2) > timeDead)
    strErr = sprintf('The Trace offset must be a multiple of the sample time, %g.',sampleTime);
    error(strErr);
end

timeRange(2) = rem(rem(time_offset, timeRange(1)) + timeRange(1), timeRange(1));

timeRange(3) = time_decision;

% Check that the decision point in between the end points of the plot when
% scatter plot is enabled
if (scatterLine(1) ~= 0)
    timeRangeEnd = timeRange(2) + timeRange(1);
    if (timeRange(3) < timeRange(2)) | (timeRange(3) > timeRangeEnd)
        strErr = sprintf( ...
            'The Decision point parameter, %g, is invalid.  Its value must be greater than or equal to %g and less than or equal to %g.', ...
             timeRange(3), timeRange(2), timeRangeEnd);
         error(strErr);
    end
end
...
triggered = 0;
if (abs(mod_time - timeRange(3)) < timeDead)
  triggered = 1;
end;
x(2) = mod_time;

if(scatterLine(1) ~= 0)
    if ( abs(rem(time_decision + sampleTime/2, sampleTime) - sampleTime/2) > timeDead)
        strErr = sprintf('The Decision point must be a multiple of the sample time, %g.',sampleTime);
        error(strErr);
    end
end
...
%=============================================================================
% LocalBlockNameChangeFcn
% Function that handles name changes on the Discrete Eye-Scatter.
%=============================================================================
%
...
%=============================================================================
% LocalBlockLoadCopyFcn
% Function that initializes the Discrete Eye-Scatter's UserData when it is
...
%=============================================================================
% LocalBlockDeleteFcn
% Function that handles the Discrete Eye-Scatter's deletion from a block
% diagram.
%=============================================================================
%
...
%=============================================================================
% GetEyesamplFigure
% Retrieves the figure window associated with this S-function Discrete Eye-Scatter
...
%=============================================================================
% SetEyesamplFigure
% Stores the figure window associated with this S-function Discrete Eye-Scatter
...
% Creates the figure window associated with this S-function Discrete Eye-Scatter.
%=============================================================================
%
...
if isempty(b)
   figureHandle = figure(...
      ...
      'Visible',      'off', ...
      'DeleteFcn',    'eyesampl2([],[],[],''DeleteFigure'')'...
      );
else
  figureHandle = b;
end;
...
%  warning(warnmsg);
  ...
  callbacks={
    'CopyFcn',       'eyesampl2([],[],[],''CopyBlock'')' ;
    'DeleteFcn',     'eyesampl2([],[],[],''DeleteBlock'')' ;
    'LoadFcn',       'eyesampl2([],[],[],''LoadBlock'')' ;
    'StartFcn',      'eyesampl2([],[],[],''Start'')' ;
    'StopFcn'        'eyesampl2([],[],[],''Stop'')' ;
    'NameChangeFcn', 'eyesampl2([],[],[],''NameChange'')' ;
  };
