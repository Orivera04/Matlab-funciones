...
3. The textbook discusses the following algorithm:
    ...
    This implements the trapezoidal rule.
    The function f(x) is integrated over the interval [a,b].

    To run the example for Algorithm 7.1 the user needs to use the script file 
    named a7_1.m.  This is accomplished by executing the Matlab command:

    a7_1 

4. The Matlab script in  a7_1.m  will call the subroutine named  traprl.m 
    which is included in the sub directory or folder named  chap_7.
    ...
    in the proper sub directory or folder along with the script file a7_1.m  and
    functions (subroutines) files  traprl.m  and grpoly.m.  These commands are:
...
5. Once the process of writing a function M-files has been mastered,
    ...
    delete these lines from the script file  a7_1.m.
