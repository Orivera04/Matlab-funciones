...
%  24 August 1995, revised 29 April 1998, 27 Feb 1999, 9 May 2004
...
if nr ~= round(nd/ncol)
   ...
   fprintf('\tnrow = %f\tncol = %d\n',nr,ncol);
   fprintf('Data cannot be reshaped into a matrix with %d columns',ncol)
   if nrowl>0
     fprintf('\nHere are the column labels\n\t');
     for j=1:ncol,  fprintf('%s  ',labels(j,:));  end,  fprintf('\n');
   end
   error(sprintf('Failed to read data from %s',fname))
end
