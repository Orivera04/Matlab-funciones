...
%   $Revision: 1.5.4.3 $  $Date: 2004/01/16 20:04:21 $
    ...
    if ~isempty(varargin{i}),
        if isempty(c),
            ...
            if ~isa(varargin{i},'instrument'),
                error('MATLAB:serial:horzcat:nonInstrumentConcat', 'Instrument objects can only be concatenated with other instrument objects.')
            end
            c=varargin{i};
        else
...
if length(c.jobject) ~= numel(c.jobject)
   error('MATLAB:serial:horzcat:nonMatrixConcat', 'Only a row or column vector of instrument objects can be created.')
end
