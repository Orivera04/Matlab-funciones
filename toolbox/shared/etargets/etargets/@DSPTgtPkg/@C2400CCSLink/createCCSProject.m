function createCCSProject (h, target_state, modelInfo, sources, buildOptions)
...
% $RCSfile: createCCSProject.m,v $
% $Revision: 1.1.6.2 $ $Date: 2004/04/01 16:17:18 $
...
disp('### Creating project in Code Composer Studio(tm)');
...
for i=1:length (sources),
    if ~isempty (sources{i}),
        if isempty(findstr(sources{i},'rt_nonfinite.c'))
...
h.exportCCSObject (target_state.ccsObj, modelInfo.name);
...
curOpts(2).optstring = build_options{2};

% set build options for linker, and compiler
for i=1:2;
    ccsObj.setbuildopt (curOpts(i).name, curOpts(i).optstring);
end

% [EOF] createCCSProject.m
