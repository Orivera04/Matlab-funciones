% Script p4_7_22b.m; max tf for spec. (Vf,gaf,hf), glider w. parabolic
% lift-drag polar using FOPTB; s=[V ga h]'; h in l, V in sqrt(gl), t in
% sqrt(l/g), l=2m/(rho*S*Cla), alm=sqrt(Cdo/(eta*Cla));   4/97, 3/28/02
%
sf=[4 0 -8.1723]; nu=[18.1444 9.1042 3.9801]; tf=30.0043; p0=[sf nu tf];
eta=.5; c=pi/180; name='gldt'; s0=[4 0 0]'; 
optn=optimset('Display','Iter','MaxIter',50);
p=fsolve('foptb',p0,optn,name,s0);
[f,t,y]=foptb(p,name,s0); N=length(t); V=y(:,1); ga=y(:,2); h=y(:,3);
lv=y(:,4); lg=y(:,5); al=lg./(2*eta*V.*lv);    
%
...
%
figure(3); clf; es=2.614^2/2; plot(V.^2/2,h,8,h(1),'ro',8,0,...
