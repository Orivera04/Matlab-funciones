...
case 'Resize',
    ...
    %
    axesGeom = i_CreateAxesGeom(scopeUserData);
    [modified, scopeUserData] = i_ResizeAxes(scopeFig, scopeUserData, ...
                                             axesGeom);
...
case 'BlockOpen',
    ...
    [modified, scopeFig, scopeUserData] = ...
    ...
    %
    if strcmp(get_param(scopeBlk,'Wireless'), 'on'),
...
case 'AxesClick'
 ...
 case 'SetSelectedAxes',
  ...
  scopeUserData  = i_ChangeBlueAxes(scopeFig,scopeUserData,axH);
  set(scopeFig,'UserData',scopeUserData);
...
case 'BlockStart',
    ...
    %
    [scopeUserData, modified] = i_SimulationStart(scopeFig, scopeUserData);
    if modified == 1,
        set(scopeFig, 'UserData', scopeUserData);
    end
...
case 'BlockTerminate',
    ...
    [scopeUserData, modified] = i_SimulationTerminate(scopeFig, scopeUserData);
    if modified == 1,
        set(scopeFig, 'UserData', scopeUserData);
    end
...
case 'ExtLogInitialize',
    ...
    [scopeUserData, bModified] = i_SimulationStart(scopeFig, scopeUserData);
    if bModified == 1,
        set(scopeFig, 'UserData', scopeUserData);
    end
...
case 'ExtLogTerminate',
    ...
    [scopeUserData, bModified] = i_SimulationTerminate(scopeFig, scopeUserData);
    if bModified == 1,
        set(scopeFig, 'UserData', scopeUserData);
    end
...
case {'BlockDelete', 'BlockIsBeingDestroyed'},
    ...
    %
    currentScope = get_param(bdroot(scopeBlk), 'FloatingScope');
    if ~strcmp(currentScope, ''),
        if get_param(currentScope, 'Handle') == scopeBlkH,
            set_param(bdroot(scopeBlk), 'FloatingScope', '');
        end
    end
    ...
    if ishandle(scopeFig),
        ...
        for i=1:nAxes,
            ...
            if ishandle(axUserData.propDlg),
                ...
                dlgName    = i_GetAxesPropDlgName(ax, scopeBlk, axesIdxStr);
...
case 'PropDialogApply',
    ...
    [modified, scopeUserData] = i_UpdateAxesConfig(scopeFig, scopeUserData);
    if (modified),
        set(scopeFig, 'UserData', scopeUserData);
    end
...
case 'AxesContextMenu',
    ...
    menuItem      = varargin{2};
    [modified, scopeUserData] = i_ManageContextMenuCB( ...
...
case 'ScopeBar',
    ...
    [modified, scopeUserData] = i_ManageScopeBar( ...
...
%      varargout{1} = i_GetSelection(BlockHandle, AxesNumber);
 ...
 case 'RemoveSelection'
  ...
  if ~floating,
    ...
    i_RemoveSelection(BlockHandle, AxesNumber, remSel);
  end
...
function i_UpdateTitles(scopeUserData),
...
function [modified, scopeUserData] = i_UpdateAxesConfig(...
...
if (nAxesNeeded ~= nAxes),
    ...
    if ~strcmp(scopeUserData.tickLabelOpt, axesGeom.tickLabelOpt),
        ...
        [modified, scopeUserData] = i_ResizeAxes( ...
...
%
scopebar(scopeFig, 'ZoomModeSwitch', get_param(scopeUserData.block,'ZoomMode'));
...
function i_RestoreDefaultAxesLimits(scopeFig, scopeUserData),
...
function [modified, scopeFig, scopeUserData] = ...
    i_ProcessOpenRequest(scopeFig, scopeUserData),
...
if scopeFig ~= INVALID_HANDLE,
    ...
    case 'off',
        ...
        if ~strcmp(simStatus, 'stopped'),
            ...
            i_RestoreDefaultAxesLimits(scopeFig, scopeUserData);
...
function [scopeUserData, modified] = i_SimulationStart(scopeFig, scopeUserData),
...
i_UpdateAxesConfig(scopeFig,scopeUserData);
i_CreateLinesIfNeeded(scopeFig,scopeUserData);
i_SetEnableForNonRuntimeCtrls(scopeFig, scopeUserData, 'off');
i_RestoreDefaultAxesLimits(scopeFig, scopeUserData);
...
function [scopeUserData, modified] = ...
    i_SimulationTerminate(scopeFig, scopeUserData),
...
function [modified, scopeUserData] = ...
    i_ResizeAxes(scopeFig, scopeUserData, axesGeom),
...
function i_UpdateYLimits(scopeFig, scopeUserData),
...
function error = i_ApplyAxesPropDialog(dialogFig, axesIdx),
...
i_UpdateYLimits(scopeFig, scopeUserData);
i_UpdateTitles(scopeUserData);
...
case 'OK',
    error = i_ApplyAxesPropDialog(dialogFig, axesIdx);
    if ~error,
        close(dialogFig);
    end
...
case 'Apply',
    i_ApplyAxesPropDialog(dialogFig, axesIdx);
...
function [modified, scopeUserData] = i_ManageContextMenuCB( ...
    scopeFig, scopeUserData, contextAxes, menuItem),
...
function i_FindRequestAllAxes(scopeFig, scopeUserData),
...
try,
  ...
  %
  simscope('ResizeHiLites', get_param(block,'Figure'));
end
...
function [modified, scopeUserData] = i_ManageScopeBar(scopeFig, scopeUserData, buttonType, buttonAction),
...
case 'ActionIcon',
    ...
    case 'Find',
        i_FindRequestAllAxes(scopeFig, scopeUserData);
...
%
function scopeFigFocusChanged = i_GrabWirelessScopeFocus(scopeFig)
  scopeUserData  = get(scopeFig, 'UserData');
  ...
  if ~isempty(oldScope) 
    ...
    if ~strcmp(scopeBlockName,oldScope)
      ...
      i_SetWirelessScopeLockdownMode(oldScopeUserData, 'on');
    end
  else,
    scopeFigFocusChanged = 1;
  end
...
%
function i_SetWirelessScopeLockdownMode(scopeUserData, state)
    ...
    %
    scopeFig = get_param(scopeUserData.block, 'Figure');
...
%endfunction i_SetWirelessScopeLockdownMode
...
%
%
function scopeUserData  = i_ChangeBlueAxes(scopeFig,scopeUserData,axH),
  ...
  % scope to be the current scope in focus.
  %
  scopeFigFocusChange = i_GrabWirelessScopeFocus(scopeFig);
  ...
  %
  set_param(scopeUserData.block, 'SelectedAxesIdx', ax);
  scopeLockedDown = onoff(get_param(scopeUserData.block, 'LockDownAxes'));
  ...
  if (ax ~= oldAxes | scopeFigFocusChange | scopeLockedDown )
    ...
    %
    i_SetWirelessScopeLockdownMode(scopeUserData, 'off');
