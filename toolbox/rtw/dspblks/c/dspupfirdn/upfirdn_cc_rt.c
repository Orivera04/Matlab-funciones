/*
 *  upfirdn_cc_rt.c - FIR Rate Converter block runtime helper function
 *
 *  Copyright 1995-2003 The MathWorks, Inc.
 *  $Revision: 1.4.2.3 $  $Date: 2004/04/12 23:49:31 $
 */
#include "dsp_rt.h"

EXPORT_FCN void MWDSP_UpFIRDn_CC(   const creal32_T *u,            /* input port*/
        ...
        for (i=0; i < inFrameSize; i++) {
            *(tap0+mIdx) = *u++;
            ...
            for (j=0; j < iFactor; j++) {
                ...
                creal32_T *tap    = tap0 + mIdx - ( (iFactor-j)*n0 );
                ...
                while (m--) {
                    ...
                    rsum += (tap->re) * (cff->re) - (tap->im) * (cff->im);
                    isum += (tap->im) * (cff->re) + (tap->re) * (cff->im);
                    ++cff;
                }               
                sums[j].re = rsum;
                sums[j].im = isum;
