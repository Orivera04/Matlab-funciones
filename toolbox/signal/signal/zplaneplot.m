...
%   Author(s): T. Krauss, P. Costa
%   Copyright 1988-2004 The MathWorks, Inc.
%   $Revision: 1.11.4.4 $  $Date: 2004/04/13 00:19:10 $

error(nargchk(2,5,nargin));

% Parse the optional input arguments.
[ax, markers, color, msg] = parseinputs(varargin{:});
error(msg);
...
% Annotate the axes
set(get(ax,'xlabel'),'string',xlate('Real Part'));
set(get(ax,'ylabel'),'string',xlate('Imaginary Part'));
...
%                       Utility Functions
%-------------------------------------------------------------------
function [ax,markers,color,msg] = parseinputs(varargin)
%
% Outputs:
...
if isequal(lenZ+lenP,length(propcell)) | lenZ+lenP==0,    
    ...
    error(['A ', propstr, ' must be specified for each column of Z and P.']);
end
