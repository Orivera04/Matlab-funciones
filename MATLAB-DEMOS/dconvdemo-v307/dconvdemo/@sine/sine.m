function f = sine(varargin)
%SINE Create a SINE object.
%   f = SINE('Parameter1','Value1','Parameter2','Value2',...)
%   create an object of class 'sinusoid' with the given parameters.
%
%   f = SINE(Signal,'Parameter1','Value1','Parameter2','Value2',...)
%   uses the SINE object Signal as a template.
%
...
if (nargin > 0) & isa(varargin{1},'sinusoid')
   f = varargin{1};
...
f.YData = f.Amplitude*sin(2*pi/f.Period*(0:f.Length-1)+f.Phase);
f.XData = f.XData';
f.YData = f.YData';
if ~isa(f,'sine');
   f = class(f,'sine');
