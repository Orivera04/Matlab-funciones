function [A11,A12,A13,A14,A15,A16,A21,A22,A23,A24,A25,A26,A31,A32,A33,A34,...
          ...
          gref,Pmf ] = psbregulset(Vinf,mac,deltadeg)
...
nom=eval(mv{2});			% nominal parameters
sta=eval(mv{3});		% stator parameters
Rs=sta(1); Ll=sta(2); Lmd=sta(3); Lmq=sta(4);
fld=eval(mv{4});		% field parameters
Rfd=fld(1); Llfd=fld(2);
dam=eval(mv{6});		% damper parameters
Rkd=dam(1); Llkd=dam(2); Rkq=dam(3); Llkq=dam(4);
mec=eval(mv{7});		% mechanical parameters
H=mec(1); F=mec(2);
ini=eval(mv{8}); th_init=ini(2);
...
[gref,Pmf]=psbreguldelta(deltadeg);
