function [smod,psi]=daugment(smod,p,initpsi,DO_DESIGNTYPE)
...
%  $Revision: 1.2.2.4 $  $Date: 2004/04/04 03:26:39 $
...
if nargin<3
    initpsi=(2*sum(log(abs(diag(R)))))./(size(X,2));
end
...
if nc*(nf+k)<2.5e6;
    ...
    for n=1:p
        ...
        % update stuff
        initpsi=initpsi+log(1+mx)./k;
        if p>1
            % update Ai for next iteration
            ri=cholupdate(ri,(ri'*d(:,i))/sqrt(1+mx),'-');
        end
        if ~allowreps(smod)
    ...
    for n=1:p
        ...
        if p>1
            ...
            d= ri*maxln';
            % update Ai for next iteration
            ri= cholupdate(ri,(ri'*d)/sqrt(1+mx),'-');
        end
