function [IVar, IndexSet] = instdelete(varargin)
%INSTDELETE Complement of a subset of instruments found by matching conditions.
%
%   ISubSet = instdelete(InstSet, 'FieldName', FieldList,'Data' , DataList, ...      
...
%
%   Output:
%     ISubSet - Variable containing instruments not matching the
%     input criteria. Instruments are deleted from ISubSet if all the
%     Field, Index, and Type conditions are met.  An instrument meets 
%     an individual Field condition if the stored FieldName data matches 
%     any of the rows listed in the DataList for that FieldName.
%
%
%   Examples:
...
%
%   2) Create a new variable, InstSet, with all options deleted. 
%      InstSet = instdelete(ExampleInst, 'Type','Option');
%      instdisp(InstSet)
%
%   See also INSTSELECT, INSTFIND, INSTGET, INSTADDFIELD.

%   Author(s) : J. Akao 31-Mar-1999
%   Copyright 1995-2002 The MathWorks, Inc. 
%   $Revision: 1.10 $  $Date: 2002/04/14 21:39:58 $

%-----------------------------------------------------------------------
% Call instfind to do the matching
%-----------------------------------------------------------------------
IndexKill = instfind(varargin{:});

% The instrument variable is the first in
IVar = varargin{1};
NInstOld = size(IVar.IndexTable.TypeI, 1);

% Do a quick job in IndexTable.  This doesn't remove unused Types.
IVar.IndexTable.TypeI(IndexKill) = [];
IVar.IndexTable.RowK(IndexKill)  = [];

% Compute the kept indices
IndexSet = (1:NInstOld)';
IndexSet(IndexKill) = [];
