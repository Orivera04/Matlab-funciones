...
3. The textbook discusses the following algorithm:
    ...
    This finds a local minimum of a function f(x).
    The method is the golden ratio search.
    It is assumed that f(x) is unimodal over [a,b].

    To run the example for Algorithm 8.1 the user needs to use the script file 
    named a8_1.m.  This is accomplished by executing the Matlab command:

    a8_1 

4. The Matlab script in  a8_1.m  will call the subroutine named  golden.m 
    which is included in the sub directory or folder named  chap_8.
    ...
    in the proper sub directory or folder along with the script file a8_1.m  and
    function (subroutine) file  golden.m.  These commands are:
...
5. Once the process of writing a function M-files has been mastered,
    ...
    delete these lines from the script file  a8_1.m.
