function [Price, PriceTree] = compoundbyeqp(BinStockTree, varargin)
%COMPOUNDBYEQP Price compound options by an EQP binomial tree.
%
...
%   $Revision: 1.1.6.1 $  $Date: 2003/08/29 04:46:22 $
...
if nargin<10
    str1 = 'The function compoundbyeqp requires ten input arguments:';
    str2 = ['EQPTree, UOptSpec, UStrike, USettle, UExerciseDates,UAmericanOpt, ',...
             'COptSpec, CStrike, CSettle, and CExerciseDates'];
    msg = sprintf('%s\n%s', str1, str2);
    error('finderiv:compoundbyeqp:InvalidInputs',msg)
end

    
%Checking input arguments.
if ~isafin(BinStockTree, 'BinStockTree') | ~strcmpi(BinStockTree.Method, 'EQP')
    error('finderiv:compoundbyeqp:InvalidTree','EQPTree must be a Binary Stock Tree created with EQPTREE.')
end
