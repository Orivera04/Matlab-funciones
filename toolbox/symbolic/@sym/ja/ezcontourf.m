...
%
% EZCONTOURF(f)は、fが文字列か、または、2変数'x'と'y'の数学関数を表わす
% シンボリックな表現のとき、CONTOURFを使って、f(x,y)のコンターラインをプ
% ロットします。関数fは、デフォルトの領域 -2*pi < x < 2*pi, -2*pi < y < 
% 2*piにプロットされます。計算されるグリッドは、生ずる変化量に従って選択
% されます。
%
% EZCONTOURF(f,DOMAIN)は、デフォルトDOMAIN = [-2*pi,2*pi,-2*pi,2*pi]の代
% わりに、指定したDOMAINにfをプロットします。DOMAINは、4行1列のベクトル
% [xmin,xmax,ymin,ymax]、または、(a < x < b, a < y < bにプロットするため
% の)1行2列ベクトル[a,b]です。
%
% fが変数uとv(xとyではなく)の関数の場合、領域の終了点umin, umax, vmin, 
% vmax は、アルファベット順に並べ替えられます。そのため、EZCONTOURF(u^2-
%  v^3,[0,1],[3,6]) は、0 < u < 1, 3 < v < 6で、u^2 - v^3に対してコンタ
% ーラインをプロットします。
%
% EZCONTOURF(...,fig)は、figureウィンドウfigにデフォルトの領域でプロット
% します。
%
% 例題：
