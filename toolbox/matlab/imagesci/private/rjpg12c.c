 ...
 *      GRAY = rjpgc(filename); 
 * 
 * RGB is a mxnx3 uint16 array containing the 36-bit image stored in
 * the jpeg file filename.    
 * 
 * GRAY is a mxn uint16 array containing the 12-bit grayscale image
 ...
 * $Revision: 1.1.6.1 $  $Date: 2003/12/13 03:01:20 $
 */

static char rcsid[] = "$Id: rjpg12c.c,v 1.1.6.1 2003/12/13 03:01:20 batserve Exp $";
...
ReadRgbJPEG(j_decompress_ptr cinfoPtr)
{
    ...
    JSAMPARRAY buffer;
    int current_row;
    uint16_T *pr_red, *pr_green, *pr_blue;
    ...
    img = mxCreateNumericArray(3, dims, mxUINT16_CLASS, mxREAL);
...
ReadGrayJPEG(j_decompress_ptr cinfoPtr)
{
    ...
    img = mxCreateNumericArray(2, dims, mxUINT16_CLASS, mxREAL);
    ...
    pr_gray   = (uint16_T *) mxGetData(img);
