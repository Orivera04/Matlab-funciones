...
switch lower(method)
    ...
    case 'flattening' 
        L = compute_geometric_laplacian(vertex,face,laplacian);
        [U,S,V] = eigs(L, ndim+1, 'SM');
        if abs(S(1))<eps
            vertex1 = U(:,end-ndim+1:end);
        elseif abs(S(ndim))<eps
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function xy = compute_parameterization_free(vertex,face, lap_type)
...
if ~isstr(laplacian)
    ...
    L = compute_geometric_laplacian(vertex,face,lap_type);
end
