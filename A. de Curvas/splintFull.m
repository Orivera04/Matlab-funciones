function [yhat,aa,bb,cc,dd] = splintFull(x,y,xhat,opt1,opt2)
% splintFull  Cubic-spline interpolation with various end conditions; full matrix storage
%
% Synopsis:  yhat = splintFull(x,y,xhat)
%            yhat = splintFull(x,y,xhat,endType)
%            yhat = splintFull(x,y,xhat,fp1,fpn)
%            [yhat,a,b,c,d] = splintFull(x,y,xhat)
%            [yhat,a,b,c,d] = splintFull(x,y,xhat,endType)
%            [yhat,a,b,c,d] = splintFull(x,y,xhat,fp1,fpn)
%
...
A     = tridiag(n,bbeta,alpha,gamma);       %  Full tridiagonal matrix
...
% --- Evaluate the piecewise polynomials
xx = xhat - x(i);
yhat = a(i) + xx.*(b(i) + xx.*(c(i) + xx.*d(i)) ); % nested, vectorized evaluation

if nargout>1,  aa = a;  bb = b;  cc = c;  dd = d;  end  %  create optional outputs
