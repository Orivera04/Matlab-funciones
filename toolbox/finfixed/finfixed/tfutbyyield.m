function [QtdFutPrice, AccrIntEnd] = tfutbyyield(varargin)
%TFUTBYYIELD Future prices of T-bonds given spot curve and yield.
%   Prices of T-bond futures given a spot curve and bond yields at settlement.
%
%   [QtdFutPrice, AccrInt] = tfutbyyield(SpotCurve, Yield, SettleFut, ...
...
%
%        Yield - [Nx1] vector of yield to maturities at settlement date.
%
...
%   Interpolation   = 1;
% 
%   [QtdFutPrice, AccrInt] = tfutbyyield(SpotCurve, Yield, ...
...
%   $Revision: 1.6.6.4 $  $Date: 2004/04/06 01:09:14 $

if nargin<7
    error('finfixed:tfutbyyield:invalidInputs','Not enough input arguments. Try "help tfutbyprice".')
else
    ...
    if spotcol < 3
        if spotcol<2
            error('finfixed:tfutbyyield:invalidSpotCurve','Need at least 2 columns, [SpotDates , SpotRates]')
        else
    ...
    if min(spotdates) < SettleFut(1)
        error('finfixed:tfutbyyield:invalidSpotRateDate','First spot rate data must be on, or after SettleFut')
    end
    
    % check that Settlement date is a scalar
    if any(SettleFut ~= SettleFut(1))
        error('finfixed:tfutbyyield:invalidSettleFut',['SettleFut is a Valuation Date,'...
    ...
    if any(max(spotdates)<MatFut)
        error('finfixed:tfutbyyield:invalidSpotRateData',['Not enough SpotRateData. Please include spot rate up to:', sprintf('\n'), ...
                datestr(max(MatFut))])
    end
    ...
    if any(CouponRate<0)
        error('finfixed:tfutbyyield:invalidCouponRate',['Negative CouponRate detected, ',.... 
...
if nargin<8 | isempty(varargin{8})
    ...
    if numel(Interpolation)-1
        error('finfixed:tfutbyyield:scalarInterpolation','Interpolation must be a scalar 0,1,or 2')
    end
    
    if (Interpolation < 0 | Interpolation > 2)
        error('finfixed:tfutbyyield:invalidInterpolation','Unknown interpolation method, try 0, 1, or 2')
    end
end

% Size expander.
[Yield, SettleFut, MatFut, CF, CouponRate, Maturity] = ...
    finargsz(1, Yield(:), SettleFut(:), MatFut(:), ...
...
if any(SettleFut > MatFut)
    error('finfixed:tfutbyyield:invalidMatFut','Futures maturity must be greater than or equal to its settle')
end

if any(MatFut > Maturity)
    error('finfixed:tfutbyyield:invalidMaturity','Bond maturity must be greater than or equal to its settle')
end
...
% compute the corresponding Cash Price price
dcf = CFlowAmounts .* (1+Yield(:,ones(numcol,1))/2) .^ -TFactors;
dcf(isnan(dcf)) = 0;
Price = sum(dcf,2);
...
% Computing/interpolating for appropriate continuously compounded
% spotrate for intervening cash flows
...
lastcpnbfrMatFut = CFlowDates(sub2ind([numrow, numcol], [1:numrow]', idxnotnan));
...
denomend = CFlowDates(sub2ind([numrow, numcol], [1:numrow]', idxnotnan+1)) - ...
    lastcpnbfrMatFut;
