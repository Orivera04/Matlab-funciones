/*
 *  FIR_DF1_CR_RT.C - DSP FIR DF filter runtime helper function.
 *
 *  Copyright 1995-2003 The MathWorks, Inc.
 *  $Revision: 1.3.2.3 $  $Date: 2004/04/12 23:44:33 $
 */
#include "dsp_rt.h"

EXPORT_FCN void MWDSP_FIR_DF_CR(const creal32_T        *u,
                            creal32_T        *y,
    ...
    for (k=0; k < numChans; k++) {   /* channel loop */
        ...
        while (i--) {   /* frame loop */
            ...
            for (j=indexN; j < ordNUM; j++) {
                psum.re  += (filt_mem+j)->re * (*num);
                psum.im  += (filt_mem+j)->im * (*num++);
            }
            for (j=0; j < indexN; j++) {
                psum.re  += (filt_mem+j)->re * (*num);
                psum.im  += (filt_mem+j)->im * (*num++);
