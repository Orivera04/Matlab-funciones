...
if (nargin < 2)
   ...
   error('GARCH:aicbic:UnspecifiedNumParams' , ' Number of parameters ''NumParams'' must be specified.');

else

   if prod(size(numParams)) ~= length(numParams)    % Check for a vector.
      error('GARCH:aicbic:NonVectorNumParams' , ' ''NumParams'' must be a vector.');
   end

   numParams  =  numParams(:);

   if any(round(numParams) - numParams) | any(numParams <= 0)
      error('GARCH:aicbic:NonPositiveNumParams' , ' All elements of ''NumParams'' must be positive integers.')
   end
   ...
   if length(numParams) ~= length(LLF)
      error('GARCH:aicbic:VectorLengthMismatch1' , ' Length of ''NumParams'' and ''LLF'' must be the same.');
   end
...
if nargout >= 2
   ...
   if nargin < 3
      error('GARCH:aicbic:UnspecifiedNumObs' , ' Sample size ''NumObs'' is required for BIC.');
   end

   if prod(size(numObs)) ~= length(numObs)    % Check for a vector.
      error('GARCH:aicbic:NonVectorNumObs' , ' ''NumObs'' must be a vector.');
   end

   numObs  =  numObs(:);

   if any(round(numObs) - numObs) | any(numObs <= 0)
      error('GARCH:aicbic:NonPositiveNumObs' , ' All elements of ''NumObs'' must be positive integers.')
   end
   ...
   if length(numObs) ~= length(LLF)
      error('GARCH:aicbic:VectorLengthMismatch2' , ' Sizes of ''NumObs'' and ''LLF'' must be the same.');
   end
